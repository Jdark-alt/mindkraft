        ...
        function renderCombosPanel(log) {
            ...
            el.innerHTML = sorted.map(([pair, count]) => {
                ...
                var rowHtml = '<div class="combo-row">';
                rowHtml += '<div class="combo-names">';
                rowHtml += '<span class="combo-chip">' + escapeHtml(parts[0]) + '</span>';
                rowHtml += '<span class="combo-plus">+</span>';
                rowHtml += '<span class="combo-chip">' + escapeHtml(parts[1]) + '</span>';
                rowHtml += '</div>';
                rowHtml += '<div class="combo-count">' + count + '× performed together</div>';
                rowHtml += '</div>';
                return rowHtml;
            }).join('');
        }
        ...
        function renderCalendar() {
            ...
            const selectedCalId = calSel ? String(calSel.value) : '';
            const filtered = selectedCalId
                ? scopeFiltered.filter(a => String(a.id) === selectedCalId)
                : scopeFiltered;
        ...
        window.toggleInstallGuide = function() {
            ...
            const chevron = document.getElementById('installGuideChevron');
            const isOpen = body.classList.toggle('open');
            if (chevron) chevron.classList.toggle('open', isOpen);
            // Show Android fallback if no deferred prompt available
            if (isOpen) {
                var fallback = document.getElementById('androidInstallFallback');
                var btn = document.getElementById('androidInstallBtn');
                if (fallback && !window._pwaInstallPrompt) {
        ...
        window.triggerPwaInstall = function() {
            if (!window._pwaInstallPrompt) {
                document.getElementById('androidInstallFallback').style.display = 'block';
                return;
            }
        ...
        window.toggleCustomThemePanel = function() {
            ...
            var chevron = document.getElementById('themeCustomChevron');
            var btn = document.getElementById('themeCustomToggleBtn');
            if (!panel) return;
        ...
        function loadTheme() {
            ...
            // Show custom panel toggle (but don't auto-expand) if custom was active
            if (customActive) {
                var wrap = document.getElementById('themeCustomPanelWrap');
        ...
        window.applyThemePreset = function(id, el) {
            ...
            if (!t) return;
            // First reset all 11 custom colour vars to their defaults so no custom bleed-through
            CUSTOM_COLOR_VARS.forEach(function(v) {
            ...
            // Hide custom panel wrap + slot/reset buttons
            var wrap = document.getElementById('themeCustomPanelWrap');
        ...
        (function() {
            ...
            // Capture the install prompt early — expose on window for settings card button
            ...
            window.addEventListener('load', function() {
                // Hide the settings install card if already running as PWA
                if (isInstalled()) {
