        ...
        function renderChallenges() {
            ...
            // Update toolbar count pill
            const countEl = document.getElementById('challengeCount');
            if (countEl) {
                const active = challenges.filter(c => c.status === 'active').length;
                countEl.textContent = active === 0 ? '' : (active + ' active');
            }
        ...
        function renderChallengeCard(challenge, index) {
            ...
            // ── Hero progress block ───────────────────────────────────────
            //   - With metric:  show "X / Y unit" big
            //   - With per-activity, no metric: show "N%" big
            //   - Any-activity:  show "N completions" big
            let heroHtml = '';
            let mainPct = 0;
            if (hasMetric) {
                ...
                mainPct = metricPct;
                // Format numbers nicely — drop trailing zeros for integers
                const fmtNum = (v) => Number.isInteger(v) ? String(v) : (Math.round(v * 100) / 100).toString();
                heroHtml = `
                    <div class="ch-hero-row">
                        <span class="ch-hero-num">${fmtNum(metricCurrent)}</span>
                        <span class="ch-hero-sep">/</span>
                        <span class="ch-hero-target">${fmtNum(+challenge.metricQty)}</span>
                        <span class="ch-hero-unit">${escapeHtml(challenge.metricUnit || '')}</span>
                    </div>`;
            } else if (hasPerActivity) {
                mainPct = activityPct;
                heroHtml = `
                    <div class="ch-hero-row">
                        <span class="ch-hero-num">${Math.floor(activityPct)}<span style="font-size:0.6em;font-weight:600;color:var(--color-text-secondary);letter-spacing:0;">%</span></span>
                        <span class="ch-hero-target" style="margin-left:6px;">${totalCurrent} / ${totalTarget} done</span>
                    </div>`;
            } else {
                // Any-activity, no metric — show completion count
                const anyCount = challenge.currentCount || 0;
                mainPct = isCompleted ? 100 : 0;
                heroHtml = `
                    <div class="ch-hero-row">
                        <span class="ch-hero-num">${anyCount}</span>
                        <span class="ch-hero-unit" style="margin-left:6px;">${anyCount === 1 ? 'completion' : 'completions'}</span>
                    </div>`;
            }

            // ── Main progress bar ──────────────────────────────────────────
            const progressBar = `<div class="ch-bar"><div class="ch-bar-inner" style="width:${mainPct}%;"></div></div>`;

            // ── Metric update row (only on active metric challenges) ───────
            let metricUpdateHtml = '';
            if (isActive && hasMetric) {
                const cur = challenge.metricCurrent || 0;
                metricUpdateHtml = `
                <div class="ch-metric-update">
                    <input class="ch-metric-input" type="number" inputmode="decimal" id="metric-input-${challenge.id}"
                           placeholder="Update progress" step="any" min="0" max="${challenge.metricQty}"
                           value="${cur > 0 ? cur : ''}">
            ...
            if (isActive) {
                ...
                actionBtn = `
                    <button class="ch-complete${readyClass}" onclick="completeChallenge(${index})"
                        ...
                        ${checkSvg}<span>${allTargetsMet ? 'Claim bonus' : 'Complete challenge'}</span>
                    </button>`;
            } else if (isCompleted) {
                actionBtn = `
                ...
                actionBtn = `
                    <button class="ch-take-again" onclick="takeAgainChallenge(${index})" style="opacity:0.85;">
                        ${refreshSvg}<span>Retry</span>
                    </button>`;
            }

            // ── Activity breakdown rows (shown when chevron is expanded) ───
            ...
            // ── Breakdown chevron — shown only if there's a breakdown to show.
            //    We expose it when: per-activity rows exist (hasPerActivity), OR
            //    a metric challenge that also has activities tied to it.
            const showBreakdown = hasPerActivity;
            const breakdownCollapsed = challenge.activityProgressCollapsed !== false; // default collapsed
            let breakdownHtml = '';
            if (showBreakdown) {
                breakdownHtml = `
                <div class="ch-breakdown-head" onclick="toggleChallengeBreakdown('${challenge.id}')">
                    <div class="ch-breakdown-head-left">
                        <span>Challenge breakdown</span>
                        <span class="ch-breakdown-pct">${Math.floor(activityPct)}%</span>
                    </div>
                ...
                <div class="ch-breakdown-body${breakdownCollapsed ? '' : ' expanded'}" id="ch-breakdown-${challenge.id}"
                     style="max-height:${breakdownCollapsed ? '0' : '800px'};">
                    ${activityRowsHtml}
                </div>`;
            }

            // ── Subtitle: bonus XP · days left · activity count · enforced ──
            const metaParts = [];
