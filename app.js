        ...
        function renderCalendar() {
            ...
            // via a module-level variable so innerHTML rebuilds can't reset it.
            ...
            if (calSel) {
                calSel.innerHTML = '<option value="">All Activities</option>' +
                    scopeFiltered.map(a => `<option value="${a.id}" ${a.id === window._calendarSelId ? 'selected' : ''}>${escapeHtml(a.name)}</option>`).join('');
        ...
        // Capture the beforeinstallprompt event for Android one-tap install
        ...
        window.triggerAndroidInstall = function() {
            if (window._deferredInstallPrompt) {
                ...
                // Prompt not available (already installed or non-Android browser)
        ...
        function loadTheme() {
            ...
            if (customActive) {
                ...
                // Panel stays collapsed on load — user expands it when needed
            }
        }
        ...
        window.activateCustomTheme = function(el) {
            ...
            // Don't auto-open the panel — user can expand via the collapse header
        };

        window.applyThemePreset = function(id, el) {
            var t = THEMES.find(function(x){return x.id===id;});
            if (!t) return;
            // First, reset ALL custom colour vars to their defaults so no leftover
            // custom-theme values bleed through when switching to a preset.
            CUSTOM_COLOR_VARS.forEach(function(v) {
