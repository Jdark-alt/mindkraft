        ...
        // Returns { missed, lastExamined } where lastExamined is the last Date walked.
        function countMissedWindowsSince(activity, lastAccountedWindow) {
            ...
            if (!todayCycleStart) return { missed: 0, lastExamined: lastAccountedWindow };
            const todayCycleStartMs = todayCycleStart.getTime();

            let cursor = getNextCycleWindowStart(activity, lastAccountedWindow);
            if (!cursor || isNaN(cursor.getTime())) return { missed: 0, lastExamined: lastAccountedWindow };
            ...
            while (cursor.getTime() < todayCycleStartMs) {
                ...
                lastExamined = cursor;
                if (missed >= 7) break; // safety cap
                cursor = nextCursor;
            }
            return { missed, lastExamined };
        }

        // ── getLastAccountedWindow ────────────────────────────────────────
        // Derives the starting anchor for the missed-window walk.
        // Identical migration logic for both regular and skip-negative activities.
        function getLastAccountedWindow(activity) {
            ...
            // Migration: lastSkipCheckDate was the old system's stamp
            ...
            // Anchor from earliest positive completion
            if (activity.completionHistory && activity.completionHistory.length > 0) {
                ...
                if (earliest !== Infinity) {
                    return getCycleWindowStart(activity, new Date(earliest));
                }
            }
        ...
        function processStreakShields(activity, today) {
            ...
            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin) return true; // stamp only

            const { missed, lastExamined } = countMissedWindowsSince(activity, lastWin);
