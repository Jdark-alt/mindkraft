        ...
        // CONTRACT:
        //
        //   activity.streak         — stored integer, always authoritative for display.
        //                             Written on completion (+1) and at login (repaired).
        //   activity.streakPauseUses — stored integer 0-3, shields consumed this streak.
        //                             Written at login and on completion.
        //   activity.bestStreak     — highest streak ever reached. Never decremented.
        //   activity.streakGrantedDate — UTC date string, prevents double-grant in one day.
        //
        //   calculateStreak()       — READ ONLY. Returns stored value adjusted for
        //                             any misses since lastCompleted. NO history walk.
        //                             Safe to call on every render.
        //
        //   getShieldsUsedDisplay() — READ ONLY. Returns stored streakPauseUses plus
        //                             any live unacknowledged misses. NO history walk.
        //
        //   processStreakShields()  — WRITE. Called ONCE per day at login.
        //                             Walks up to 60 days of completionHistory to
        //                             verify/repair stored streak and streakPauseUses.
        //                             This is the only place history is consulted.
        //
        // SHIELD RULES:
        //   3 shields per streak. Each missed closed window costs 1 shield.
        //   Streak survives while shields remain. On 4th miss: streak=0, shields reset.
        //   Shields reset to 0 whenever streak resets.
        // ══════════════════════════════════════════════════════════════════
        ...
        // Returns the stored streak, zeroed if misses since lastCompleted
        // exceed available shields.
        function calculateStreak(activity) {
            if (!activity.lastCompleted) return 0;
            if (activity.frequency === 'occasional') return 0;

            // Perform-negative: no shields, expires after grace window
            ...
            // Count missed closed windows since last completion.
            // Anchored from lastCompleted so no history walk needed.
            const missed = _missedWindowsFromLastCompleted(activity);
            const shieldsLeft = Math.max(0, MAX_SHIELDS - (activity.streakPauseUses || 0));
            if (missed > shieldsLeft) return 0;
            return stored;
        }

        // ── getShieldsUsedDisplay ─────────────────────────────────────────
        // READ-ONLY, called on every render. No history walk.
        // Returns stored shields used + any live unacknowledged misses.
        function getShieldsUsedDisplay(activity) {
            ...
            const missed = _missedWindowsFromLastCompleted(activity);
            return Math.min(MAX_SHIELDS, (activity.streakPauseUses || 0) + missed);
        }

        // ── _missedWindowsFromLastCompleted ───────────────────────────────
        // PRIVATE. Counts closed cycle windows between the lastCompleted window
        // and today's open window that had no completion.
        // Anchored purely from lastCompleted — no history needed, no stored anchor.
        // Capped at MAX_SHIELDS+1 (4) — we only need to know if missed > MAX_SHIELDS.
        function _missedWindowsFromLastCompleted(activity) {
            if (!activity.lastCompleted) return 0;
            const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
            const todayCycleStart = getCycleWindowStart(activity, todayMidnight);
            if (!todayCycleStart) return 0;

            const lastCompWin = getCycleWindowStart(activity, new Date(activity.lastCompleted));
            if (!lastCompWin) return 0;

            // If last completion is in today's window or a future window, no misses
            if (lastCompWin.getTime() >= todayCycleStart.getTime()) return 0;

            // Count windows between lastCompWin (exclusive) and todayCycleStart (exclusive)
            let missed = 0;
            let cursor = getNextCycleWindowStart(activity, lastCompWin);
            const CAP = MAX_SHIELDS + 1;
            while (cursor && cursor.getTime() < todayCycleStart.getTime()) {
                missed++;
                if (missed >= CAP) return missed;
                cursor = getNextCycleWindowStart(activity, cursor);
            }
            return missed;
        }
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // Read live streak (stored value, adjusted for any misses since lastCompleted).
            // If broken, reset stored fields so new streak starts clean.
            ...
            // Persist any live unacknowledged misses into streakPauseUses now,
            // so the shield count stays in sync after a gap completion.
            if (!isOccasional && liveStreak > 0) {
                const liveMissed = _missedWindowsFromLastCompleted(activity);
                if (liveMissed > 0) {
        ...
        window.undoActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            if (!isOccasional && !stillHasToday && activity.streakGrantedDate === todayStr && activity.streak > 0) {
                ...
                activity.streakGrantedDate = null;
                // Undo shield consumption if shields were consumed during this completion
                if ((activity.streakPauseUses || 0) > 0) {
                    activity.streakPauseUses = Math.max(0, activity.streakPauseUses - 1);
                }
            }
        ...
        // Walks completionHistory backwards from lastCompleted, counting
        // consecutive completed windows. Scans at most MAX_HISTORY_DAYS (60)
        // of entries to bound runtime. Returns the consecutive-window count.
        ...
        // 1. Verifies/repairs stored activity.streak from recent history.
        // 2. Writes activity.streakPauseUses for missed windows.
        // 3. Resets streak+shields if missed windows > available shields.
        // This is the ONLY place completionHistory is walked.
        ...
        function processStreakShields(activity, today) {
            ...
            // ── Step 1: Verify/repair stored streak ───────────────────────
            // Derive from recent history (last 60 days). Take max(stored, history)
            // so users with streaks > 60 days keep their stored value.
            ...
            if (verifiedStreak !== stored) {
                ...
                if (verifiedStreak > (activity.bestStreak || 0)) {
                    activity.bestStreak = verifiedStreak;
                }
                changed = true;
            }

            // ── Step 2: Count missed windows since last completion ─────────
            const missed = _missedWindowsFromLastCompleted(activity);
            if (missed === 0) return changed || true;
            ...
            if (missed <= available) {
                ...
                // Streak broken — reset for fresh start
                activity.streak = 0;
                activity.streakGrantedDate = null;
                activity.streakPauseUses = 0;
                activity.streakPaused = false;
