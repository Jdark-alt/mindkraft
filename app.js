        ...
        function getTopActivitiesThisLevel() {
            const levelStart = window.userData.levelStartedAt ? new Date(window.userData.levelStartedAt) : null;
            const actXP = [];
            (window.userData.dimensions || []).forEach(dim => {
        ...
        async function buildLevelUpCard(newLevel) {
            ...
            const uOlive    = root.getPropertyValue('--color-accent-olive').trim()  || '#7a7b4d';
            const uGreen    = root.getPropertyValue('--color-accent-green').trim()  || '#6b7c3f';
            ...
            glow(180,  420, 680, uAccent,   0.18);
            glow(900, 1560, 700, uRed,      0.13);
            glow(540,  960, 460, uProgress, 0.10);
            ...
            // Diagonal rule lines
            ctx.save(); ctx.strokeStyle = uAccent; ctx.globalAlpha = 0.06; ctx.lineWidth = 1;
            ...
            function hRule(y) {
                ...
                g.addColorStop(0, 'transparent'); g.addColorStop(0.3, uAccent);
                g.addColorStop(0.7, uProgress);   g.addColorStop(1, 'transparent');
                ctx.strokeStyle = g; ctx.globalAlpha = 0.38; ctx.lineWidth = 1;
            ...
            // ══════════════════════════════════════════════
            const brandIconY = 124;

            // Icon circle — smaller
            ctx.save();
            const iconGrad = ctx.createRadialGradient(W/2, brandIconY, 0, W/2, brandIconY, 56);
            iconGrad.addColorStop(0, `rgba(${_hexToRgbStr(uProgress)}, 0.22)`);
            iconGrad.addColorStop(1, `rgba(${_hexToRgbStr(uAccent)}, 0.06)`);
            ctx.beginPath(); ctx.arc(W/2, brandIconY, 56, 0, Math.PI * 2);
            ctx.fillStyle = iconGrad; ctx.fill();
            ctx.strokeStyle = uAccent; ctx.globalAlpha = 0.3; ctx.lineWidth = 1.5; ctx.stroke();
            ctx.restore();

            // Reset font before emoji to use system emoji font on all platforms
            ctx.font = '64px serif';
            ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
            ctx.fillText('🧠', W/2, brandIconY);
            ctx.textBaseline = 'alphabetic';

            // Brand name
            ctx.font = '800 58px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.textAlign = 'center';
            const brandGrad = ctx.createLinearGradient(W/2 - 200, 0, W/2 + 200, 0);
            brandGrad.addColorStop(0, '#ffffff');
            brandGrad.addColorStop(1, uProgress);
            ctx.fillStyle = brandGrad;
            ctx.fillText('Mindkraft', W/2, 236);

            // Tagline — shorter
            ctx.font = '400 30px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = 'rgba(176,176,176,0.65)';
            ctx.fillText('Gamify your life.', W/2, 282);
            ...
            const pillY = 388;
            ctx.save();
            roundRect(W/2 - 160, pillY - 40, 320, 64, 32);
            ctx.fillStyle = `rgba(${_hexToRgbStr(uAccent)}, 0.15)`; ctx.fill();
            ctx.strokeStyle = uAccent; ctx.globalAlpha = 0.5; ctx.lineWidth = 1.5; ctx.stroke();
            ctx.restore();
            ctx.font = '700 34px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = uProgress; ctx.textAlign = 'center';
            ctx.fillText('LEVEL UP', W/2, pillY);

            // ══════════════════════════════════════════════
            // SECTION 2 — Hero level number (no ring, just glow)
            // ══════════════════════════════════════════════
            const heroCY = 680; // vertical center of hero zone

            // Soft glow behind number — in user's progress color
            glow(W/2, heroCY, 340, uProgress, 0.13);
            glow(W/2, heroCY, 180, uProgress, 0.12);

            // "LEVEL" label — small, above number
            ctx.font = '500 52px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = 'rgba(255,255,255,0.45)';
            ctx.textAlign = 'center'; ctx.textBaseline = 'alphabetic';
            ctx.fillText('LEVEL', W/2, heroCY - 100);

            // Level number — large, gradient
            const numFS = newLevel >= 100 ? 280 : 340;
            ctx.font = `800 ${numFS}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif`;
            ctx.textBaseline = 'middle';
            const numGrad = ctx.createLinearGradient(W/2, heroCY - 170, W/2, heroCY + 170);
            numGrad.addColorStop(0, '#ffffff');
            numGrad.addColorStop(0.5, uProgress);
            numGrad.addColorStop(1, uAccent);
            ctx.fillStyle = numGrad;
            ctx.fillText(String(newLevel), W/2, heroCY + 60);
            ctx.textBaseline = 'alphabetic';

            // ── Character title pill — right below number ──
            // catXP = all-time (for identity/title), levelCatXP = this level only (for stats)
            const catXP = getCategoryXP();
            const levelCatXP = levelStart
                ? getCategoryXPSince(levelStart)
                : getCategoryXP(); // fallback to all-time if no timestamp
            const charTitle = getCharacterTitle(newLevel, catXP);
            const ctY = heroCY + 280;

            ctx.font = '600 38px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.textAlign = 'center';
            const ctW = Math.min(ctx.measureText(charTitle).width + 100, 560);
            ctx.save();
            roundRect(W/2 - ctW/2, ctY - 48, ctW, 70, 35);
            ctx.fillStyle = `rgba(${_hexToRgbStr(uProgress)}, 0.12)`; ctx.fill();
            ctx.strokeStyle = uProgress; ctx.globalAlpha = 0.38; ctx.lineWidth = 1.5; ctx.stroke();
            ctx.restore();
            ctx.fillStyle = uProgress;
            ctx.fillText(charTitle, W/2, ctY);
            ...
            // ══════════════════════════════════════════════
            const sTop = ctY + 96;
            const sW = (W - PAD * 2 - 24) / 2;
            const sH = 178;

            function statCard(x, y, w, h, label, val, sub, ac) {
                ctx.save(); roundRect(x, y, w, h, 20);
                ctx.fillStyle = 'rgba(255,255,255,0.04)'; ctx.fill();
                ctx.strokeStyle = ac || uAccent; ctx.globalAlpha = 0.2; ctx.lineWidth = 1; ctx.stroke(); ctx.restore();
                ctx.font = '500 25px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                ctx.fillStyle = 'rgba(255,255,255,0.38)';
                ctx.textAlign = 'center';
                ctx.fillText(label.toUpperCase(), x + w/2, y + 44);
                ctx.font = `700 ${val.length > 8 ? 42 : 52}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif`;
                ctx.fillStyle = ac || uProgress;
                ctx.fillText(val, x + w/2, y + 108);
                if (sub) {
                    ctx.font = '400 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                    ctx.fillStyle = 'rgba(255,255,255,0.26)';
                    ctx.fillText(sub, x + w/2, y + 148);
                }
            }
            ...
            let xpThisLevel = 0;
            if (levelStart) {
                // Sum completions since this level started
                (window.userData.dimensions || []).forEach(dim => {
                    (dim.paths || []).forEach(path => {
                        (path.activities || []).forEach(act => {
                            (act.completionHistory || []).forEach(e => {
                                if (e.isPenalty) return;
                                if (new Date(e.date) < levelStart) return;
                                xpThisLevel += Math.abs(e.xp || 0);
                            });
                        });
                    });
                });
            } else {
                // No levelStartedAt — show current XP within level bar
                xpThisLevel = window.userData.currentXP || 0;
            }
            // If we have a levelStart but earned 0 XP (levelled up instantly in testing),
            // show current XP within the level bar so the card isn't blank
            if (xpThisLevel === 0) xpThisLevel = window.userData.currentXP || 0;

            statCard(PAD, sTop, sW, sH, 'XP This Level', xpThisLevel.toLocaleString(), getDaysSinceLastLevel() || '—', uProgress);
            statCard(PAD + sW + 24, sTop, sW, sH, 'Total XP', (window.userData.totalXP || 0).toLocaleString(), 'all time', uAccent);

            // ══════════════════════════════════════════════
            // SECTION 4 — Top activity
            // ══════════════════════════════════════════════
            const topActs = getTopActivitiesThisLevel();
            const aSecY = sTop + sH + 36;

            ctx.font = '500 25px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = 'rgba(255,255,255,0.32)';
            ctx.textAlign = 'left';
            ctx.fillText('TOP ACTIVITIES THIS LEVEL', PAD, aSecY);

            let actBlockBottom = aSecY + 16;

            if (topActs.length > 0) {
                const priH = 146;
                const priY = aSecY + 14;
                ctx.save(); roundRect(PAD, priY, W - PAD * 2, priH, 18);
                const ag = ctx.createLinearGradient(PAD, priY, W - PAD, priY);
                ag.addColorStop(0, `rgba(${_hexToRgbStr(uAccent)},0.15)`);
                ag.addColorStop(1, `rgba(${_hexToRgbStr(uProgress)},0.06)`);
                ctx.fillStyle = ag; ctx.fill();
                ctx.strokeStyle = uAccent; ctx.globalAlpha = 0.25; ctx.lineWidth = 1; ctx.stroke(); ctx.restore();

                ctx.font = '600 40px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                ctx.fillStyle = '#fff'; ctx.textAlign = 'left';
                let pName = topActs[0].name;
                const maxNW = W - PAD * 2 - 240;
                while (ctx.measureText(pName).width > maxNW && pName.length > 3) pName = pName.slice(0, -2) + '…';
                ctx.fillText(pName, PAD + 30, priY + 60);

                ctx.save(); roundRect(W - PAD - 200, priY + 24, 200, 50, 10);
                ctx.fillStyle = `rgba(${_hexToRgbStr(uProgress)},0.18)`; ctx.fill(); ctx.restore();
                ctx.font = '700 30px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                ctx.fillStyle = uProgress; ctx.textAlign = 'center';
                ctx.fillText(`+${topActs[0].xp.toLocaleString()} XP`, W - PAD - 100, priY + 58);

                if (topActs[0].streak > 0) {
                    ctx.font = '28px serif';
                    ctx.fillStyle = uOlive; ctx.textAlign = 'left';
                    ctx.fillText(`🔥 ${topActs[0].streak} day streak`, PAD + 30, priY + 112);
                }

                actBlockBottom = priY + priH + 14;

                // Secondary chips
                if (topActs.length > 1) {
                    const chipH = 60;
                    let chipX = PAD;
                    for (let ci = 1; ci < Math.min(topActs.length, 3); ci++) {
                        const act = topActs[ci];
                        ctx.font = '400 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                        const cNW = ctx.measureText(act.name).width;
                        const cXW = ctx.measureText(` · ${act.xp.toLocaleString()} XP`).width;
                        const chipW = Math.min(cNW + cXW + 44, (W - PAD * 2 - 16) / 2);
                        ctx.save(); roundRect(chipX, actBlockBottom, chipW, chipH, 12);
                        ctx.fillStyle = 'rgba(255,255,255,0.05)'; ctx.fill();
                        ctx.strokeStyle = 'rgba(255,255,255,0.09)'; ctx.lineWidth = 1; ctx.stroke(); ctx.restore();
                        let cN = act.name;
                        while (ctx.measureText(cN).width > chipW - 56 && cN.length > 3) cN = cN.slice(0, -2) + '…';
                        ctx.font = '400 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                        ctx.fillStyle = 'rgba(255,255,255,0.62)'; ctx.textAlign = 'left';
                        ctx.fillText(cN, chipX + 16, actBlockBottom + 38);
                        ctx.fillStyle = uProgress;
                        ctx.fillText(` · ${act.xp.toLocaleString()}`, chipX + 16 + ctx.measureText(cN).width, actBlockBottom + 38);
                        chipX += chipW + 14;
                        if (chipX + 180 > W - PAD) break;
                    }
                    actBlockBottom += chipH + 14;
                }
            } else {
                ctx.font = '400 26px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                ctx.fillStyle = 'rgba(255,255,255,0.18)'; ctx.textAlign = 'left';
                ctx.fillText('Complete activities to see your top performers here.', PAD, aSecY + 64);
                actBlockBottom = aSecY + 84;
            }

            // ══════════════════════════════════════════════
            // SECTION 5 — Most active areas
            // ══════════════════════════════════════════════
            // Use level-filtered category XP; fall back to all-time if this level has no data
            const levelTop2Source = Object.values(levelCatXP).some(v => v > 0) ? levelCatXP : catXP;
            const top2 = getTop2Categories(levelTop2Source);
            if (top2.length > 0) {
                const catSecY = actBlockBottom + 32;
                ctx.font = '500 25px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                ctx.fillStyle = 'rgba(255,255,255,0.32)';
                ctx.textAlign = 'left';
                ctx.fillText('MOST ACTIVE AREAS', PAD, catSecY);
                const catW = (W - PAD * 2 - 24) / 2;
                const catH = 144;
                top2.forEach((cat, i) => {
                    const cx = PAD + i * (catW + 24), cy = catSecY + 14;
                    ctx.save(); roundRect(cx, cy, catW, catH, 18);
                    ctx.fillStyle = `rgba(${_hexToRgbStr(cat.color)},0.1)`; ctx.fill();
                    ctx.strokeStyle = cat.color; ctx.globalAlpha = 0.3; ctx.lineWidth = 1; ctx.stroke(); ctx.restore();
                    ctx.font = '52px serif'; ctx.textAlign = 'left';
                    ctx.fillText(cat.emoji, cx + 20, cy + 72);
                    ctx.font = '600 32px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif'; ctx.fillStyle = cat.color;
                    ctx.fillText(cat.label, cx + 88, cy + 64);
                    ctx.font = '400 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.38)';
                    ctx.fillText(`${cat.xp.toLocaleString()} XP`, cx + 88, cy + 104);
                });
                actBlockBottom = catSecY + 14 + catH;
            }

            // ══════════════════════════════════════════════
            // SECTION 6 — Best streak (only if fits above footer)
            // ══════════════════════════════════════════════
            const FOOTER_TOP = H - 160;
            const bestStreak = getBestActiveStreak();
            if (bestStreak && bestStreak.streak >= 3) {
                const strH = 108;
                const strY = actBlockBottom + 28;
                // Only render if it fits cleanly above the footer
                if (strY + strH < FOOTER_TOP - 20) {
                    ctx.save(); roundRect(PAD, strY, W - PAD * 2, strH, 18);
                    const sg = ctx.createLinearGradient(PAD, strY, W - PAD, strY);
                    sg.addColorStop(0, `rgba(${_hexToRgbStr(uOlive)},0.13)`);
                    sg.addColorStop(1, `rgba(${_hexToRgbStr(uGreen)},0.06)`);
                    ctx.fillStyle = sg; ctx.fill();
                    ctx.strokeStyle = uOlive; ctx.globalAlpha = 0.25; ctx.lineWidth = 1; ctx.stroke(); ctx.restore();
                    ctx.font = '500 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                    ctx.fillStyle = 'rgba(255,255,255,0.32)'; ctx.textAlign = 'left';
                    ctx.fillText('BEST ACTIVE STREAK', PAD + 30, strY + 38);
                    ctx.font = '38px serif'; ctx.fillStyle = uOlive;
                    ctx.fillText(`🔥 ${bestStreak.streak} days`, PAD + 30, strY + 86);
                    ctx.font = '400 26px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
                    ctx.fillStyle = 'rgba(255,255,255,0.28)'; ctx.textAlign = 'right';
                    let sn = bestStreak.name;
                    while (ctx.measureText(sn).width > 360 && sn.length > 3) sn = sn.slice(0, -2) + '…';
                    ctx.fillText(sn, W - PAD - 30, strY + 70);
                }
            }

            // ── Footer ──
            hRule(FOOTER_TOP);
            // Share text — "I reached Level X on Mindkraft!"
            ctx.font = '500 30px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = 'rgba(255,255,255,0.55)'; ctx.textAlign = 'center';
            ctx.fillText('I reached Level ' + newLevel + ' on Mindkraft!', W/2, FOOTER_TOP + 44);
            ctx.font = '400 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif';
            ctx.fillStyle = 'rgba(255,255,255,0.22)';
            ctx.fillText('Gamify your life at mindkraft.life', W/2, FOOTER_TOP + 84);

            return new Promise((resolve, reject) => {
                try {
                    canvas.toBlob(blob => {
                        if (blob) { resolve(blob); return; }
                        // toBlob returned null — try lower quality (iOS memory limit)
                        canvas.toBlob(blob2 => {
        ...
        // ── Share level-up card via Web Share API (with download fallback) ─
        // Pre-built card blob — populated as soon as level-up fires, so it's ready
        // before the user taps Share (avoids Android's user-gesture expiry on async ops)
        window._levelUpCardCache = null;

        window.prebuildLevelUpCard = async function(level) {
            if (!window.userData || !window.currentUser) return; // guard: not logged in yet
            try {
                window._levelUpCardCache = null;
                ...
                // Non-fatal — shareLevelUpCard will build on demand as fallback
            }
        };
        ...
        window.shareLevelUpCard = async function(level) {
            ...
            try {
                // Use pre-built blob if available and for the right level — avoids gesture expiry
                let blob;
                if (window._levelUpCardCache && window._levelUpCardCache.level === level) {
                    blob = window._levelUpCardCache.blob;
                } else {
                    // Fallback: build now (may fail on Android due to gesture timing)
                    if (btn) { btn.disabled = true; btn.textContent = 'Building…'; }
                    blob = await buildLevelUpCard(level);
                }

                const file = new File([blob], `mindkraft-level-${level}.png`, { type: 'image/png' });

                // Try Web Share API with file (iOS Safari, Android Chrome)
                ...
                if (_canShareFiles) {
                    await navigator.share({
                        files: [file],
                        title: `I just hit Level ${level} on Mindkraft!`,
                        text: `Leveled up to ${level} on Mindkraft — gamify your life 🎮`,
                    });
                    showToast('Shared! 🎉', 'olive');
                } else if (navigator.share) {
                    // File sharing not supported — share text+url only
                    await navigator.share({
                        title: `I just hit Level ${level} on Mindkraft!`,
                        text: `I reached Level ${level} on Mindkraft — Gamify your life at mindkraft.life 🎮`,
                    });
                    showToast('Shared! 🎉', 'olive');
                } else {
                    // Download fallback — <a> click works on Android/desktop;
                    // iOS Safari in standalone mode needs window.open instead
                    const url = URL.createObjectURL(blob);
                    const isIosSafari = /iP(hone|ad|od)/.test(navigator.userAgent) && /WebKit/.test(navigator.userAgent);
                    if (isIosSafari) {
                        window.open(url, '_blank');
                        showToast('Image opened — press and hold to save, then share 📸', 'blue');
                    } else {
                        const a = document.createElement('a');
                        a.href = url; a.download = `mindkraft-level-${level}.png`; a.click();
                        showToast('Card downloaded — share it anywhere! 🎉', 'blue');
                    }
                ...
                // Last resort: try downloading
                try {
                    if (window._levelUpCardCache && window._levelUpCardCache.level === level) {
                        const url = URL.createObjectURL(window._levelUpCardCache.blob);
                        const isIosSafari2 = /iP(hone|ad|od)/.test(navigator.userAgent) && /WebKit/.test(navigator.userAgent);
                        if (isIosSafari2) {
                            window.open(url, '_blank');
                            showToast('Image opened — press and hold to save 📸', 'blue');
                        } else {
                            const a = document.createElement('a');
                            a.href = url; a.download = `mindkraft-level-${level}.png`; a.click();
                            showToast('Card downloaded — share it anywhere! 🎉', 'blue');
                        ...
                        showToast('Could not share — try again', 'red');
                    }
                } catch(e2) {
                    showToast('Could not share — try again', 'red');
                }
        ...
        // calculateStreak — READ-ONLY, never mutates activity fields.
        // Safe to call from rendering code without causing silent data changes.
        function calculateStreak(activity) {
            if (!activity.lastCompleted) return activity.streak || 0;
            ...
            const graceDays = getStreakGraceDays(activity);
            const missedCycles = daysDiff <= graceDays ? 0 : Math.ceil((daysDiff - graceDays) / graceDays);

            // Perform-negative: no shields, streak resets on any miss
            ...
            const shieldsLeft = Math.max(0, 3 - usedShields);

            if (missedCycles === 0)          return activity.streak || 1;
            if (missedCycles <= shieldsLeft) return activity.streak || 1; // shields cover it
            return 0;
        }

        // consumeStreakShields — WRITE path only.
        // Called exclusively from completeActivity so shields are consumed exactly
        // once per completion, never silently during a render pass.
        function consumeStreakShields(activity) {
            ...
            if (missedCycles <= shieldsLeft) {
                activity.streakPauseUses = usedShields + missedCycles;
                activity.streakPaused = false;
            } else {
                // Streak already broken — reset shields for the new streak
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // Consume streak shields on the write path only (not during rendering)
            if (!isOccasional) consumeStreakShields(activity);
        ...
        window.undoActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // Revert streak grant only if no user completions remain today (exclude penalties)
        ...
        // Auto-shield consumption happens in consumeStreakShields() (write path only).
        async function processStreakPauses() {
            ...
            // Persist any changes (penalties OR expired pauses) to Firestore
            if (anyChanged) {
        ...
        function processSkipPenalty(activity, today) {
            ...
            // Determine the most recent real (positive-XP) completion as our origin.
            // NEVER use new Date(null) — that gives Jan 1 1970 and causes a runaway
            // penalty scan spanning thousands of days.
            let lastPositiveDate = null;
            const posEntries = (activity.completionHistory || []).filter(e => (e.xp || 0) > 0);
            if (posEntries.length > 0) {
                lastPositiveDate = new Date(posEntries[posEntries.length - 1].date);
            } else if (activity.lastCompleted) {
                lastPositiveDate = new Date(activity.lastCompleted);
            }

            // Guard: never penalise a brand-new activity with no completion history
            if (!lastPositiveDate || isNaN(lastPositiveDate.getTime())) {
            ...
            //   • If we have run before (prevCheckStr exists), resume from that day —
            //     the user may have missed it.
            //   • Otherwise start the day after the last positive completion.
            const prevCheckStr = activity.lastSkipCheckDate;
            let startDay;
            if (prevCheckStr) {
                startDay = new Date(prevCheckStr + 'T00:00:00');
            } else {
                const ref = new Date(lastPositiveDate); ref.setHours(0,0,0,0);
                startDay = new Date(ref);
                startDay.setDate(startDay.getDate() + cycleDays);
            }
            startDay.setHours(0, 0, 0, 0);

            // Stamp today BEFORE scanning so re-entry is blocked even on early return
            activity.lastSkipCheckDate = today;

            const todayMidnightMs = todayMidnight.getTime();
            let missedCycles = 0;
            const cursor = new Date(startDay);
            while (cursor.getTime() < todayMidnightMs) {
