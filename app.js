        ...
        window.analyticsState = {
            ...
            chartTimeRange: 'all',
            chartOverlays: [],
            hideMain: false,  // hide Overall line when comparing
        };
        window.calendarOffset = 0;
        window.xpChartInstance = null;

        // ── Toggle analytics filter panel ─────────────────────────────
        ...
        // ── Generic section collapse toggle (XP lb, streak lb) ───────
        ...
        window.setChartMode = function(mode, btn) {
            ...
            // support both old filter-pill buttons and new an-mode-tab buttons
        ...
        function renderAnalyticsSummary(activities, log) {
            ...
            // XP today from period log (calendar-day match)
            const todayStr = localToday();
            ...
            // Highest LIVE streak (current, not historic best)
        ...
        function renderXPChart(fullLog) {
            ...
            // Apply hideMain — filter out main series if toggle is on and overlays exist
        ...
        function renderXPLeaderboard(activities, log) {
            ...
            el.innerHTML = ranked.map((a, i) => {
                ...
                const medal = i === 0 ? '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#f5c563" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-left:3px;"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>' : '';
                return `<div class="rank-row">
                    <span class="rank-num">#${i+1}</span>
                    <span class="rank-label" title="${escapeHtml(a.name)}">${escapeHtml(a.name)}${medal}</span>
        ...
        function renderStreakBoard(activities) {
            ...
            const ranked = [...activities]
                ...
                .slice(0, 10);
            if (!ranked.length) { el.innerHTML = '<div class="empty-state" style="padding:24px 0"><p>No streaks yet</p></div>'; return; }
            const max = ranked[0].streak || 1;
            el.innerHTML = ranked.map((a, i) => {
                const dimColor = DIM_HEX_MAP[a.dimColor] || '#fb923c';
                const pct = ((a.streak||0) / max * 100).toFixed(1);
                const rankLabel = i === 0
                    ? '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#f5c563" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>'
                    : `#${i+1}`;
                return `<div class="rank-row">
                    ...
                    <span class="rank-value">${a.streak}d</span>
                </div>`;
            }).join('');
        }
        ...
        function renderFrequencyChart(activities, log) {
            ...
            el.innerHTML = ranked.map((a, i) => {
                ...
                const topBadge = i === 0
                    ? '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#f5c563" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-left:3px;"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>'
                    : '';
                return `<div class="rank-row">
                    <span class="rank-label" style="width:130px;" title="${escapeHtml(a.name)}">${escapeHtml(a.name)}${topBadge}</span>
                    <div class="rank-bar-track">
        ...
        function renderCalendar() {
            ...
            // Resolve accent color from CSS variable for consistent theming
            const accentRgb = '90,159,212'; // matches --color-progress: #5a9fd4
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                const bg = count > 0 ? `rgba(${accentRgb},${intensity.toFixed(2)})` : '';
