        ...
        function getLastAccountedWindow(activity) {
            ...
            // First run for all other activities: anchor to the window containing
            // lastCompleted. This means we only check windows AFTER the last completion,
            // never re-walking old history that may have been pruned from completionHistory.
            if (activity.lastCompleted) {
                return getCycleWindowStart(activity, new Date(activity.lastCompleted));
            }
            if (activity.createdAt) {
                return getCycleWindowStart(activity, new Date(activity.createdAt));
            }
            return null;
        }
        ...
        function processStreakShields(activity, today) {
            ...
            // ── Streak recovery: undo incorrect streak=0 from bad migration ──────
            // If streak is 0 but the user completed the activity recently (within the
            // grace window for their frequency), the zero was written by the bad
            // getLastAccountedWindow migration path. Restore from bestStreak.
            if ((activity.streak || 0) === 0 && activity.lastCompleted && activity.bestStreak > 0) {
                const lastMidnight = new Date(activity.lastCompleted); lastMidnight.setHours(0,0,0,0);
                const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
                const daysDiff = Math.round((todayMidnight - lastMidnight) / (1000 * 60 * 60 * 24));
                const graceDays = getStreakGraceDays(activity);
                if (daysDiff <= graceDays + 1) {
                    // Recent completion — streak was incorrectly wiped. Restore it.
                    activity.streak = activity.bestStreak;
                    activity.streakPauseUses = 0;
                    // Stamp lastAccountedWindow to lastCompleted's window so we only
                    // check genuinely new misses going forward.
            ...
            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin) {
                // No anchor at all — stamp today's window and exit safely.
            ...
            // Advance lastAccountedWindow to prevent double-counting on next login.
            // Do this regardless of penalty outcome.
            let advanced = false;
            ...
            if (missed <= available) {
                // Shields absorb all misses — streak survives, mark dots consumed
                activity.streakPauseUses = usedShields + missed;
            } else {
                // More misses than shields — streak breaks, reset for new streak
                activity.streak = 0;
                activity.streakGrantedDate = null;
                activity.streakPauseUses = 0;
                activity.streakPaused = false;
