                ...
                if (user) {
                    ...
                    if (!window.userData.onboardingComplete &&
                        ...
                        if (ts >= 0 && ts < 4) {
                            ...
                            // Check if categorization nudge should fire
                            setTimeout(() => checkCategorizationPrompt(), 1000);
                        }
                    }
                } else {
                    window.currentUser = null;
        ...
        function renderActivityCards(activities) {
            ...
            return activities.map(activity => {
                ...
                const shieldsLeft = Math.max(0, getActivityShieldCap(currentStreak) - shieldsUsed);
                let shieldBadge = '';
                ...
                // Streak milestone shield hint — show once per tier, disappears at next tier
                if (currentStreak > 0 && !isOccasional) {
                    const nextMilestone = SHIELD_MILESTONES.find(m => m > currentStreak);
                    if (currentStreak >= 100) {
                        detailBadges.push('<span class="activity-badge badge-streak-maxed" title="XP bonus locked in at max — your streak keeps growing!">👑 Streak maxed</span>');
                    } else if (nextMilestone) {
                        const daysAway = nextMilestone - currentStreak;
                        detailBadges.push(`<span class="activity-badge badge-streak-hint" title="Reach a ${nextMilestone}-day streak to earn an extra shield">🛡 +1 in ${daysAway}d</span>`);
                    }
                }
        ...
        function renderActivityGridCards(activities) {
            ...
            activities.forEach(function(activity) {
                ...
                var shieldsLeft = Math.max(0, getActivityShieldCap(currentStreak) - shieldsUsed);
        ...
        window.saveActivity = async function(event) {
            ...
            if (!isNewActivity) {
                ...
                // Advance tutorial from step 0 → 1
                if ((window.userData.tutorialStep ?? -1) === 0) {
                    window.userData.tutorialStep = 1;
                }
            }
            ...
            // Trigger tutorial step 1 after first activity creation
            if (isNewActivity && window.userData.tutorialStep === 1) {
                setTimeout(() => showCurrentTutorialStep(), 400);
            }
        };
        ...
        //   activity.shieldsConsumed   — shields used this streak (0–cap).
        ...
        //   getActivityShieldCap()    — returns dynamic shield cap for the activity
        //
        // SHIELD RULES:
        //   Base: 3 shields per streak. Each missed closed window costs 1 shield.
        //   Milestone bonus: +1 shield at streak 25 / 50 / 75 / 100 (max +4).
        //   Shield cap: 10 absolute max (7 from streak system, 3 reserved for
        //   future mechanics). Cap is evaluated from the STORED streak entering
        //   the walk, so the user's established streak level determines protection.
        //   Shields reset automatically when a new streak begins.
        //
        // XP BONUS:
        //   Streak multiplier (calculateConsistencyMultiplier) is capped at streak
        //   100 — the bonus locks in at its day-100 value forever. The streak
        //   counter itself continues to climb with no limit.
        // ══════════════════════════════════════════════════════════════════

        const BASE_SHIELDS        = 3;
        const SHIELD_MILESTONES   = [25, 50, 75, 100];
        const SHIELD_ABSOLUTE_CAP = 10;

        // Returns the shield cap for a given streak level.
        // Base 3, +1 per milestone threshold the streak meets or exceeds.
        function getActivityShieldCap(streak) {
            const milestoneBonus = SHIELD_MILESTONES.filter(m => (streak || 0) >= m).length;
            return Math.min(SHIELD_ABSOLUTE_CAP, BASE_SHIELDS + milestoneBonus);
        }
        ...
        function calculateConsistencyMultiplier(streak) {
            ...
            // XP bonus locks in at the day-100 value. Streak keeps climbing — only
            // the multiplier is capped so long-streak users aren't disadvantaged.
            const cappedStreak = Math.min(streak, 100);
            var exp = getStreakScaling();
            return +(1 + 0.1 * Math.pow(cappedStreak, exp)).toFixed(2);
        }
        ...
        function recomputeStreakFromHistory(activity) {
            ...
            // Build cheat sheet: one pass over history to map each window.
            // Key: window-start timestamp (number). Present in Set = at least one
            // completion fell inside that window. Every walk-step below becomes a
            // single O(1) Set.has() lookup instead of scanning the full history
            ...
            // Start backwards walk from most recently CLOSED window
            let cursor = getCycleWindowStart(activity, new Date(todayWindow.getTime() - 1));
            if (!cursor) {
                const hasToday = completedWindowSet.has(todayWindow.getTime());
                activity.streak = hasToday ? 1 : 0;
                activity.shieldsConsumed = 0;
                activity.streakStartWindow = hasToday ? todayStr : null;
                activity.streakGrantedDate = hasToday ? todayStr : null;
            ...
            // streakStartWindow floor: mirrors processStreakSystem exactly.
            // Prevents the walk from bridging across a previous streak break —
            // without this, old completions on the far side of a break would
            // merge with the current streak via shield consumption.
            ...
            // Use stored streak for cap: retroactive recompute should respect the
            // milestone tier the user had earned going into this recalculation.
            let shieldsLeft = getActivityShieldCap(activity.streak || 0);
            const MAX_WALK = 400;
            ...
            for (let i = 0; i < MAX_WALK; i++) {
                if (!cursor || cursor < walkFloor) break;
                // Single O(1) lookup — no inner Array.some() scan
                const hit = completedWindowSet.has(cursor.getTime());
                if (hit) {
                    streak++;
                    streakStartWindow = cursor;
        ...
        function checkStreakMilestone(activityName, streak) {
            ...
            const shieldTiers = new Set([25, 50, 75, 100]);
            const isShieldTier = shieldTiers.has(streak);
            const emojis = { 7:'🔥', 14:'⚡', 25:'🛡', 30:'🌟', 50:'🛡', 60:'💎', 75:'🛡', 100:'👑' };
            const label = isShieldTier
        ...
        //
        // SHIELD RULES:
        //   Base 3 shields. +1 per milestone (25/50/75/100) met by the stored streak
        //   entering the walk. Cap evaluated before the walk so the user's established
        //   streak level determines miss-tolerance. Shields reset on new streak.
        ...
        function processStreakSystem(activity, today) {
            ...
            let streak = 0;
            let shieldsConsumed = 0;
            // Evaluate shield cap from stored streak before the walk so the user's
            // established milestone level determines their miss-tolerance.
            const storedStreakForCap = activity.streak || 0;
            let shieldsLeft = getActivityShieldCap(storedStreakForCap);
            const MAX_WALK = 400;
            ...
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        ...
                        let migShieldsLeft = BASE_SHIELDS; // legacy migration — no milestone data
        ...
        //   -1 / undefined = not started (no tutorial needed)
        //    0 = waiting for user to create first activity
        //    1 = activity created → show Friends tab intro
        //    2 = Friends seen → show Challenges tab intro
        //    3 = Challenges seen → show Analytics tab intro
        //    4 = complete
        ...
        const TUTORIAL_STEPS = [
            ...
            {
                emoji: '🏆',
                title: 'Meet the Challenges tab',
                body: 'Set personal challenges tied to your activities — with a deadline and a bonus XP reward when you complete them.',
                cta: 'Got it →',
                action: 'advanceTutorial()',
                tab: 'challenges'
            },
            {
                emoji: '📊',
                title: 'Meet the Analytics tab',
                body: 'Track your XP over time, see which habits are building streaks, and measure progress across every area of your life.',
                cta: 'Got it →',
                action: 'advanceTutorial()',
                tab: 'analytics'
            }
        ...
        window.advanceTutorial = async function() {
            const step = window.userData.tutorialStep ?? 0;
            const nextStep = step + 1;
            window.userData.tutorialStep = nextStep;
            await saveUserData().catch(() => {});
            hideTutorialOverlay();
            if (nextStep < TUTORIAL_STEPS.length) {
                setTimeout(() => showCurrentTutorialStep(), 350);
            } else {
                // Tutorial complete — switch back to activities
                switchTab('activities');
                showToast('🎉 Tour complete! You\'re all set.', 'blue');
            }
        };
