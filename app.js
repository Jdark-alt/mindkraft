        ...
        function renderActivitiesList() {
            ...
            if (sort === 'smart') {
                ...
                userGroups.forEach(function(g) {
                    (g.activityIds || []).forEach(function(aid) { actGroupMap[aid] = g.id; });
                });
                ...
                // Order: active-now first, then upcoming (by minutes-until-start),
                // then all-day, then past-today (timed groups whose window ended).
                ...
                function _bucketRank(g) {
                    ...
                    var s = _groupTimeToMin(g.timeStart);
                    if (s < 0) return 2; // all-day
                    var delta = s - nowMin;
                    if (delta > 0) return 1; // upcoming today
                    return 3; // already passed
                }
                ...
                orderedGroups.forEach(function(g) {
                    ...
                    var hex = DIM_HEX_MAP[g.color] || DIM_HEX_MAP.blue;
                    ...
                    html += '<div class="act-group routine-group'
                        + (isActive ? ' is-active' : '') + '" data-group="' + key + '" style="--routine-color:' + hex + ';">'
                        + '<div class="act-group-header routine-group-header" onclick="toggleActivityGroup(\'' + key + '\')">'
                        + '<span class="collapse-icon ' + (isExpanded ? 'expanded' : '') + '">▼</span>'
                        + '<span class="routine-color-dot"></span>'
        ...
        function renderActivityGridCards(activities) {
            ...
            // Build group lookup — derived from canonical userData.groups so
            // the grid view stays in sync with the list view's groups.
            var actGroupMap = {};
            var groupOrder = [];
            var _userGroups = getGroups();
            if (_userGroups.length) {
                // Order groups same as list view: active → upcoming → all-day → past
                var _now = new Date();
                ...
                _ordered.forEach(function(g) {
                    ...
                    // Lazy-populate the legacy shim so the renderer below can keep using it.
                    window._gridGroups[g.id] = {
                        name: g.name,
                        color: DIM_HEX_MAP[g.color] || DIM_HEX_MAP.blue,
        ...
        function getGroups() {
            return (window.userData && window.userData.groups) || [];
        }
        ...
        // Is "now" inside the group's active window?
        // All-day groups (no timeStart/timeEnd) are NEVER active — they have no
        // temporal claim on attention and shouldn't auto-expand. Wrap-around
        ...
        // Id of the timed group whose window contains now, or null.
        function getActiveGroupId(nowDate) {
            var d = nowDate || new Date();
            var groups = getGroups();
            for (var i = 0; i < groups.length; i++) {
                if (isGroupActiveNow(groups[i], d)) return groups[i].id;
            }
            return null;
        }

        // Id of the timed group whose start is next in the future. An early-
        // morning group is "upcoming" late at night (wraps to tomorrow).
        function getNextUpcomingGroupId(nowDate) {
            ...
            for (var i = 0; i < groups.length; i++) {
                var s = _groupTimeToMin(groups[i].timeStart);
                if (s < 0) continue;
                var delta = s - nowMin;
                if (delta <= 0) delta += 1440;
                if (delta < bestDelta) { bestDelta = delta; bestId = groups[i].id; }
            }
            return bestId;
        }
        ...
        // Assign an activity to a group (moves from any previous group).
        // Pass groupId=null to clear membership entirely.
        ...
        var _groupModalSelectedColor  = 'blue';
        var GROUP_COLOR_TOKENS = ['blue', 'green', 'olive', 'red'];
        ...
        window.openGroupModal = function(groupId) {
            ...
            if (groupId) {
                ...
                startEl.value = g.timeStart || '07:00';
                endEl.value   = g.timeEnd   || '10:00';
                _groupModalSelectedColor = g.color || 'blue';
                ...
                // Default time = current ±1h (clamped to 0..23)
                var now = new Date();
                var sH = Math.max(0,  now.getHours() - 1);
                var eH = Math.min(23, now.getHours() + 2);
                startEl.value = (sH < 10 ? '0' : '') + sH + ':00';
                endEl.value   = (eH < 10 ? '0' : '') + eH + ':00';
                _groupModalSelectedColor = 'blue';
        ...
        function renderGroupColorSwatches() {
            var row = document.getElementById('groupColorSwatches');
            if (!row) return;
            row.innerHTML = GROUP_COLOR_TOKENS.map(function(c) {
                var sel = c === _groupModalSelectedColor ? ' selected' : '';
                var hex = DIM_HEX_MAP[c] || '#4a7c9e';
                return '<div class="group-color-swatch' + sel + '" data-color="' + c + '"'
        ...
        window.selectGroupColor = function(color) {
            _groupModalSelectedColor = color;
            renderGroupColorSwatches();
        };
        ...
        window.renderGroupActPicker = function() {
            ...
            list.innerHTML = allActs.map(function(a) {
                var otherGroup     = findGroupForActivity(a.id);
                var isInThisGroup  = _groupModalSelectedActIds.has(a.id);
                var isInOtherGroup = otherGroup && otherGroup.id !== editingId;
                var checked        = isInThisGroup ? 'checked' : '';
                var selCls         = isInThisGroup ? ' selected' : '';
                var meta           = isInOtherGroup
                    ? '<span class="group-act-item-already">in ' + escapeHtml(otherGroup.name) + '</span>'
                    : '<span class="group-act-item-meta">' + escapeHtml(a.dimName || '') + '</span>';
        ...
        window.saveGroupFromModal = async function(event) {
            ...
            var color  = _groupModalSelectedColor || 'blue';
            var actIds = Array.from(_groupModalSelectedActIds || []);

            if (editingId) {
                updateGroup(editingId, {
                    name: name, color: color, timeStart: timeStart, timeEnd: timeEnd
                });
                // Reconcile: strip ids the user de-selected, then re-assign all kept/new ones
                // (setActivityGroup moves them out of any other group automatically).
                var prev = (findGroupById(editingId).activityIds || []).slice();
                prev.forEach(function(id) {
                    if (actIds.indexOf(id) === -1) setActivityGroup(id, null);
                });
                actIds.forEach(function(id) { setActivityGroup(id, editingId); });
            } else {
                var newG = addGroup({
                    name: name, color: color, timeStart: timeStart, timeEnd: timeEnd
                });
                if (newG) actIds.forEach(function(id) { setActivityGroup(id, newG.id); });
            }
        ...
        window.deleteGroupFromModal = async function() {
            ...
            (g.activityIds || []).slice().forEach(function(id) { setActivityGroup(id, null); });
