        ...
        function renderActivityCards(activities) {
            ...
            // Ring circumference (r=21, viewBox 52×52)
            const RING_CIRC = 131.95;
            ...
            return activities.map(activity => {
                ...
                const dimRgb = _dimRgb(dimHex);

                // Ring progress (streak toward 5 = bonus threshold)
                ...
                // XP text for ring (compact)
                let ringXpText;
                if (activity.isNegative) {
                    ringXpText = `−${displayXP}`;
                } else {
                    ringXpText = `+${displayXP}`;
                }
                const isNegAtRisk = activity.isNegative;
                ...
                <div class="activity-item ${itemClass}" data-aid="${activity.id}" onclick="${clickHandler}"
                     style="--dim-color:${dimHex};--dim-rgba:rgba(${dimRgb},0.17);">
                    <div class="act-dim-gradient"></div>
                    ...
                    <div class="act-ring-wrap">
                        <svg class="act-ring-svg" viewBox="0 0 52 52" xmlns="http://www.w3.org/2000/svg">
                            <circle class="act-ring-bg" cx="26" cy="26" r="21" fill="none" stroke-width="3.5"/>
                            <circle class="act-ring-done-fill" cx="26" cy="26" r="21" stroke="none"/>
                            <circle class="act-ring-fill" cx="26" cy="26" r="21" fill="none" stroke-width="3.5"
                        ...
                        <div class="act-ring-label">
                            ${completedToday
                                ...
                                : `<span class="act-ring-xp-text">${ringXpText}</span>`
                            }
                        </div>
                    </div>
                </div>
        ...
        window.cycleGridCardType = function(activityId) {
            var cur = window._gridCardTypes[activityId] || 1;
            window._gridCardTypes[activityId] = (cur % 6) + 1;
            renderActivitiesList();
        };

        // Build ring SVG for grid cards (smaller, 44×44 with r=18)
        function _gcRing(completedToday, currentStreak, dimHex, xpText) {
            var CIRC = 113.10; // 2π×18
            var prog = Math.min(currentStreak, 5) / 5;
            var offset = (CIRC * (1 - prog)).toFixed(2);
            return `<div class="gc-ring-wrap">
                <svg class="gc-ring-svg" viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg">
                    <circle class="gc-ring-bg" cx="22" cy="22" r="18" fill="none" stroke-width="3"/>
                    <circle class="gc-ring-done-fill" cx="22" cy="22" r="18" stroke="none"/>
                    <circle class="gc-ring-fill" cx="22" cy="22" r="18" fill="none" stroke-width="3"
        ...
        function renderActivityGridCards(activities) {
            ...
            // Build group lookup: activityId → group
            var actGroupMap = {};
            var groupOrder = [];
            ...
            // Separate grouped and ungrouped activities
            var ungrouped = [];
            var byGroup = {};
            ...
            function renderCard(activity) {
                ...
                var showUndo = todayCount > 0 && !notScheduledToday;
                ...
                // Badges
                var badges = '';
                if (currentStreak > 0 && activity._shieldsUsed > 0) badges += '<span class="gc-badge gc-badge-shield">' + Math.max(0, getShieldCap(activity) - activity._shieldsUsed) + ' 🛡</span>';
                ...
                var ringHtml = (type === 1 || type === 3 || type === 4) ? '' : _gcRing(completedToday, currentStreak, dimHex, xpText);
                ...
                return '<div class="grid-card ' + stateClass + '" data-aid="' + activity.id + '" data-type="' + type + '" onclick="' + clickHandler + '"'
                    ...
                    + '<button class="gc-type-btn" onclick="event.stopPropagation();cycleGridCardType(\'' + activity.id + '\')" title="Change card size">' + type + '</button>'
        ...
        // ── Grid card overlay (expand without disturbing grid) ────────────
        window.openGridCardOverlay = function(activityId) {
            ...
            var mult = (activity.frequency === 'occasional') ? 1 : calculateConsistencyMultiplier(completedToday ? streak : streak+1);
            var displayXP = Math.floor(activity.baseXP * mult);
            ...
            content.innerHTML =
                ...
                + '              stroke-dasharray="' + RING_CIRC + '" stroke-dashoffset="' + ringOffset + '" transform="rotate(-90 26 26)" style="opacity:' + (completedToday ? '0' : '1') + '"/>'
                + '    </svg>'
                ...
                + (completedToday
                    ...
                    : '<span style="font-size:12px;font-weight:800;color:' + dimHex + ';">' + xpText + '</span>')
                + '    </div>'
                + '  </div>'
                + '  <div class="grid-overlay-meta">'
                + '    <span class="activity-badge badge-frequency">' + escapeHtml(activity.dimensionName) + ' › ' + escapeHtml(activity.pathName) + '</span>'
                + '    <span class="activity-badge ' + (activity.isNegative ? 'badge-negative' : 'badge-xp') + '">' + xpText + ' XP' + (mult > 1 ? ' (' + mult + '×)' : '') + '</span>'
                + (streak > 0 ? '<span class="activity-badge badge-streak">🔥 ' + streak + ' day streak</span>' : '')
                + '    <span class="activity-badge badge-frequency">' + activity.frequency + '</span>'
                + '  </div>'
                + '</div>'
                + '<div class="grid-overlay-actions">'
                + (!notScheduled && canComplete && !completedToday
                    ? '<button class="grid-overlay-btn complete" onclick="completeActivityById(\'' + activity.id + '\');closeGridCardOverlay();">✓ Complete</button>'
                    : (completedToday ? '<button class="grid-overlay-btn" style="opacity:0.5;cursor:default;">✓ Done today</button>' : ''))
                + (todayCount > 0
                    ? '<button class="grid-overlay-btn undo" onclick="undoActivityById(\'' + activity.id + '\');closeGridCardOverlay();">↩ Undo</button>'
                    : '')
                + '</div>';
