        ...
        // ── Shield system constants ───────────────────────────────────────
        const BASE_SHIELDS      = 3;               // every streak starts with 3
        const SHIELD_MILESTONES = [25, 50, 75, 100]; // +1 shield at each
        const SHIELD_ABS_CAP    = 7;               // 3 base + 4 milestone bonuses

        // Returns the shield cap stored on the activity, with BASE_SHIELDS fallback.
        // NEVER re-derives from streak math at render time — always reads the
        // authoritative stored value written by processStreakSystem / recomputeStreak.
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            if (!isOccasional) {
                ...
                if (shouldGrantStreak) {
                    ...
                    // Shield milestone bonus: +1 shield when streak crosses 25/50/75/100.
                    // completeActivity is the only place streak increments in real-time,
                    // so this is the right place to grant immediately rather than waiting
                    // for processStreakSystem at the next login.
                    if (SHIELD_MILESTONES.includes(newStreak)) {
                        activity.shieldCapUsed = Math.min(SHIELD_ABS_CAP, (activity.shieldCapUsed || BASE_SHIELDS) + 1);
                    }
                }
            }
        ...
        function recomputeStreakFromHistory(activity) {
            ...
            // Same logic as processStreakSystem: start with the stored cap so milestone
            // shields are respected during retroactive edits.
            let shieldsLeft = getShieldCap({ shieldCapUsed: activity.shieldCapUsed });
            const MAX_WALK = 400;
            ...
            // BUG FIX: if the walk broke (streak=0 from closed windows) but there's a
            // completion today, we're starting a FRESH streak — don't carry the broken
            // walk's shieldsConsumed (which would show "0 shields left" on a 1-day streak).
            if (hasTodayCompletion) {
                if (streak === 0) shieldsConsumed = 0;
                streak++;
            }
        ...
        function processStreakSystem(activity, today) {
            ...
            let streak = 0;
            let shieldsConsumed = 0;
            // Start the walk with however many shields this activity currently has.
            // shieldCapUsed is the authoritative stored cap — reflects milestone bonuses
            // earned in previous completeActivity calls. Fallback to BASE_SHIELDS for
            // new activities or activities not yet touched by the milestone system.
            let shieldsLeft = getShieldCap({ shieldCapUsed: activity.shieldCapUsed });
            const MAX_WALK = 400;
            let hitFloor = false;
            ...
            if (finalStreak === 0) {
                activity.streakStartWindow = null;
                activity.shieldCapUsed     = BASE_SHIELDS; // streak broke — reset for next run
            } else {
                // Self-heal: cap must be at least what this streak warrants.
                // Covers gaps where completeActivity didn't fire (e.g. a streak reaching
                // a milestone via a retroactive edit rather than a live completion).
                const warranted = Math.min(SHIELD_ABS_CAP,
                    BASE_SHIELDS + SHIELD_MILESTONES.filter(m => finalStreak >= m).length);
                activity.shieldCapUsed = Math.min(SHIELD_ABS_CAP,
                    Math.max(activity.shieldCapUsed || BASE_SHIELDS, warranted));
            }
