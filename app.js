        ...
        async function applyRetroactiveRecalculation(activity, dimIndex) {
            ...
            // 5. Global totalXP → level
            const newTotalXP = recomputeTotalXPFromHistory();
            window.userData.totalXP = newTotalXP;
            const { level, currentXP } = recomputeLevelFromTotalXP(newTotalXP);
        ...
        window.retroactiveComplete = async function(activityId, dateStr) {
            ...
            await applyRetroactiveRecalculation(foundActivity, foundDi);
        ...
        window.openRetroPicker = function(dateStr) {
            ...
            const date = new Date(dateStr + 'T12:00:00');
            const dayNames = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
            title.textContent = `Add for ${dayNames[date.getDay()]}, ${date.getDate()} ${date.toLocaleString('default',{month:'short'})}`;
            ...
            allActs.forEach(act => {
                if (act.allowMultiplePerDay) return;
                const done = (act.completionHistory || []).some(
                    e => !e.isPenalty && toLocalDateStr(new Date(e.date)) === dateStr
                );
                if (done) alreadyLogged.add(act.id);
            });

            list.innerHTML = allActs.map(act => {
                const done = alreadyLogged.has(act.id);
                return `
                <div class="retro-picker-item ${done ? 'retro-picker-already' : ''}"
                     onclick="${done ? '' : `closeRetroPicker(); retroactiveComplete('${act.id}','${dateStr}')`}">
                    <span class="retro-picker-name">${escapeHtml(act.name)}</span>
                    <span class="retro-picker-xp">${done ? '✓ done' : `+${act.baseXP} XP`}</span>
                </div>`;
            }).join('');
        ...
        function recomputeStreakFromHistory(activity) {
            ...
            // Update lastCompleted from history
            ...
            // Start the backwards walk from the most recently CLOSED window
            let cursor = getCycleWindowStart(activity, new Date(todayWindow.getTime() - 1));
            if (!cursor) {
                // Only today's window exists — check if there's a completion in it
                const hasToday = userHistory.some(e => {
                    const t = new Date(e.date).getTime();
                    const ws = todayWindow.getTime();
                    const we = ws + (getNextCycleWindowStart(activity, todayWindow)?.getTime() - ws || 86400000);
                    return t >= ws && t < we;
                });
            ...
            const oldestEntry = userHistory[0];
            const historyFloorWindow = getCycleWindowStart(activity, new Date(oldestEntry.date)) || actCreatedWindow;
            ...
            // Check if today's open window has a completion (contributes to streak but isn't a closed window)
            const todayWinEnd = getNextCycleWindowStart(activity, todayWindow);
            const hasTodayCompletion = todayWinEnd && userHistory.some(e => {
                const t = new Date(e.date).getTime();
                return t >= todayWindow.getTime() && t < todayWinEnd.getTime();
            });

            for (let i = 0; i < MAX_WALK; i++) {
                if (!cursor || cursor < walkFloor) break;
                const nextWindowStart = getNextCycleWindowStart(activity, cursor);
                if (!nextWindowStart) break;
                const cStart = cursor.getTime();
                const cEnd = nextWindowStart.getTime();
                const hit = userHistory.some(e => {
                    const t = new Date(e.date).getTime();
                    return t >= cStart && t < cEnd;
                });
                if (hit) {
                    streak++;
                    streakStartWindow = cursor; // track oldest hit
            ...
            // Add today's open window if completed
            ...
            // streakGrantedDate: set to today if today has a completion
            activity.streakGrantedDate = hasTodayCompletion ? todayStr : null;

            // Reset streak fields if streak is 0
            if (streak === 0) {
                activity.streakStartWindow = null;
                activity.streakGrantedDate = null;
            }
