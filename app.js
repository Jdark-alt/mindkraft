        ...
        function renderCalendar() {
            ...
            // Filter by selected calendar activity — read AFTER the rebuild so .value is current
        ...
        window.toggleInstallGuide = function() {
            ...
            const btn  = document.getElementById('installGuideToggleBtn');
            if (!body) return;
            const isOpen = body.classList.toggle('open');
            btn.classList.toggle('open', isOpen);
        };
        ...
        window.toggleCustomThemePanel = function() {
            const body = document.getElementById('themeCustomPanel');
            const btn  = document.getElementById('customThemeToggleBtn');
            if (!body) return;
            const isOpen = body.classList.toggle('open');
            btn.classList.toggle('open', isOpen);
            // Build pickers lazily on first open
            if (isOpen && !body._built) {
                body._built = true;
                var saved = (window.userData && window.userData.settings && window.userData.settings.theme) || {};
                buildColorGrid();
        ...
        // Hide the install card entirely when already running as a PWA
        (function() {
            function hideInstallCardIfPWA() {
                if (window.isRunningAsPWA && window.isRunningAsPWA()) {
                    var card = document.getElementById('installGuideCard');
                    if (card) card.style.display = 'none';
                }
            }
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', hideInstallCardIfPWA);
            } else {
                hideInstallCardIfPWA();
            }
        })();
        ...
        function loadTheme() {
            ...
            // Show the collapsible row (but keep panel collapsed) when custom is active
            if (customActive) {
                var collapsible = document.getElementById('customThemeCollapsible');
                if (collapsible) collapsible.style.display = 'block';
                var themeResetRow = document.getElementById('themeResetRow');
                if (themeResetRow) themeResetRow.style.display = 'flex';
            }
        }
        ...
        window.activateCustomTheme = function(el) {
            ...
            // Show the collapsible row — but don't force the panel open
            var collapsible = document.getElementById('customThemeCollapsible');
            if (collapsible) collapsible.style.display = 'block';
            var themeResetRow = document.getElementById('themeResetRow');
            if (themeResetRow) themeResetRow.style.display = 'flex';
        ...
        window.applyThemePreset = function(id, el) {
            ...
            if (!t) return;
            // First, reset ALL custom-editor vars to their defaults so nothing from
            // a previous custom theme bleeds through
            CUSTOM_COLOR_VARS.forEach(function(v) {
            ...
            // Hide custom panel + collapsible row
            var collapsible = document.getElementById('customThemeCollapsible');
            if (collapsible) collapsible.style.display = 'none';
            var themeResetRow = document.getElementById('themeResetRow');
            if (themeResetRow) themeResetRow.style.display = 'none';
            var panel = document.getElementById('themeCustomPanel');
            if (panel) panel.classList.remove('open');
            var panelBtn = document.getElementById('customThemeToggleBtn');
            if (panelBtn) panelBtn.classList.remove('open');
            document.querySelectorAll('.theme-swatch').forEach(function(s){ s.classList.remove('active'); });
        ...
        //   • Scans every day from (lastSkipCheckDate + 1) up to *yesterday* inclusive.
        //   • Days that weren't completed count as missed (capped at 7 total).
        //   • Penalty = missedDays × baseXP (linear, not exponential).
        //   • One shield consumed per missed day; streak breaks when all 3 shields are gone.
        //   • BUG FIX: activities with zero history are skipped — no false penalty on first day.
        //   • BUG FIX: same-day refreshes never trigger a penalty.
        function processSkipPenalty(activity, today) {
            if (!activity.isSkipNegative) return;
            if (activity.frequency !== 'daily') return;   // skip-penalty only for daily right now

            // Already processed today — nothing to do
            if (activity.lastSkipCheckDate === today) return;

            // ── Guard: never penalise a brand-new activity ────────────────
            // If the activity has never been completed AND has no check history,
            // it just started today (or was never used). Just stamp the date and leave.
            ...
            // ── Determine the window to scan ──────────────────────────────
            // Start: the day after the last check date (or the day after last completion
            //        for activities that pre-date the lastSkipCheckDate field).
            const prevCheckStr = activity.lastSkipCheckDate;   // may be null/undefined
            let startDay;
            if (prevCheckStr) {
                // Scan FROM the last-checked date itself — that day may have been missed
                // (e.g. lastSkipCheckDate = yesterday → startDay = yesterday → loop checks it).
                startDay = new Date(prevCheckStr + 'T00:00:00');
            } else {
                // First time: fall back to lastCompleted. That day WAS completed so skip it
                // (+1) and start scanning the day after.
            ...
            while (cursor < todayMidnight) {
                if (!wasCompletedOnDay(activity, cursor)) {
                    missedDays++;
                }
                cursor.setDate(cursor.getDate() + 1);
            }

            // Cap at 7 consecutive missed days
            missedDays = Math.min(7, missedDays);

            // Stamp today so we don't re-run in the same session
            activity.lastSkipCheckDate = today;

            if (missedDays === 0) {
                // All days were completed — reset the skip streak counter
                activity.skipStreak = 0;
                return;
            }

            // ── Accumulate skip streak ────────────────────────────────────
            activity.skipStreak = (activity.skipStreak || 0) + missedDays;

            // ── Shield consumption ────────────────────────────────────────
            const MAX_SHIELDS = 3;
            const usedShields = activity.streakPauseUses || 0;
            const availableShields = Math.max(0, MAX_SHIELDS - usedShields);
            const shieldsToConsume = Math.min(missedDays, availableShields);
            const newUsedShields = usedShields + shieldsToConsume;

            activity.streakPauseUses = Math.min(MAX_SHIELDS, newUsedShields);

            // If the penalty exhausted remaining shields, break the streak
            if (missedDays > availableShields) {
                activity.streak = 0;
                activity.streakPauseUses = 0;
                activity.streakPaused = false;
            }

            // ── XP Penalty (linear: N × baseXP) ──────────────────────────
            const penaltyPerDay = activity.baseXP || 10;
            const totalPenalty = penaltyPerDay * missedDays;

            window.userData.currentXP -= totalPenalty;
            window.userData.totalXP   -= totalPenalty;

            // Level-down if needed (never below level 1 / 0 XP)
            while (window.userData.currentXP < 0 && window.userData.level > 1) {
                window.userData.level  -= 1;
                window.userData.currentXP += calculateXPForLevel(window.userData.level);
            }
            ...
            // Record in history so the log reflects the deduction
            recordCompletion(activity, -totalPenalty, true);

            // Apply penalty to the parent dimension's level track
            ...
            activity.lastPenaltyDays = missedDays;
        }
        ...
        async function saveAutoBackup(userData) {
            ...
            // Store backup inline in user's main doc — avoids subcollection permission errors
            try {
        ...
        (function() {
            ...
            window.addEventListener('beforeinstallprompt', function(e) {
                e.preventDefault();
                _deferredPrompt = e;
                window._mkInstallPrompt = e; // expose for settings install button
            });
        ...
        // Check whether app is already running as an installed PWA
        window.isRunningAsPWA = function() {
            return window.matchMedia('(display-mode: standalone)').matches ||
                   window.navigator.standalone === true;
        };

        // Called by the settings Install button on Android
        window.triggerInstallPrompt = async function() {
            var btn = document.getElementById('settingsInstallBtn');
            if (!window._mkInstallPrompt) {
                showToast('Install prompt not available. Try from your browser menu.', 'olive');
                return;
            }
            if (btn) btn.disabled = true;
            window._mkInstallPrompt.prompt();
            var result = await window._mkInstallPrompt.userChoice;
            window._mkInstallPrompt = null;
            if (btn) btn.disabled = false;
            if (result.outcome === 'accepted') {
                showToast('🎉 Mindkraft installed!', 'green');
                // Hide the whole install card
                var card = document.getElementById('installGuideCard');
                if (card) card.style.display = 'none';
            }
        };
