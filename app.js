        ...
        function renderActivityCards(activities) {
            ...
            return activities.map(activity => {
                ...
                // Dimension color for ring only
                const dimHex = DIM_HEX_MAP[activity.dimColor] || DIM_HEX_MAP.blue;
                ...
                // XP display in row (OLD position — unchanged)
                let xpText;
                if (isSkipMode && !completedToday) xpText = `+${displayXP}`;
                else xpText = `${activity.isNegative ? '−' : '+'}${displayXP}`;
                ...
                return `
                <div class="activity-item ${itemClass}" data-aid="${activity.id}" onclick="${clickHandler}"
                     style="--dim-color:${dimHex};">
                    ...
                    <!-- Progress ring around check placeholder -->
        ...
        function renderActivityGridCards(activities) {
            ...
            function renderCard(activity) {
                ...
                // something to undo today. Stops pointer events from bubbling.
                var cornerUndoHtml = (todayCount > 0 && !notScheduledToday)
                    ? '<button class="gc-corner-undo" onpointerdown="event.stopPropagation()"'
                      + ' onpointerup="event.stopPropagation()"'
                      + ' onclick="event.stopPropagation();undoActivityById(\'' + activity.id + '\')"'
                      + ' title="Undo last completion" aria-label="Undo">↩</button>'
                    : '';
            ...
            groupOrder.forEach(function(gid) {
                ...
                html += '<div class="act-group routine-group" data-group="' + key + '">'
                    ...
                    + timeBadge
                    + '<span class="act-group-count">' + acts.length + '</span>'
                    + '<button class="routine-edit-btn" onclick="event.stopPropagation();openGroupModal(\'' + gid + '\')" title="Edit group">✎</button>'
                    + '</div>'
                    + '<div class="act-group-body ' + (isExpanded ? 'expanded' : '') + '">'
                    + (acts.length
        ...
        function renderChallenges() {
            ...
            if (failedChallenges.length > 0) {
                ...
                html += `
                <div onclick="toggleFailedChallenges()" style="cursor:pointer;display:flex;align-items:center;justify-content:space-between;margin:24px 0 12px 0;user-select:none;padding:4px 0;">
                    <h3 style="font-size:18px;margin:0;display:flex;align-items:center;gap:8px;">
                        Failed
                        <span style="font-size:12px;font-weight:500;color:var(--color-text-secondary);background:rgba(142,59,95,0.15);border-radius:20px;padding:2px 8px;">${failedChallenges.length}</span>
                    </h3>
                    <span style="font-size:12px;color:var(--color-text-secondary);transform:${failedOpen ? 'rotate(180deg)' : 'rotate(0)'};transition:transform 0.2s;display:inline-block;padding:4px;">▼</span>
                </div>
        ...
        function renderChallengeCard(challenge, index) {
            ...
            if (hasMetric) {
                ...
                // When there's also per-activity breakdown, the input goes INSIDE
                // the breakdown section (renamed to "Update progress"). When no
                // breakdown exists, the input stays visible right under the bar.
                const inputRowInline = (isActive && !hasPerActivity) ? `
                ...
                if (hasPerActivity) {
                    ...
                    const inputRowInside = isActive ? `
                        <div class="ch-update-row" style="margin-bottom:14px;">
                            <input class="ch-update-input" type="number" id="metric-input-${challenge.id}" placeholder="Current value" step="any" min="0" max="${challenge.metricQty}" value="${metricCurrent > 0 ? metricCurrent : ''}">
                            <button type="button" class="ch-update-btn" onclick="updateMetricProgress('${challenge.id}')">Update</button>
                        </div>` : '';
                    progressHtml += `
                <div class="ch-breakdown-toggle" onclick="toggleActivityProgress('${challenge.id}')">
                    <span>📋 Update progress &nbsp;<span style="color:var(--color-accent-blue);font-weight:600;">${Math.floor(activityPct)}%</span></span>
                ...
                <div class="ch-breakdown-body${collapsed ? ' collapsed' : ''}" id="ch-breakdown-${challenge.id}" style="max-height:${collapsed ? '0' : '600px'};">
                    <div style="padding-top:10px;">${inputRowInside}${activityRowsHtml}</div>
                </div>`;
                }
        ...
        function checkStreakMilestone(activityName, streak) {
            ...
            _showToastPill({
                ...
                accent:    isShieldTier ? 'rgba(20,60,100,0.95)'  : 'rgba(90,60,10,0.95)',
                accentEnd: isShieldTier ? 'rgba(40,120,190,0.95)' : 'rgba(180,120,20,0.95)',
                border:    isShieldTier ? 'rgba(90,159,212,0.55)' : 'rgba(255,180,50,0.5)',
            });
        }
        ...
        function renderProfileSpiderChart() {
            ...
            const gridStrong  = isLight ? 'rgba(15,23,42,0.14)' : 'rgba(255,255,255,0.10)';
            const gridSoft    = isLight ? 'rgba(15,23,42,0.07)' : 'rgba(255,255,255,0.05)';
            const spokeColor  = isLight ? 'rgba(15,23,42,0.10)' : 'rgba(255,255,255,0.07)';
            ...
            [0.25, 0.5, 0.75, 1].forEach(frac => {
                ...
                ctx.closePath();
                ctx.strokeStyle = (frac === 1) ? gridStrong : gridSoft;
                ctx.lineWidth = 0.8;
                ctx.stroke();
            });
        ...
        function showToast(message, color = 'blue') {
            ...
            toast.style.setProperty('--mk-toast-bg', map[color]||map.blue);
            toast.textContent = message;
            document.body.appendChild(toast);
            setTimeout(() => { toast.classList.add('mk-toast--out'); setTimeout(()=>toast.remove(),300); }, 3000);
        }
        ...
        window.renderFriendsTab = async function() {
            ...
            if (pendingRequests.length > 0) {
                requestsHTML = `
                <div class="mk-friends-requests">
                    <div class="mk-friends-section-label">Friend Requests (${pendingRequests.length})</div>
                    ${pendingRequests.map(r => {
                        const alreadyFriend = (window.userData.friends || []).includes(r.fromUID);
                        const av = r.fromPhotoURL
                            ? `<img class="mk-friend-avatar" src="${escapeHtml(r.fromPhotoURL)}" alt="">`
                            : `<div class="mk-friend-avatar mk-friend-avatar--initial">${escapeHtml((r.fromName||'?')[0].toUpperCase())}</div>`;
                        return `
                        <div class="mk-request-card">
                            ${av}
                            <div class="mk-request-info">
                                <div class="mk-request-name">${escapeHtml(r.fromName||'Someone')}</div>
                                <div class="mk-request-sub">added you as a friend</div>
                            </div>
                            <div class="mk-request-actions">
                                ${alreadyFriend
                                    ? `<button class="mk-btn-ghost" onclick="dismissFriendRequest('${escapeHtml(r.docId)}')">Dismiss</button>`
                                    : `<button class="mk-btn-primary" onclick="acceptFriendRequest('${escapeHtml(r.fromUID)}','${escapeHtml(r.fromCode||'')}','${escapeHtml(r.docId)}')">Add Back</button>
                                       <button class="mk-btn-ghost" onclick="dismissFriendRequest('${escapeHtml(r.docId)}')">Dismiss</button>`
                                }
                            </div>
                        </div>`;
            ...
            lb.innerHTML = requestsHTML + `
                <div class="mk-friends-leaderboard">
                    <div class="mk-friends-section-head">
                        <h2 class="mk-friends-section-title">Leaderboard</h2>
                        <span class="mk-friends-metric-tag">${metricSubLabel}</span>
                    </div>
                    <!-- Metric selector -->
                    <div class="mk-leaderboard-pills">
                        <button class="mk-lb-pill${metric==='weeklyXP'?' active':''}"
                            onclick="setLeaderboardMetric('weeklyXP')">XP this week</button>
                        <button class="mk-lb-pill${metric==='xpPerHour'?' active':''}"
                            onclick="setLeaderboardMetric('xpPerHour')">XP / hr <span class="mk-lb-pill-sub">(yesterday)</span></button>
                        <button class="mk-lb-pill${metric==='xpToday'?' active':''}"
                            onclick="setLeaderboardMetric('xpToday')">XP today</button>
                    </div>
                    ${topEntries.length === 0 ? '<div class="mk-empty-line">No one on the leaderboard yet.</div>' :
                    topEntries.map((e, i) => {
                        const rank      = i + 1;
                        const rankClass = rank <= 3 ? ' top' : '';
                        const isMe   = e.isMe;
                        const avatar = e.photoURL
                            ? `<img class="mk-friend-avatar" src="${escapeHtml(e.photoURL)}" alt="">`
                            : `<div class="mk-friend-avatar mk-friend-avatar--initial">${escapeHtml((e.displayName||'?')[0].toUpperCase())}</div>`;
                        return `
                        <button class="mk-lb-row${rankClass}${isMe ? ' me' : ''}" onclick="openFriendProfileCard('${escapeHtml(e.uid)}')">
                            <span class="mk-lb-rank">${rank}</span>
                            ${avatar}
                            <div class="mk-lb-text">
                                <div class="mk-lb-name">${escapeHtml(e.displayName || 'Adventurer')}${isMe ? ' <span class="mk-lb-youtag">YOU</span>' : ''}</div>
                                <div class="mk-lb-sub">Lv ${e.level || 1} · ${escapeHtml(e.characterTitle || '')}</div>
                            </div>
                            <div class="mk-lb-value">
                                <span class="mk-lb-value-num">${metricVal(e).toLocaleString()}</span>
                                <span class="mk-lb-value-unit">${metric==='xpPerHour' ? 'XP/hr' : 'XP'}</span>
                            </div>
                        </button>`;
                    }).join('')}
                </div>`;
            ...
            add.innerHTML = `
                <div class="mk-friends-section-label" style="margin-top:24px;">Add a friend</div>
                <div class="mk-add-row${atCap ? ' is-disabled' : ''}">
                    <input id="friendCodeInput" type="text" maxlength="7" placeholder="MK-XXXX"
                        ${atCap ? 'disabled' : ''}
                        class="mk-friend-code-input"
                        oninput="this.value=this.value.toUpperCase()" onkeydown="if(event.key==='Enter')addFriendByCode()">
                    <button class="mk-btn-primary" onclick="addFriendByCode()" ${atCap ? 'disabled' : ''}>Add</button>
                </div>
                <div id="friendAddStatus" class="mk-add-status">
            ...
            if (friendEntries.length === 0) {
                all.innerHTML = `
                    <div class="mk-friends-section-label" style="margin-top:24px;">All friends (0)</div>
                    <div class="mk-empty-line">Add a friend using their MK code above ↑</div>`;
            } else {
                all.innerHTML = `
                    <div class="mk-friends-section-label" style="margin-top:24px;">All friends (${friendEntries.length}/20)</div>
                    <div class="mk-friends-list">
                    ${friendEntries.sort((a, b) => (a.displayName || '').localeCompare(b.displayName || '')).map(e => {
                        const avatar = e.photoURL
                            ? `<img class="mk-friend-avatar mk-friend-avatar--lg" src="${escapeHtml(e.photoURL)}" alt="">`
                            : `<div class="mk-friend-avatar mk-friend-avatar--lg mk-friend-avatar--initial">${escapeHtml((e.displayName || '?')[0].toUpperCase())}</div>`;
                        return `
                        <button class="mk-friend-row" onclick="openFriendProfileCard('${escapeHtml(e.uid)}')">
                            ${avatar}
                            <div class="mk-friend-text">
                                <div class="mk-friend-name">${escapeHtml(e.displayName || 'Adventurer')}</div>
                                <div class="mk-friend-sub">Lv ${e.level || 1} · ${escapeHtml(e.characterTitle || '')}</div>
                            </div>
                            <svg class="mk-friend-chev" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                        </button>`;
                    }).join('')}
                    </div>`;
            }
        ...
        window.openFriendProfileCard = function(uid) {
            ...
            const gridSVG = gridLevels.map(pct =>
                ...
                }).join(' ')}" fill="none" stroke="var(--color-border)" stroke-width="1"/>`
            ).join('');
            const axesSVG = cats.map((_,i) => {
                const a = -Math.PI/2 + i * angleStep;
                return `<line x1="${cx}" y1="${cy}" x2="${cx+r*Math.cos(a)}" y2="${cy+r*Math.sin(a)}" stroke="var(--color-border)" stroke-width="1"/>`;
            }).join('');
            ...
            const avatar = data.photoURL
                ? `<img class="mk-friend-avatar mk-friend-avatar--xl" src="${escapeHtml(data.photoURL)}" alt="">`
                : `<div class="mk-friend-avatar mk-friend-avatar--xl mk-friend-avatar--initial">${escapeHtml((data.displayName||'?')[0].toUpperCase())}</div>`;
            ...
            document.getElementById('friendProfileContent').innerHTML = `
                <div class="mk-fpc-identity">
                    ${avatar}
                    <div class="mk-fpc-id-text">
                        <div class="mk-fpc-title">${escapeHtml(data.characterTitle || '')}</div>
                        <div class="mk-fpc-name">${escapeHtml(data.displayName || 'Adventurer')}</div>
                        <div class="mk-fpc-level">Level ${data.level || 1}</div>
                    </div>
                </div>
                <div class="mk-fpc-stats">
                    ${[
                    ...
                    ].map(t => `<div class="mk-stat-tile">
                        <div class="mk-stat-val">${t.val}</div>
                        <div class="mk-stat-lbl">${t.lbl}</div>
                    </div>`).join('')}
                </div>
                <div class="mk-fpc-spider">${spiderSVG}</div>
                <div class="mk-fpc-legend">
                    ${cats.map(c => `<div class="mk-legend-item">
                        <span class="mk-legend-dot" style="background:${c.color};"></span>${c.label}
                    </div>`).join('')}
                </div>
                ${!isMe ? `
                <div class="mk-fpc-actions">
                    <button class="mk-btn-ghost mk-btn-ghost--wide" onclick="toggleLeaderboardVisibility('${escapeHtml(uid)}')">
                        ${(window.userData.leaderboardHidden || []).includes(uid) ? 'Add to leaderboard' : 'Remove from leaderboard'}
                    </button>
                    <button class="mk-btn-ghost mk-btn-ghost--wide mk-btn-ghost--danger" onclick="removeFriend('${escapeHtml(uid)}')">
                        Remove friend
                    </button>
                </div>` : ''}`;
