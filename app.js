        ...
        async function loadUserData(uid) {
            ...
            if (userDoc && userDoc.exists()) {
                ...
                // (legacy: settings.gridCardTypes — silently dropped by the
                //  Today view migration, see renderTodayView block above)
            } else {
        ...
        const DEFAULT_ACTIVITY_SORT = 'by-routine';
            ...
            if (sort === 'smart') {
                ...
                orderedGroups.forEach(function(g) {
                    ...
                    if (g.timeStart && g.timeEnd) {
                        timeBadge = '<span class="routine-time-badge'
                            + (isActive ? ' active' : '') + '">'
                            + escapeHtml(formatTime12(g.timeStart)) + ' – '
                            + escapeHtml(formatTime12(g.timeEnd)) + '</span>';
                    } else {
                        timeBadge = '<span class="routine-time-badge all-day">All day</span>';
                    }
                    html += '<div class="act-group routine-group" data-group="' + key + '">'
                        ...
                        + '<button class="routine-edit-btn" onclick="event.stopPropagation();openGroupModal(\'' + g.id + '\')" title="Edit group">✎</button>'
                        + '</div>'
                        + '<div class="act-group-body ' + (isExpanded ? 'expanded' : '') + '">'
                        + (acts.length
                ...
                if (ungrouped.length) {
                    ...
                    html += '<div class="act-group" data-group="' + ukey + '">'
                        ...
                        + '<span class="collapse-icon ' + (uExpanded ? 'expanded' : '') + '">▼</span>'
        ...
        function updateViewToggleIcon() {
            ...
            btn.title = isGrid ? 'Switch to Manage view' : 'Switch to Today view';
            if (isGrid) btn.classList.add('active'); else btn.classList.remove('active');
        }

        // Dispatcher: renders activities in whichever view mode is active.
        // 'list' = Manage view (the full inventory, filtered & sorted by user)
        // 'grid' = Today view (algorithmic command surface; only built when active)
        function renderActivityContent(activities) {
            if (window._activityViewMode === 'grid') {
                return renderTodayView(activities);
            }
            return renderActivityCards(activities);
        }

        // ══════════════════════════════════════════════════════════════════
        // TODAY VIEW (Phase 1 — scaffolding)
        // ══════════════════════════════════════════════════════════════════
        // This replaces the legacy "grid view" which let users hand-pick a
        // card size for every activity. The new Today view is an algorithmic
        // command surface answering "what should I do now?" — composed of
        // blocks that earn their place based on the user's actual data
        // (streaks, cycles, dimensions, neglect, time-of-day, etc.).
        //
        // PHASE PLAN
        //   P0 — Strip old picker, long-press, _gridCardTypes, etc.  ✓
        //   P1 — Scaffold: container, daily cache hook, placeholder Hero ✓ (this file)
        //   P2 — Build 8 blocks one at a time
        //   P3 — Composition + 10-card slot budget + clean-row layout
        //   P4 — Visual polish, animations, empty/loading states
        //   P5 — Optional "Customize Today" intention sheet
        //
        // The toolbar toggle stays a grid icon; tooltip now reads "Today view"
        // when in list mode and "Manage view" when in Today. No visible label
        // change yet — that's a Phase 4 polish call.

        // Daily cache for signals that don't need real-time recomputation:
        //   - 14d XP distribution across dimensions (→ dominant dimension)
        //   - 14d completion histogram per activity hour-of-day (→ time match)
        //   - Longest active streaks (→ streak showcase)
        //   - Recently broken streaks (→ comeback card)
        //   - Dormant activities (→ dormant nudge)
        // Refreshed once per app boot AND at the next 4am after boot.
        // Real-time blocks (Hero, At-risk, Cycle deadline) compute fresh
        // on every render.
        window._todayCache = window._todayCache || {
            stamp: 0,                // ms timestamp of last compute
            stampDate: '',           // local YYYY-MM-DD of last compute
            xpByDim: {},             // { dimColor → totalXP last 14d }
            xpTotal: 0,
            dominantDimColor: null,  // top dim if ≥55% of xpTotal
            dominantDimPct: 0,
            hourHistogram: {},       // { activityId → medianHour (0-23) or null }
            topStreaks: [],          // [{ aid, streak, name, dimColor }] ≥14d, sorted desc
            recentlyBroken: [],      // [{ aid, brokenAt, prevStreak, name, dimColor }] last 48h, streak ≥14
            dormant: []              // [{ aid, daysSince, name, dimColor }] ≥7 days, sorted desc
        };

        function _isTodayCacheStale() {
            if (!window._todayCache.stamp) return true;
            if (window._todayCache.stampDate !== localToday()) return true;
            return false;
        }

        function computeTodayCache() {
            var all = [];
            (window.userData.dimensions || []).forEach(function(dim) {
                (dim.paths || []).forEach(function(path) {
                    (path.activities || []).forEach(function(a) {
                        all.push(Object.assign({}, a, {
                            _dimName:  dim.name,
                            _pathName: path.name,
                            _dimColor: dim.color || 'blue'
                        }));
                    });
                });
            });
            ...
            var DAY_MS = 86400000;
            var window14d = todayMs - 14 * DAY_MS;
            var todayIso = localToday();

            // 14-day XP by dimension
            var xpByDim = {}, xpTotal = 0;
            all.forEach(function(a) {
                var xpEach = a.baseXP || 0;
                (a.completionDates || []).forEach(function(iso) {
                    var t = new Date(iso).getTime();
                    if (isNaN(t) || t < window14d) return;
                    xpByDim[a._dimColor] = (xpByDim[a._dimColor] || 0) + xpEach;
                    xpTotal += xpEach;
                });
            });
            var dominantDimColor = null, dominantDimPct = 0;
            Object.keys(xpByDim).forEach(function(c) {
                var pct = xpTotal > 0 ? (xpByDim[c] / xpTotal) : 0;
                if (pct > dominantDimPct) { dominantDimPct = pct; dominantDimColor = c; }
            });
            if (dominantDimPct < 0.55) dominantDimColor = null;

            // Hour histogram — proper implementation needs completion-time
            // precision. Stub for Phase 1; revisited in Phase 2.
            var hourHistogram = {};

            // Top active streaks (≥14d)
            var topStreaks = all
                .filter(function(a) { return (a.streak || 0) >= 14; })
                .map(function(a) {
                    return { aid: a.id, streak: a.streak, name: a.name, dimColor: a._dimColor };
                })
                .sort(function(a, b) { return b.streak - a.streak; });

            // Recently broken streaks — needs lastStreakBreak field or
            // inference from completionDates gap. Phase 2.
            var recentlyBroken = [];

            // Dormant (≥7 days untouched)
            var dormant = all
                .filter(function(a) {
                    if (a.frequency === 'occasional') return false;
                    var dates = a.completionDates || [];
                    if (dates.length === 0) return true;
                    var lastMs = new Date(dates[dates.length - 1]).getTime();
                    if (isNaN(lastMs)) return false;
                    return Math.floor((todayMs - lastMs) / DAY_MS) >= 7;
                })
                .map(function(a) {
                    var dates = a.completionDates || [];
                    var daysSince = dates.length === 0
                        ? 999
                        : Math.floor((todayMs - new Date(dates[dates.length - 1]).getTime()) / DAY_MS);
                    return { aid: a.id, daysSince: daysSince, name: a.name, dimColor: a._dimColor };
                })
            ...
            window._todayCache = {
                ...
                xpByDim:         xpByDim,
                xpTotal:         xpTotal,
                dominantDimColor: dominantDimColor,
                dominantDimPct:   dominantDimPct,
                hourHistogram:   hourHistogram,
                topStreaks:      topStreaks,
                recentlyBroken:  recentlyBroken,
                dormant:         dormant
            };
        }
        ...
        // Hours remaining until local midnight. Used for streak-risk
        // detection (daily activities lose their streak at midnight if
        // not completed). Returns a float (e.g. 3.5 for "3h 30min left").
        ...
        // Cycle progress for non-daily, non-occasional activities.
        // Returns {completionsThisCycle, target, daysRemainingInCycle} or
        // null if the activity doesn't have a cycle concept (e.g. daily,
        // occasional). Works for weekly / biweekly / monthly / custom.
        function _cycleProgress(a) {
            if (!a || a.frequency === 'daily' || a.frequency === 'occasional') return null;
            var now = new Date();
            var nowMs = now.getTime();
            var DAY_MS = 86400000;
            var cycleStartMs, cycleEndMs, target;

            if (a.frequency === 'weekly') {
                // Mon-Sun. JS Sunday=0; we want Mon=0.
                var dow = (now.getDay() + 6) % 7;
                var ms = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
                cycleStartMs = ms - dow * DAY_MS;
                cycleEndMs   = cycleStartMs + 7 * DAY_MS;
                target       = a.timesPerCycle || 1;
            } else if (a.frequency === 'biweekly') {
                // Anchor on an arbitrary fixed Monday so cycles are stable.
                // 2024-01-01 was a Monday → seed.
                var seedMs = new Date(2024, 0, 1).getTime();
                var weeksSinceSeed = Math.floor((nowMs - seedMs) / (7 * DAY_MS));
                var biweekStart = seedMs + (weeksSinceSeed - (weeksSinceSeed % 2)) * 7 * DAY_MS;
                cycleStartMs = biweekStart;
                cycleEndMs   = biweekStart + 14 * DAY_MS;
                target       = a.timesPerCycle || 1;
            } else if (a.frequency === 'monthly') {
                cycleStartMs = new Date(now.getFullYear(), now.getMonth(), 1).getTime();
                cycleEndMs   = new Date(now.getFullYear(), now.getMonth() + 1, 1).getTime();
                target       = a.timesPerCycle || 1;
            } else if (a.frequency === 'custom') {
                // Custom-cycle activities track _cycleCompletions on the
                // activity directly. We rely on that + cycleDays as the
                // window length.
                var cycleDays = a.cycleDays || a.intervalDays || 7;
                // We don't always know the cycle start; approximate by
                // walking back cycleDays from now.
                cycleStartMs = nowMs - cycleDays * DAY_MS;
                cycleEndMs   = nowMs + DAY_MS; // remaining today + future
                target       = a.timesPerCycle || 1;
                // For custom we trust the pre-computed counter.
                return {
                    completionsThisCycle: a._cycleCompletions || 0,
                    target: target,
                    daysRemainingInCycle: Math.ceil((cycleEndMs - nowMs) / DAY_MS)
                };
            } else {
                return null;
            }

            // For weekly/biweekly/monthly count completionDates within window
            var done = 0;
            (a.completionDates || []).forEach(function(iso) {
                var t = new Date(iso).getTime();
                if (!isNaN(t) && t >= cycleStartMs && t < cycleEndMs) done++;
            });
            return {
                completionsThisCycle: done,
                target: target,
                daysRemainingInCycle: Math.max(0, Math.ceil((cycleEndMs - nowMs) / DAY_MS))
            };
        }

        // ── Hero scoring (Phase 2: full formula) ──────────────────────────
        // Inputs: activity (with pre-computed _completedToday, _canComplete,
        // _streak, _cycleCompletions), context object with cached signals.
        // Returns a numeric score; highest wins Hero slot. Negative scores
        // mean the activity is disqualified (already done, can't complete).
        function _scoreHero(a, ctx) {
            // Hard disqualifiers — never become Hero
            if (a._completedToday && !(a.allowMultiplePerDay && a.frequency !== 'occasional')) {
                return -Infinity;
            }
            if (!a._canComplete && !(a.allowMultiplePerDay && a.frequency !== 'occasional')) {
                return -Infinity;
            }
            // Activities not scheduled for today (custom days) — disqualify
            if (a.frequency === 'custom' && a.customSubtype === 'days' && a._isScheduledDay === false) {
                return -Infinity;
            }

            var score = 0;
            var streak = a._streak || 0;
            var hoursLeft = ctx.hoursUntilMidnight;

            // ── At-risk component (scaled by streak length) ──
            // A 1-day streak being at-risk isn't worth interrupting your day.
            // A 30-day streak absolutely is. Linear within bands.
            if (a.frequency === 'daily' && streak > 0 && hoursLeft < 12) {
                var bandLT4 = hoursLeft < 4;
                if (streak >= 30)      score += bandLT4 ? 70 : 35;
                else if (streak >= 14) score += bandLT4 ? 50 : 25;
                else if (streak >= 7)  score += bandLT4 ? 30 : 15;
                else if (streak >= 3)  score += bandLT4 ? 15 : 7;
                else                   score += bandLT4 ? 5  : 2;
            }

            // ── Routine fit (if user has routines) ──
            if (ctx.hasRoutines) {
                if (ctx.activeRoutineActivityIds.has(a.id)) {
                    score += 40;
                } else if (ctx.nextRoutineActivityIds.has(a.id) && ctx.nextRoutineStartsInMin <= 90) {
                    score += 20;
                }
            } else {
                // Time-of-day fallback — runs only when no routines defined.
                // Currently a stub: hour histogram needs completion timestamps
                // with hour precision, which the data model doesn't track yet.
                // TODO Phase 2.5: start recording completion timestamps with
                // hour precision (extend completionDates → completionTimes)
                // so this branch contributes real signal.
                var median = ctx.cache.hourHistogram[a.id];
                if (typeof median === 'number') {
                    var currentHour = new Date().getHours();
                    var delta = Math.abs(currentHour - median);
                    // Wrap-around: e.g. median=23, now=1 should be delta=2
            ...
            // ── Cycle proximity ──
            var cp = _cycleProgress(a);
            if (cp && cp.completionsThisCycle < cp.target) {
                var needed = cp.target - cp.completionsThisCycle;
                if (needed === 1 && cp.daysRemainingInCycle <= 2)      score += 35;
                else if (needed === 1 && cp.daysRemainingInCycle <= 6) score += 20;
                else if (needed === 2 && cp.daysRemainingInCycle <= needed) score += 25;
                else if (needed === 2)                                  score += 10;
            }

            // ── Neglect bump (days since last completion) ──
            var daysSince = _daysSinceLastCompletion(a);
            if (daysSince >= 4 && daysSince <= 7)        score += daysSince * 2;
            else if (daysSince >= 8 && daysSince <= 14)  score += Math.min(30, daysSince * 3);
            else if (daysSince >= 15)                    score += 30;

            // ── baseXP weight ──
            score += Math.min(20, (a.baseXP || 0) / 3);

            // ── Streak length bonus ──
            score += Math.min(15, streak / 3);

            // ── Recency penalty: same dimension <30min ago ──
            if (ctx.recentDimCompletions[a.dimColor || 'blue']) {
                score -= 20;
            }

            // ── Already in at-risk strip → demote ──
            if (ctx.atRiskActivityIds.has(a.id)) {
                score -= 60;
            }
        ...
        // Days since the most recent completion (any kind). Returns a large
        // number (999) if never completed.
        function _daysSinceLastCompletion(a) {
            var dates = a.completionDates || [];
            if (dates.length === 0) return 999;
            var lastMs = new Date(dates[dates.length - 1]).getTime();
            if (isNaN(lastMs)) return 999;
            var now = Date.now();
            return Math.floor((now - lastMs) / 86400000);
        }

        // Did the user complete an activity in this dimension in the last
        // 30 minutes? Returns boolean keyed by dimColor.
        function _recentDimCompletions() {
            var out = {};
            var cutoff = Date.now() - 30 * 60 * 1000;
            (window.userData.dimensions || []).forEach(function(dim) {
                var dc = dim.color || 'blue';
                (dim.paths || []).forEach(function(path) {
                    (path.activities || []).forEach(function(a) {
                        (a.completionDates || []).forEach(function(iso) {
                            var t = new Date(iso).getTime();
                            if (!isNaN(t) && t >= cutoff) out[dc] = true;
                        });
                    });
                });
            });
        ...
        // Build a context object for scoring. Computed once per render so
        // every Hero/block read shares the same view of "now."
        function _buildTodayContext(activities) {
            if (_isTodayCacheStale()) {
                try { computeTodayCache(); } catch(e) {}
            }

            // Routine fit
            var groups = (typeof getGroups === 'function') ? getGroups() : [];
            var hasRoutines = groups.length > 0;
            var activeRoutineActivityIds = new Set();
            var nextRoutineActivityIds   = new Set();
            var nextRoutineStartsInMin   = Infinity;
            if (hasRoutines) {
                var now = new Date();
                var minOfDay = now.getHours() * 60 + now.getMinutes();
                var activeGid = (typeof getActiveGroupId === 'function')
                    ? getActiveGroupId(now)
                    : null;

                groups.forEach(function(g) {
                    var startMin = (typeof _groupTimeToMin === 'function')
                        ? _groupTimeToMin(g.timeStart)
                        : -1;
                    var ids = g.activityIds || [];
                    if (g.id === activeGid) {
                        ids.forEach(function(id) { activeRoutineActivityIds.add(id); });
                    } else if (startMin > minOfDay) {
                        var minsAway = startMin - minOfDay;
                        if (minsAway < nextRoutineStartsInMin) {
                            nextRoutineStartsInMin = minsAway;
                            nextRoutineActivityIds = new Set(ids);
                        }
                    }
                });
            }
            ...
            return {
                cache: window._todayCache,
                hoursUntilMidnight: _hoursUntilMidnight(),
                hasRoutines: hasRoutines,
                activeRoutineActivityIds: activeRoutineActivityIds,
                nextRoutineActivityIds: nextRoutineActivityIds,
                nextRoutineStartsInMin: nextRoutineStartsInMin,
                recentDimCompletions: _recentDimCompletions(),
                atRiskActivityIds: new Set()  // populated by the at-risk block before Hero runs
            };
        }

        // ══════════════════════════════════════════════════════════════════
        // BLOCK LIBRARY
        // Each block returns { html, slotsUsed } or null. Blocks are pure:
        // given the same (activities, ctx), return the same output. The
        // composer in renderTodayView decides order + slot budget.
        // ══════════════════════════════════════════════════════════════════

        // SVG building blocks reused across blocks
        var _SVG_FIRE = '<svg viewBox="0 0 24 24" fill="currentColor" width="11" height="11"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>';
        var _SVG_WARN = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" width="11" height="11"><path d="M10.3 3.86 1.82 18a2 2 0 0 0 1.7 3h16.96a2 2 0 0 0 1.7-3L13.7 3.86a2 2 0 0 0-3.4 0Z"/><line x1="12" y1="9" x2="12" y2="13"/><circle cx="12" cy="17" r="1" fill="currentColor"/></svg>';

        // ──────────────────────────────────────────────────────────
        // Block: AT-RISK STRIP
        // Streaks ≥7 days, within 4h of midnight, not done today.
        // Up to 3 inside one block. MUST run before Hero so its
        // activity-IDs populate ctx.atRiskActivityIds (Hero demotes).
        // ──────────────────────────────────────────────────────────
        function blockAtRisk(activities, ctx, max) {
            max = max || 3;
            var hoursLeft = ctx.hoursUntilMidnight;
            if (hoursLeft >= 4) return null;
            var risks = activities
                .filter(function(a) {
                    return a.frequency === 'daily'
                        && !a._completedToday
                        && (a._streak || 0) >= 7
                        && a._canComplete;
                })
                .sort(function(x, y) { return (y._streak || 0) - (x._streak || 0); })
                .slice(0, max);

            if (risks.length === 0) return null;

            // Populate ctx so Hero scoring sees them
            risks.forEach(function(a) { ctx.atRiskActivityIds.add(a.id); });

            var inner = risks.map(function(a) {
                var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;
                return '<button class="today-atrisk-card" data-aid="' + a.id + '"'
                    + ' style="--dim-color:' + dim + ';"'
                    + ' onclick="completeActivityById(\'' + a.id + '\')">'
                    + '<div class="today-atrisk-name">' + escapeHtml(a.name) + '</div>'
                    + '<div class="today-atrisk-meta">'
                    +   '<span class="today-atrisk-streak">' + _SVG_FIRE + (a._streak || 0) + 'd</span>'
                    +   '<span class="today-atrisk-time">' + _formatHoursLeft(hoursLeft) + ' left</span>'
                    + '</div>'
                    + '</button>';
            }).join('');
            ...
            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-atrisk">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-icon">' + _SVG_WARN + '</span>'
                +       '<span class="today-block-title">Streak at risk</span>'
                +       '<span class="today-block-sub">' + _formatHoursLeft(hoursLeft) + ' until midnight</span>'
                +     '</div>'
                +     '<div class="today-atrisk-grid">' + inner + '</div>'
                +   '</div>'
                + '</div>';
            return { html: html, slotsUsed: risks.length, aids: risks.map(function(a) { return a.id; }) };
        }

        function _formatHoursLeft(h) {
        ...
        // ──────────────────────────────────────────────────────────
        // Block: HERO NEXT-UP
        // The single most important activity to do right now.
        // ──────────────────────────────────────────────────────────
        function blockHero(activities, ctx) {
            var best = null, bestScore = -Infinity;
            activities.forEach(function(a) {
                var s = _scoreHero(a, ctx);
                if (s > bestScore) { bestScore = s; best = a; }
            });
            if (!best || bestScore <= -Infinity) return null;

            var dim = DIM_HEX_MAP[best.dimColor] || DIM_HEX_MAP.blue;
            var streak = best._streak || 0;
            var canDo = best._canComplete || (best.allowMultiplePerDay && best.frequency !== 'occasional');
            var click = canDo ? (' onclick="completeActivityById(\'' + best.id + '\')"') : '';

            // Pull a reason chip — the highest-weight signal explaining why
            // this activity won. Helps the user trust the algorithm.
            var reason = _heroReason(best, ctx);

            var html =
                '<div class="today-row today-row-hero">'
                +   '<div class="today-block today-hero" data-aid="' + best.id + '"'
                +     ' style="--dim-color:' + dim + ';"' + click + '>'
                +     '<div class="today-hero-kicker">'
                +       'NEXT UP'
                +       (reason ? ' <span class="today-hero-reason">' + reason + '</span>' : '')
                +     '</div>'
                +     '<div class="today-hero-name">' + escapeHtml(best.name) + '</div>'
                +     '<div class="today-hero-meta">'
                +       '<span class="today-hero-xp">+' + (best.baseXP || 0) + ' XP</span>'
                +       (streak > 0
                            ? ' <span class="today-hero-streak">' + _SVG_FIRE + streak + '</span>'
                            : '')
                +     '</div>'
                +   '</div>'
                + '</div>';
            return { html: html, slotsUsed: 1, aids: [best.id] };
        }

        // One-line explanation of why the Hero was picked. We pick the
        // single dominant signal. Trust is built by surfacing the math.
        function _heroReason(a, ctx) {
            var streak = a._streak || 0;
            if (a.frequency === 'daily' && streak >= 7 && ctx.hoursUntilMidnight < 4) {
                return 'streak ends soon';
            }
            if (ctx.activeRoutineActivityIds.has(a.id)) {
                return 'in your active routine';
            }
            var cp = _cycleProgress(a);
            if (cp && cp.completionsThisCycle < cp.target) {
                var needed = cp.target - cp.completionsThisCycle;
                if (needed === 1 && cp.daysRemainingInCycle <= 2) return 'cycle closes soon';
                if (needed <= 2 && cp.daysRemainingInCycle <= needed) return 'cycle catching up';
            }
            var daysSince = _daysSinceLastCompletion(a);
            if (daysSince >= 8) return daysSince + ' days untouched';
        ...
        // ──────────────────────────────────────────────────────────
        // Block: COMEBACK CARD
        // A streak ≥14d that broke in the last 48h. Supportive copy,
        // one-tap restart. Capped at 1 (wellbeing tradeoff).
        // ──────────────────────────────────────────────────────────
        function blockComeback(activities, ctx) {
            // Phase 2: infer broken streaks from completionDates gaps.
            // A "broken" streak: previously-active daily activity that
            // had ≥14 consecutive days ending with a 2-day gap that ended
            // within the last 48h. We surface the most recent break only.
            var now = Date.now();
            var DAY_MS = 86400000;
            var bestBreak = null;
            activities.forEach(function(a) {
                if (a.frequency !== 'daily') return;
                if (a._completedToday) return; // they already restarted
                var dates = (a.completionDates || []).slice().sort();
                if (dates.length < 14) return;

                // Walk backwards to find the most recent gap
                var lastMs = new Date(dates[dates.length - 1]).getTime();
                if (isNaN(lastMs)) return;
                var hoursSinceLast = (now - lastMs) / 3600000;
                // The gap that "broke" the streak must end within last 48h.
                // For a daily activity, missing yesterday breaks it today.
                if (hoursSinceLast < 24 || hoursSinceLast > 72) return;

                // Count streak length ending at the last completion.
                var streakLen = 1;
                for (var i = dates.length - 2; i >= 0; i--) {
                    var t1 = new Date(dates[i+1]).getTime();
                    var t0 = new Date(dates[i]).getTime();
                    var gap = (t1 - t0) / DAY_MS;
                    if (gap > 1.5) break;
                    if (gap >= 0.5) streakLen++;
                }
                if (streakLen < 14) return;

                if (!bestBreak || streakLen > bestBreak.streakLen) {
                    bestBreak = { activity: a, streakLen: streakLen, hoursSinceLast: hoursSinceLast };
                }
            });

            if (!bestBreak) return null;
            var a = bestBreak.activity;
            var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;

            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-comeback" data-aid="' + a.id + '"'
                +     ' style="--dim-color:' + dim + ';"'
                +     ' onclick="completeActivityById(\'' + a.id + '\')">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-title">Pick it back up</span>'
                +     '</div>'
                +     '<div class="today-comeback-body">'
                +       '<div class="today-comeback-name">' + escapeHtml(a.name) + '</div>'
                +       '<div class="today-comeback-stat">'
                +         '<span class="today-comeback-num">' + bestBreak.streakLen + '</span>'
                +         '<span class="today-comeback-label">day streak — start a new one today</span>'
                +       '</div>'
                +     '</div>'
                +   '</div>'
                + '</div>';
            return { html: html, slotsUsed: 1, aids: [a.id] };
        }

        // ──────────────────────────────────────────────────────────
        // Cycle deadline — TWO-STAGE.
        // Stage 1 (pickCycleCandidates): pure scoring; returns all eligible
        //   activities sorted by urgency. Composer slices into Tier 1 (most
        //   urgent ≤ 1 day remaining) and Tier 3 (the rest).
        // Stage 2 (renderCycleBlock): given a slice of candidates, emits
        //   the markup. Pure render.
        // ──────────────────────────────────────────────────────────
        function pickCycleCandidates(activities, ctx) {
            var heroId = ctx._heroId;
            var atRisk = ctx.atRiskActivityIds;
            var out = [];
            activities.forEach(function(a) {
                if (a.id === heroId) return;
                if (atRisk.has(a.id)) return;
                if (a._completedToday && !(a.allowMultiplePerDay && a.frequency !== 'occasional')) return;
                var cp = _cycleProgress(a);
                if (!cp) return;
                var needed = cp.target - cp.completionsThisCycle;
                if (needed <= 0) return;
                if (cp.daysRemainingInCycle > needed + 1) return;
                out.push({
                    activity: a, cp: cp,
                    urgency: needed / Math.max(0.5, cp.daysRemainingInCycle)
                });
            });
            out.sort(function(x, y) { return y.urgency - x.urgency; });
            return out;
        }

        function renderCycleBlock(candidates, ctx, label) {
            if (!candidates || candidates.length === 0) return null;
            label = label || 'Cycle closing';
            var inner = candidates.map(function(c) {
                var a = c.activity, cp = c.cp;
                var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;
                var pct = Math.min(100, Math.round((cp.completionsThisCycle / cp.target) * 100));
                return '<button class="today-cycle-card" data-aid="' + a.id + '"'
                    + ' style="--dim-color:' + dim + ';"'
                    + ' onclick="completeActivityById(\'' + a.id + '\')">'
                    + '<div class="today-cycle-name">' + escapeHtml(a.name) + '</div>'
                    + '<div class="today-cycle-progress">'
                    +   '<div class="today-cycle-bar"><div class="today-cycle-bar-fill" style="width:' + pct + '%"></div></div>'
                    +   '<div class="today-cycle-stat">' + cp.completionsThisCycle + '/' + cp.target
                    +     '<span class="today-cycle-deadline"> · ' + cp.daysRemainingInCycle + 'd left</span></div>'
                    + '</div>'
                    + '</button>';
            }).join('');
            ...
            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-cycle-wrap">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-title">' + label + '</span>'
                +     '</div>'
                +     '<div class="today-cycle-grid">' + inner + '</div>'
                +   '</div>'
                + '</div>';
            return {
                html: html,
                slotsUsed: candidates.length,
                aids: candidates.map(function(c) { return c.activity.id; })
            };
        }

        // ──────────────────────────────────────────────────────────
        // Block: STREAK SHOWCASE
        // Active streaks ≥14d. Up to 2, longest first. Identity block —
        // celebrates the user's earned commitment. No tap action (purely
        // a status surface; tap-to-do happens elsewhere).
        // ──────────────────────────────────────────────────────────
        function blockStreakShowcase(activities, ctx, max) {
            max = max || 2;
            var top = (ctx.cache.topStreaks || []).slice(0, max);
            if (top.length === 0) return null;

            // Hydrate with activity data
            var hydrated = top.map(function(s) {
                var found = null;
                activities.forEach(function(a) { if (a.id === s.aid) found = a; });
                return { meta: s, activity: found };
            }).filter(function(x) { return x.activity; });
            if (hydrated.length === 0) return null;

            var inner = hydrated.map(function(h) {
                var dim = DIM_HEX_MAP[h.meta.dimColor] || DIM_HEX_MAP.blue;
                var size = h.meta.streak >= 60 ? 'today-streak-card--xl'
                          : h.meta.streak >= 30 ? 'today-streak-card--lg'
                          : '';
                return '<div class="today-streak-card ' + size + '"'
                    + ' style="--dim-color:' + dim + ';">'
                    + '<div class="today-streak-num">' + h.meta.streak + '</div>'
                    + '<div class="today-streak-unit">days</div>'
                    + '<div class="today-streak-name">' + escapeHtml(h.meta.name) + '</div>'
                    + '</div>';
            }).join('');

            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-streak-wrap">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-title">Streaks you\'ve earned</span>'
                +     '</div>'
                +     '<div class="today-streak-grid">' + inner + '</div>'
                +   '</div>'
                + '</div>';
            return {
                html: html,
                slotsUsed: hydrated.length,
                aids: hydrated.map(function(h) { return h.activity.id; })
            };
        }

        // ──────────────────────────────────────────────────────────
        // Block: DIMENSION FOCUS
        // A single dimension is ≥55% of last 14 days of XP. Banner-style
        // identity statement. 1 slot.
        // ──────────────────────────────────────────────────────────
        function blockDimensionFocus(activities, ctx) {
            var dc = ctx.cache.dominantDimColor;
            if (!dc) return null;
            var pct = Math.round((ctx.cache.dominantDimPct || 0) * 100);

            // Find the dimension name
            var dimName = '';
            (window.userData.dimensions || []).forEach(function(d) {
                if ((d.color || 'blue') === dc) dimName = d.name;
            });
            if (!dimName) return null;

            var dim = DIM_HEX_MAP[dc] || DIM_HEX_MAP.blue;
            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-dim-focus" style="--dim-color:' + dim + ';">'
                +     '<div class="today-dim-pct">' + pct + '%</div>'
                +     '<div class="today-dim-text">'
                +       '<div class="today-dim-title">' + escapeHtml(dimName) + ' is your focus</div>'
                +       '<div class="today-dim-sub">' + pct + '% of your XP this past 14 days</div>'
                +     '</div>'
                +   '</div>'
                + '</div>';
            return { html: html, slotsUsed: 1, aids: [] };
        }

        // ──────────────────────────────────────────────────────────
        // Block: QUICK WINS
        // Low-XP (≤15) pending activities, rendered as small tiles. Up to
        // `max` (default 3, expanded by the composer when slots are free).
        // Suppressed entirely if no low-XP pending — composer then falls
        // back to blockMoreToDo for any remaining pending activities.
        // ──────────────────────────────────────────────────────────
        function blockQuickWins(activities, ctx, max) {
            max = max || 3;
            var excluded = ctx._usedAids || new Set();
            var quickList = activities.filter(function(a) {
                if (excluded.has(a.id)) return false;
                if (a._completedToday && !(a.allowMultiplePerDay && a.frequency !== 'occasional')) return false;
                if (!a._canComplete && !(a.allowMultiplePerDay && a.frequency !== 'occasional')) return false;
                if (a.frequency === 'custom' && a.customSubtype === 'days' && a._isScheduledDay === false) return false;
                return (a.baseXP || 0) <= 15;
            }).slice(0, max);

            if (quickList.length === 0) return null;

            return {
                html: _renderTileGroup(quickList, 'Quick wins', 'today-quick'),
                slotsUsed: quickList.length,
                aids: quickList.map(function(a) { return a.id; })
            };
        }

        // ──────────────────────────────────────────────────────────
        // Block: MORE TO DO  (fallback when quick-wins is empty/insufficient)
        // Any pending activity not yet placed by another block. Used by the
        // composer to fill leftover slots so the screen never feels empty
        // for a power user with mostly mid/high-XP activities.
        // ──────────────────────────────────────────────────────────
        function blockMoreToDo(activities, ctx, max) {
            max = max || 3;
            var excluded = ctx._usedAids || new Set();
            ...
            // Order: higher XP first so the "more to do" group leads with
            // meatier items — users will engage with these less often than
            // the Hero, so showcase the substantive ones.
            .sort(function(x, y) { return (y.baseXP || 0) - (x.baseXP || 0); })
            .slice(0, max);

            if (list.length === 0) return null;
            return {
                html: _renderTileGroup(list, 'More to do', 'today-quick'),
                slotsUsed: list.length,
                aids: list.map(function(a) { return a.id; })
            };
        }

        // Shared tile renderer used by Quick wins + More to do (same visual).
        function _renderTileGroup(list, label, cssBase) {
            var tiles = list.map(function(a) {
                var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;
                return '<button class="today-quick-tile" data-aid="' + a.id + '"'
                    + ' style="--dim-color:' + dim + ';"'
                    + ' onclick="completeActivityById(\'' + a.id + '\')">'
                    + '<div class="today-quick-name">' + escapeHtml(a.name) + '</div>'
                    + '<div class="today-quick-xp">+' + (a.baseXP || 0) + ' XP</div>'
                    + '</button>';
            }).join('');
            return '<div class="today-row">'
                +   '<div class="today-block today-quick-wrap">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-title">' + label + '</span>'
                +     '</div>'
                +     '<div class="today-quick-grid">' + tiles + '</div>'
                +   '</div>'
                + '</div>';
        }
        ...
        // ──────────────────────────────────────────────────────────
        // Block: DORMANT NUDGE
        // Activities untouched for ≥7 days. Up to `max` (default 2;
        // composer can request more if there's slot room).
        // ──────────────────────────────────────────────────────────
        function blockDormantNudge(activities, ctx, max) {
            max = max || 2;
            var excluded = ctx._usedAids || new Set();
            var d = (ctx.cache.dormant || []).slice();

            // Hydrate + filter
            var hydrated = d.map(function(rec) {
                var found = null;
                activities.forEach(function(a) { if (a.id === rec.aid) found = a; });
                return { meta: rec, activity: found };
            }).filter(function(x) {
                if (!x.activity) return false;
                if (excluded.has(x.activity.id)) return false;
                if (x.activity._completedToday) return false;
                return true;
            }).slice(0, max);

            if (hydrated.length === 0) return null;

            var tiles = hydrated.map(function(h) {
                var a = h.activity;
                var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;
                return '<button class="today-dormant-tile" data-aid="' + a.id + '"'
                    + ' style="--dim-color:' + dim + ';"'
                    + ' onclick="completeActivityById(\'' + a.id + '\')">'
                    + '<div class="today-dormant-name">' + escapeHtml(a.name) + '</div>'
                    + '<div class="today-dormant-sub">' + h.meta.daysSince + 'd untouched</div>'
                    + '</button>';
            }).join('');

            var html =
                '<div class="today-row">'
                +   '<div class="today-block today-dormant-wrap">'
                +     '<div class="today-block-header">'
                +       '<span class="today-block-title">Easy to forget</span>'
                +     '</div>'
                +     '<div class="today-dormant-grid">' + tiles + '</div>'
                +   '</div>'
                + '</div>';
            return {
                html: html,
        ...
        // ──────────────────────────────────────────────────────────
        // Block: JUST DONE  (small strip below the active blocks)
        // Activities completed in the last few hours (≤4h since their
        // most recent completionDates entry). Quiet acknowledgment that
        // the user just made progress; doesn't count toward slot budget.
        // Shows max 4 chips, scrolls horizontally if more.
        // ──────────────────────────────────────────────────────────
        function blockJustDone(activities, ctx) {
            var now = Date.now();
            var CUTOFF_MS = 4 * 3600 * 1000;
            var recent = activities
                .map(function(a) {
                    ...
                    var sinceMs = now - lastMs;
                    if (sinceMs > CUTOFF_MS || sinceMs < 0) return null;
                    return { activity: a, sinceMs: sinceMs };
                })
                .filter(Boolean)
                .sort(function(x, y) { return x.sinceMs - y.sinceMs; })
                .slice(0, 4);
            ...
            var chips = recent.map(function(r) {
                var a = r.activity;
                var dim = DIM_HEX_MAP[a.dimColor] || DIM_HEX_MAP.blue;
                var mins = Math.max(1, Math.round(r.sinceMs / 60000));
                var when = mins < 60 ? (mins + 'm ago') : (Math.round(mins / 60) + 'h ago');
                return '<div class="today-justdone-chip" style="--dim-color:' + dim + ';">'
                    + '<svg viewBox="0 0 24 24" fill="none" stroke="var(--chip-xp-fg)" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" width="10" height="10"><polyline points="20 6 9 17 4 12"/></svg>'
                    + '<span class="today-justdone-name">' + escapeHtml(a.name) + '</span>'
                    + '<span class="today-justdone-when">' + when + '</span>'
                    + '</div>';
            }).join('');

            var html =
                '<div class="today-row today-row-justdone">'
                +   '<div class="today-justdone-strip">' + chips + '</div>'
                + '</div>';
            // slotsUsed: 0 — this strip is acknowledgment, not a slot in the
            // 10-card budget. It always renders if any recent completions exist.
            return { html: html, slotsUsed: 0, aids: [] };
        }

        // ══════════════════════════════════════════════════════════════════
        // RENDER COMPOSER (Phase 3: tier-based slot budget + elastic expansion)
        // ══════════════════════════════════════════════════════════════════
        // Slot budget: 10 cards visible (excludes the "Just done" strip and
        // block headers — only counts activity-touching tiles).
        // Tier 1 — urgency (must show if exists):
        //   Hero, At-risk strip, urgent Cycle (≤1d remaining)
        // Tier 2 — identity (show if earned):
        //   Comeback, Streak showcase, Dimension focus
        // Tier 3 — momentum (fills the rest, elastic):
        //   Remaining Cycle, Quick wins → More to do, Dormant
        //
        // Dormant is GUARANTEED at least 1 slot if any dormant activity
        // exists — counters rich-get-richer dynamics.
        var TODAY_SLOT_CAP = 10;

        function renderTodayView(activities) {
            var ctx = _buildTodayContext(activities);
            // _usedAids tracks every activity ID consumed by a block so far,
            // so later blocks can skip duplicates. Excludes Just-done (which
            // shows recently-completed activities — fine to also appear in
            // another block if eligible, but in practice completed activities
            // won't qualify for the others.)
            ctx._usedAids = new Set();
            var slotsLeft = TODAY_SLOT_CAP;
            var parts = [];

            function place(block) {
                if (!block) return false;
                if (slotsLeft <= 0) return false;
                // If the block wants more slots than we have left, the
                // block's own max param should already have constrained
                // it — but as a safety, refuse to overflow.
                if (block.slotsUsed > slotsLeft) return false;
                parts.push(block.html);
                slotsLeft -= block.slotsUsed;
                (block.aids || []).forEach(function(id) { ctx._usedAids.add(id); });
                return true;
            }

            // ── Tier 1: urgency ──────────────────────────────────────────
            // At-risk first so its IDs feed Hero scoring (the −60 demote
            // ensures Hero ≠ what's already shown in the strip).
            var atRisk = blockAtRisk(activities, ctx);
            // Hero
            var hero = blockHero(activities, ctx);
            // Cycle candidates — split urgent (≤1d) into Tier 1, rest to Tier 3.
            // We need to know the Hero ID first so cycle candidates exclude it.
            ctx._heroId = (hero && hero.aids && hero.aids[0]) || null;
            var allCycle = pickCycleCandidates(activities, ctx);
            var urgentCycle = allCycle.filter(function(c) { return c.cp.daysRemainingInCycle <= 1; });
            var laterCycle  = allCycle.filter(function(c) { return c.cp.daysRemainingInCycle > 1; });

            // Place in Tier 1 order: Hero → At-risk → urgent Cycle
            place(hero);
            place(atRisk);
            if (urgentCycle.length > 0) {
                place(renderCycleBlock(urgentCycle.slice(0, 2), ctx, 'Cycle closing today'));
            }

            // ── Tier 2: identity ─────────────────────────────────────────
            // Streak showcase, Comeback, Dimension focus.
            // Comeback first when it qualifies — re-engagement is high-value
            // and capped at 1 for wellbeing. Streak showcase second.
            var comeback  = blockComeback(activities, ctx);
            var streaks   = blockStreakShowcase(activities, ctx);
            var dimension = blockDimensionFocus(activities, ctx);
            place(comeback);
            place(streaks);
            place(dimension);

            // ── Tier 3: momentum fillers + remaining cycles ──────────────
            // Remaining cycle deadlines (2-6 days out) take their spot first.
            if (laterCycle.length > 0 && slotsLeft > 0) {
                place(renderCycleBlock(laterCycle.slice(0, Math.min(2, slotsLeft)), ctx, 'Cycles to watch'));
            }

            // Quick wins: try the standard cap first. If none, fall back to
            // More-to-do for any remaining pending activity.
            if (slotsLeft > 0) {
                var qw = blockQuickWins(activities, ctx, Math.min(3, slotsLeft));
                if (qw) {
                    place(qw);
                } else {
                    // No low-XP pending; pull in mid/high-XP pending.
                    var more = blockMoreToDo(activities, ctx, Math.min(3, slotsLeft));
                    place(more);
                }
            }

            // Dormant — GUARANTEED slot if any dormant exists. To enforce
            // the guarantee even when slots are scarce, we check dormant
            // existence BEFORE placing the last few quick-wins/more-to-do.
            // Phase 3 keeps it simple: place after the fillers, capped at
            // remaining slots. If a power-user's screen is fully filled by
            // Tier 1+2, dormant gets squeezed — that's correct, urgency
            // wins over nudging.
            if (slotsLeft > 0) {
                var dormant = blockDormantNudge(activities, ctx, Math.min(2, slotsLeft));
                place(dormant);
            }

            // ── Final expansion: if we still have slot budget, expand
            // quick-wins / more-to-do up to its full pool. This handles
            // the "user with 8 activities, only quick-wins survives" case
            // — give them a roomy quick-wins block rather than empty space.
            if (slotsLeft > 2) {
                var qwExpand = blockQuickWins(activities, ctx, slotsLeft);
                if (qwExpand) {
                    place(qwExpand);
                } else {
                    var moreExpand = blockMoreToDo(activities, ctx, slotsLeft);
                    place(moreExpand);
                }
            }

            // ── Just-done strip — always last, free (slotsUsed = 0) ──────
            // Acknowledgment of recent work. Doesn't count toward budget.
            var justDone = blockJustDone(activities, ctx);

            // If we have no active blocks but DO have a Just-done strip,
            // promote it from "ack strip" into "you did it" celebration —
            // it becomes the primary content of the screen.
            if (parts.length === 0) {
                return _renderTodayEmpty(activities, justDone);
            }

            if (justDone) parts.push(justDone.html);
            return '<div class="today-view">' + parts.join('') + '</div>';
        }

        // Empty-state renderer — three distinct cases, each with copy that
        // matches what's actually true. Mis-stating "all caught up" to a
        // new user with zero activities was the bug we're fixing here.
        function _renderTodayEmpty(activities, justDone) {
            var totalActivities = activities.length;
            var completedToday = activities.filter(function(a) {
                return a._completedToday;
            }).length;

            // Case 1: new user, zero activities. Send them to the Manage
            // view to add their first one.
            if (totalActivities === 0) {
                return '<div class="today-view">'
                    + '<div class="today-empty today-empty-onboarding">'
                    +   '<div class="today-empty-title">No activities yet</div>'
                    +   '<div class="today-empty-sub">Add your first activity to start tracking. Tap Manage to set them up.</div>'
                    +   '<button class="today-empty-cta" onclick="openActivityModal(null,null)">'
            ...
            // Case 2: all today's work is done — celebratory. The Just-done
            // strip becomes the hero of the screen when this fires.
            if (completedToday > 0) {
                var jdHtml = justDone ? justDone.html : '';
                return '<div class="today-view">'
                    + '<div class="today-empty today-empty-allcaughtup">'
                    +   '<div class="today-empty-checkmark">'
                    +     '<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>'
                    +   '</div>'
                    +   '<div class="today-empty-title">All done today</div>'
                    +   '<div class="today-empty-sub">' + completedToday + ' ' + (completedToday === 1 ? 'activity' : 'activities') + ' complete. See you tomorrow.</div>'
                    + '</div>'
                    + jdHtml
                    + '</div>';
            }

            // Case 3: activities exist but none eligible right now (e.g.,
            // custom-day activities scheduled for tomorrow, weekly activities
            // already at cycle target, etc.).
            return '<div class="today-view">'
                + '<div class="today-empty">'
                +   '<div class="today-empty-title">Nothing to do right now</div>'
                +   '<div class="today-empty-sub">Your activities will surface here as they become available.</div>'
                + '</div>'
                + '</div>';
        }

        // Backwards-compat shim: a few call sites still reference
        // renderActivityGridCards. Route them to the new renderer.
        function renderActivityGridCards(activities) {
            return renderTodayView(activities);
        }

        // Migration: drop legacy settings.gridCardTypes (silent — users
        // lose their card-size preferences, which the new view doesn't use).
        if (window.userData && window.userData.settings && window.userData.settings.gridCardTypes) {
            delete window.userData.settings.gridCardTypes;
            if (typeof saveUserData === 'function') saveUserData().catch(function() {});
        }

        // Deprecated entry points kept as no-ops so stale refs don't throw.
        window.openCardTypePicker  = function() {};
        window.closeCardTypePicker = function() {};
        window.selectCardType      = function() {};
        window.openGridActionMenu  = function() {};
        window.closeGridActionMenu = function() {};
        window.openGridCardOverlay = function() {};
        window.closeGridCardOverlay = function() {};
        window.cycleGridCardType   = function() {};
        window._gcPointerDown   = function() {};
        window._gcPointerUp     = function() {};
        window._gcPointerCancel = function() {};
        window._gcPointerMove   = function() {};
        ...
        function renderDimensions() {
            ...
            const dimHtml = dimensions.map((dim, i) => {
            ...
            if (uncActs.length > 0) {
                ...
                const actRows = uncActs.map((act, ai) => {
                    ...
                    <div class="activity-item" style="cursor:default;">
                        <div class="activity-info-container">
                            ...
                            <div class="activity-details">
                                <span class="activity-badge badge-xp">+${act.baseXP} XP</span>
                            </div>
                        </div>
                        <div class="dimension-actions" onclick="event.stopPropagation()">
                            <button class="btn-icon" onclick="editActivity(${uncDimIndex},${uncPathIndex},${ai})">Edit</button>
                            <button class="btn-icon delete" onclick="deleteActivity(${uncDimIndex},${uncPathIndex},${ai})">Delete</button>
                        </div>
                    </div>`;
                }).join('');
                uncHtml = `
                <div class="dimension-card uncategorized-section" style="margin-top:16px;border-style:dashed;">
                    <div class="dimension-header" style="cursor:default;">
                        <div class="dimension-info">
                            <div class="dimension-name" style="opacity:0.7;">📥 Uncategorized</div>
                            <div class="dimension-meta"><span>${uncActs.length} ${uncActs.length === 1 ? 'activity' : 'activities'} — assign a Dimension &amp; Path to organise</span></div>
                        </div>
                    </div>
                    <div class="dimension-content expanded">
                        ${actRows}
                    </div>
                </div>`;
            }
        ...
        // ── Populate path dropdown (respects uncategorized) ───────────
        window.populateActivityPathSelect = function() {
            ...
            const paths = dim ? (dim.paths || []).filter(p => p.id !== 'uncategorized') : [];
            pathSel.innerHTML = paths.length > 0
        ...
        window.openActivityModal = function(dimIndex, pathIndex, actIndex = null) {
            ...
            if (actIndex !== null) {
                ...
                // Set dim/path dropdowns to current assignment
                const curDimId = dims[dimIndex].id;
                dimSel.value = curDimId || 'uncategorized';
                populateActivityPathSelect();
                const curPathId = dims[dimIndex].paths[pathIndex].id;
                document.getElementById('activityPathSelect').value = curPathId || 'uncategorized';
                ...
                const hasAdvanced = (
                    ...
                    (curPathId && curPathId !== 'uncategorized') ||
                ...
                if (dimIndex !== null && pathIndex !== null && dims[dimIndex] && dims[dimIndex].paths[pathIndex]) {
                    document.getElementById('activityPathSelect').value = dims[dimIndex].paths[pathIndex].id || 'uncategorized';
                }
        ...
        // ── Advanced Setup section (activity modal) ───────────────────
        // setAdvancedSectionOpen drives both the visible content and the
        // chevron rotation. Called by openActivityModal on every open with
        // a derived initial state (open if any advanced field is non-default).
        ...
        window.toggleAdvancedSection = function() {
            const content = document.getElementById('advancedSectionContent');
            if (!content) return;
            setAdvancedSectionOpen(content.style.display === 'none');
        };
        ...
        window.saveActivity = async function(event) {
            ...
            if (!selectedDimId || selectedDimId === 'uncategorized') {
                ...
                if (di === -1) { const unc = getOrCreateUncategorized(); targetDi = unc.di; targetPi = unc.pi; }
                else {
                    targetDi = di;
                    const pi = dims[di].paths.findIndex(p => p.id === selectedPathId);
                    if (pi === -1) { const unc = getOrCreateUncategorized(); targetDi = unc.di; targetPi = unc.pi; }
                    else targetPi = pi;
                }
            }
        ...
        // Shield callers pass cap+1; penalty callers pass 7.
        ...
        window.renderSearchResults = function() {
            ...
            // Action column is FIXED-WIDTH via CSS (.search-result-item
            // grid template). Buttons swap inside it, but the row never
            // resizes — completing or undoing an item doesn't reflow the
            // list.
            results.innerHTML = filtered.map(act => {
                ...
                // Status indicator next to name — checkmark when done today.
                ...
                // Streak chip — same shape/color as main list
                ...
                return `<div class="search-result-item${done ? ' completed' : ''}">
                    <div class="search-result-content">
                        <div class="search-result-name">${statusIcon}<span style="overflow:hidden;text-overflow:ellipsis;">${escapeHtml(act.name)}</span></div>
                        <div class="search-result-meta">
                            ...
                            <span>${freqLabel[act.frequency]||act.frequency}</span>
                            <span class="search-result-dot">·</span>
        ...
        function processStreakSystem(activity, today) {
            ...
            let streak = 0;
            let shieldsConsumed = 0;
            let walkCapUsed = BASE_SHIELDS; // shield cap re-derived by walk
            const MAX_WALK = 400;

            if (streakStartFloor) {
                // ── Forward walk (chronologically correct) ───────────────────
                // Walk from streakStartWindow → today, oldest window first.
                // Shields are consumed in the order misses actually occurred, which
                // matches the user's mental model: "I missed day 3 so a shield was
                // used; I missed day 8 so the streak broke."
                //
                // The backward walk had a critical flaw: it allocated shields to
                // the most RECENT misses first. A pattern like:
                //   [hit, miss, miss, hit, hit, miss, miss, miss]   (forward time)
                // Walking backward: miss×3 → use 3 shields → find hit×2 → miss
                // → no shields → stop. Result: streak=2 (wrong).
                // Walking forward: hit → miss×2 (shield×2) → hit×2 → miss×3
                // → use shield 3 on first → unshielded miss → BREAK. Result: streak=0 (correct).
                ...
                for (let i = 0; i < MAX_WALK; i++) {
                    ...
                    if (hit) {
                        streak++;
                        // Milestone bonus: +1 shield added to balance when streak
                        // crosses 25/50/75/100, exactly as completeActivity does live.
                        if (SHIELD_MILESTONES.includes(streak))
                            walkCapUsed = Math.min(SHIELD_ABS_CAP, walkCapUsed + 1);
                    } else if (streak > 0 && (walkCapUsed - shieldsConsumed) > 0) {
                        shieldsConsumed++;
                    } else if (streak > 0) {
                        // Unshielded miss — streak breaks.
                        // Reset for any new streak segment that may start later.
                        streak = 0; shieldsConsumed = 0; walkCapUsed = BASE_SHIELDS;
                    }
                ...
                // ── Backward walk (fallback: no streakStartWindow set) ────────
                // Only reached for activities that pre-date the streakStartWindow
                // field and haven't completed since. New streaks always set
                // streakStartWindow via completeActivity, so this path is rare.
                let shieldsLeft = BASE_SHIELDS;
                let hitFloor = false;
                for (let i = 0; i < MAX_WALK; i++) {
                    if (!cursor || cursor < walkFloor) { hitFloor = true; break; }
                    const nextWindowStart = getNextCycleWindowStart(activity, cursor);
                    if (!nextWindowStart) { hitFloor = true; break; }
                    const cStart = cursor.getTime(), cEnd = nextWindowStart.getTime();
                    const hit = sortedHistory.some(e => {
                        const t = new Date(e.date).getTime();
                        return t >= cStart && t < cEnd;
                    });
                    if (hit) { streak++; }
                    else if (shieldsLeft > 0) { shieldsConsumed++; shieldsLeft--; }
                    else { break; }
                    cursor = getCycleWindowStart(activity, new Date(cursor.getTime() - 1));
                }
                // History-floor extension: trust stored streak for pre-history portion
                // if the visible run was perfectly clean (no shield use, no break).
                const _stored = activity.streak || 0;
                if (hitFloor && streak > 0 && shieldsConsumed === 0 && _stored > streak)
                    streak = _stored;
                walkCapUsed = Math.min(SHIELD_ABS_CAP,
                    BASE_SHIELDS + SHIELD_MILESTONES.filter(m => streak >= m).length);
            }
            ...
            if (isFirstRun && streak === 0 && (activity.streak || 0) > 0) {
                activity.shieldsConsumed = Math.min(BASE_SHIELDS, activity.streakPauseUses || 0);
                return true;
            }
            ...
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        ...
                        let migShieldsLeft = BASE_SHIELDS;
        ...
        function showToast(message, color = 'blue') {
            ...
            toast.style.cssText = `position:fixed;top:100px;right:20px;background:${map[color]||map.blue};
                color:var(--color-text-on-accent,#fff);padding:14px 22px;border-radius:12px;font-weight:600;font-size:15px;
                z-index:10000;box-shadow:var(--shadow-lg);animation:slideIn 0.3s ease;`;
        ...
        window.obBuildOwn = async function() {
            ...
            obCloseOverlay();
            showToast('Welcome to Mindkraft! Create your first activity to get started.', 'blue');
        ...
        window.obFinishPicker = async function() {
            ...
            try {
                ...
                await saveUserData();
                obCloseOverlay();
                showToast('🎉 Your space is ready! Tap an activity to log it.', 'blue');
                if (window.switchTab) switchTab('activities');
        ...
        const TUTORIAL_STEPS = [
            {
                ...
                body: 'Activities are the habits and tasks you want to track. Tap the button below to create your first one and start earning XP!',
                cta: '+ Create Activity',
        ...
        function showTabUnlockSpotlight(tabName) {
            ...
            card.innerHTML = `
                ...
                <h3 class="spotlight-title">${meta.label} unlocked</h3>
        ...
        window.showCurrentTutorialStep = function() {
            ...
            card.innerHTML = `
                ...
                <button class="tutorial-cta" onclick="${s.action}">${s.cta}</button>
            `;
