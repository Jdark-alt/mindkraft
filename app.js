        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // For skip-negative activities, record which cycle window this completion
            // falls in so processSkipPenalty can use window-boundary logic correctly.
            if (activity.isSkipNegative && !isOccasional) {
                const _winStart = getCycleWindowStart(activity, new Date());
                if (_winStart) {
                    activity.lastCompletedCycleStart = _winStart.toISOString().slice(0, 10);
                }
            }
        ...
        // ── getCycleWindowStart ───────────────────────────────────────
        // Returns the start (local midnight) of the cycle window that contains
        // the given Date, using the EXACT same window boundaries as isCompletedToday().
        // Returns null for occasional activities (no penalty windows).
        function getCycleWindowStart(activity, dateObj) {
            ...
            if (freq === 'custom') {
                if (activity.customSubtype === 'days') {
                    // Weekly window: Monday-anchored (penalty is once per week)
        ...
        // Returns the start of the cycle window immediately after the one
        // containing dateObj. Pure date arithmetic, zero I/O.
        function getNextCycleWindowStart(activity, dateObj) {
            ...
            if (freq === 'weekly') {
                const next = new Date(current); next.setDate(current.getDate() + 7); return next;
            }
            if (freq === 'biweekly') {
                const next = new Date(current); next.setDate(current.getDate() + 14); return next;
            }
            if (freq === 'monthly') {
            ...
            if (freq === 'custom') {
                if (activity.customSubtype === 'days') {
                    ...
                    const cycleDays = activity.customDays || 1;
                    const next = new Date(current); next.setDate(current.getDate() + cycleDays); return next;
                }
            }
            return null;
        }
        ...
        // Returns true if the activity has at least one positive-XP completion
        // anywhere in [windowStart, windowEnd). Falls back to lastCompleted.
        ...
        // ── processSkipPenalty ────────────────────────────────────────────
        // Called on every login via processStreakPauses().
        //
        // Logic:
        //   1. Find the last cycle window the user actually completed
        //      (lastCompletedCycleStart if set, else derived from lastCompleted,
        //       else derived from createdAt giving one free window).
        //   2. Walk forward through every FULLY CLOSED window after that one.
        //   3. Each closed window with no completion = 1 missed cycle (cap 7).
        //   4. Apply penalty, consume shields, break streak if needed.
        //
        // The CURRENT open window is NEVER penalised.
        // Runs at most once per calendar day (lastSkipCheckDate gate).
        ...
        function processSkipPenalty(activity, today) {
            ...
            // ── Derive last-completed window start ────────────────────────
            // Priority: lastCompletedCycleStart (set by completeActivity going forward)
            //           > lastCompleted (legacy, migration-safe)
            //           > createdAt (never completed, one free window before penalty)
            let lastCompletedWindowStart = null;
            if (activity.lastCompletedCycleStart) {
                // Already tracked precisely — use it directly.
                lastCompletedWindowStart = new Date(activity.lastCompletedCycleStart + 'T00:00:00');
            } else if (activity.completionHistory && activity.completionHistory.length > 0) {
                // No prior lastCompletedCycleStart yet (pre-patch or first-time).
                // Anchor from the EARLIEST positive completion so the walk can detect
                // any missed windows between early and late completions.
                // (Using lastCompleted would skip over windows missed before a later completion.)
                const earliest = activity.completionHistory
                    .filter(e => (e.xp || 0) > 0)
                    .map(e => new Date(e.date).getTime())
                    .reduce((min, t) => Math.min(min, t), Infinity);
                if (earliest !== Infinity) {
                    lastCompletedWindowStart = getCycleWindowStart(activity, new Date(earliest));
                }
            }
            if (!lastCompletedWindowStart && activity.lastCompleted) {
                // Legacy fallback: no completionHistory array — use lastCompleted.
                lastCompletedWindowStart = getCycleWindowStart(activity, new Date(activity.lastCompleted));
            }
            if (!lastCompletedWindowStart && activity.createdAt) {
                // Never completed — give one free window before penalties start.
                lastCompletedWindowStart = getCycleWindowStart(activity, new Date(activity.createdAt));
            }
            if (!lastCompletedWindowStart) {
                activity.lastSkipCheckDate = today;
                return false;
            }

            if (!lastCompletedWindowStart || isNaN(lastCompletedWindowStart.getTime())) {
                activity.lastSkipCheckDate = today;
                return false;
            }

            // Stamp today before any further early returns
            activity.lastSkipCheckDate = today;

            // The window containing today — anything at or after this is still open
            ...
            // ── Walk closed windows ─────────────────────────────────────────────
            let missedCycles = 0;
            let cursor = getNextCycleWindowStart(activity, lastCompletedWindowStart);
            if (!cursor || isNaN(cursor.getTime())) return false;

            // lastEvaluatedWindowStart tracks the furthest window we examine so we can
            // advance lastCompletedCycleStart after the penalty is applied. This prevents
            // tomorrow's scan from re-counting the same windows again.
            let lastEvaluatedWindowStart = lastCompletedWindowStart;
            ...
            while (cursor.getTime() < todayCycleStartMs) {
                ...
                lastEvaluatedWindowStart = cursor;
            ...
            // Advance lastCompletedCycleStart to the last window we evaluated.
            // This is the critical line that prevents double-counting on future logins:
            // tomorrow's scan will start from here, not from lastCompleted.
            activity.lastCompletedCycleStart = lastEvaluatedWindowStart.toISOString().slice(0, 10);
