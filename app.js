        ...
        // Spawn a floating "+XP" bubble from a card on completion.
        // The bubble flies along an animated arc from the card to the
        // progress bar — visually "depositing" XP into the bar. On undo
        // (negative xpAmount) the trajectory reverses: bubble flies FROM
        // the progress bar TO the card, indicating XP being withdrawn.
        // The progress bar gets a brief shimmer pulse when the bubble lands
        // (or departs on undo) to confirm the transaction.
        function spawnFloatingXP(activityId, xpAmount) {
            try {
                ...
                var barContainer = document.querySelector('.progress-bar-container');
                if (!barContainer) return;

                var cardRect = card.getBoundingClientRect();
                var barRect = barContainer.getBoundingClientRect();

                // Bubble's origin (card right area) and destination (the
                // current right edge of the filled bar — the "where XP
                // gets added" point). For undo we swap origin/destination.
                var fillPct = parseFloat((bar && bar.style.width) || '0') / 100;
                if (isNaN(fillPct)) fillPct = 0;
                var barLandX = barRect.left + Math.min(barRect.width, Math.max(20, barRect.width * fillPct));
                var barLandY = barRect.top + barRect.height / 2;
                var cardOrigX = cardRect.right - 50;
                var cardOrigY = cardRect.top + cardRect.height / 2;

                var isNeg = xpAmount < 0;
                var startX = isNeg ? barLandX : cardOrigX;
                var startY = isNeg ? barLandY : cardOrigY;
                var endX = isNeg ? cardOrigX : barLandX;
                var endY = isNeg ? cardOrigY : barLandY;

                // Build bubble
                var el = document.createElement('div');
                el.className = 'act-xp-float' + (isNeg ? ' xp-float-neg' : '');
                var sign = xpAmount >= 0 ? '+' : '−';
                var iconColor = isNeg ? 'var(--color-accent-red)' : 'var(--chip-xp-fg)';
                el.innerHTML =
                    '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="' + iconColor + '" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">' +
                    (isNeg
                        ? '<line x1="5" y1="12" x2="19" y2="12"/>'
                        : '<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>'
                    ) + '</svg>' +
                    '<span>' + sign + Math.abs(xpAmount) + ' XP</span>';
                el.style.left = startX + 'px';
                el.style.top = startY + 'px';
                document.body.appendChild(el);

                // Two-frame paint: first frame appends with start coords +
                // entry class (bubble pops in at origin). Next frame moves
                // to end coords (CSS `left`/`top` transition handles the
                // flight). On flight end, fade-leave class triggers exit.
                requestAnimationFrame(function() {
                    el.classList.add('xp-float-enter');
                    // Tiny delay so the entry pop reads as a distinct beat
                    // before the flight starts (~90ms feels right).
                    setTimeout(function() {
                ...
                // Mid-flight: trigger the absorb pulse on the bar as the
                // bubble approaches. Timed to roughly match arrival.
                setTimeout(function() {
                    if (bar) {
                        bar.classList.remove('absorb-pulse');
                        // force reflow so re-adding the class restarts the animation
                        // eslint-disable-next-line no-unused-expressions
                        void bar.offsetWidth;
                        bar.classList.add('absorb-pulse');
                        setTimeout(function() { bar.classList.remove('absorb-pulse'); }, 600);
                    }
                }, 90 + 600);

                // Fade out near end of flight, then remove.
                setTimeout(function() {
                    el.classList.remove('xp-float-enter');
                    el.classList.add('xp-float-leave');
                }, 90 + 680);
                setTimeout(function() { el.remove(); }, 90 + 1050);
        ...
        window.undoActivityById = async function(activityId) {
            ...
            for (let dimIndex = 0; dimIndex < (data.dimensions || []).length; dimIndex++) {
                ...
                for (let pathIndex = 0; pathIndex < (dim.paths || []).length; pathIndex++) {
                    ...
                    const actIndex = (path.activities || []).findIndex(a => a.id === activityId);
                    if (actIndex !== -1) {
                        // Negative bubble — flies FROM the progress bar TO
                        // the card, indicating XP is being withdrawn.
                        var _act = path.activities[actIndex];
                        var _xpPreview = _act.isNegative
                            ? (_act.negativeXP || _act.baseXP || 0)
                            : -(_act.baseXP || 0);
