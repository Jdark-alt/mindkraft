        ...
        // Uses completionHistory to check for hits — no anchor fields.
        // Returns { count, lastWindowDateStr } so the caller can advance the stamp.
        // Capped at MAX_SHIELDS+1 — we only need to know if missed > MAX_SHIELDS.
        function _missedWindowsSince(activity, fromDateStr) {
            const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
        ...
        // Returns the current streakShieldWindow, or derives it from lastCompleted
        // on first run (safe: all windows up to lastCompleted are already settled).
        ...
        // Returns the current skipPenaltyWindow, or derives it from lastCompleted
        // on first run.
        ...
        function calculateStreak(activity) {
            ...
            const { count: newMissed } = _missedWindowsSince(activity, shieldWin);
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            if (!isOccasional && liveStreak > 0) {
                const shieldWin = _getStreakShieldWindow(activity);
                if (shieldWin) {
                    const { count: liveMissed, lastWindowDateStr } = _missedWindowsSince(activity, shieldWin);
                    if (liveMissed > 0) {
        ...
        window.undoActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            if (prevUserEntry) {
                ...
                // Rewind window stamps to the previous completion's window
                const _prevWin = getCycleWindowStart(activity, new Date(prevUserEntry.date));
                if (_prevWin) {
                    const _prevWinStr = toLocalDateStr(_prevWin);
                    activity.streakShieldWindow = _prevWinStr;
                    activity.skipPenaltyWindow  = _prevWinStr;
                }
            } else {
        ...
        function processStreakShields(activity, today) {
            ...
            const { count: newMissed, lastWindowDateStr } = _missedWindowsSince(activity, shieldWin);
        ...
        function processSkipPenalty(activity, today) {
            ...
            const { count: newMissed, lastWindowDateStr } = _missedWindowsSince(activity, penaltyWin);
