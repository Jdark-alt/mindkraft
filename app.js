                ...
                if (user) {
                    ...
                    if (!window.userData.onboardingComplete &&
                        ...
                        if (ts === 0) {
                            ...
                            // Categorization nudge fires after unlock queue is
                            // empty (the popup chain calls nothing else, so
                            // this still fires regardless — see below).
                            setTimeout(() => checkCategorizationPrompt(), 1400);
                        }
                    }
                } else {
                    window.currentUser = null;
        ...
        const DEFAULT_ACTIVITY_SORT = 'grouped';
                    ...
                    <div class="act-ring-wrap">
                        ...
                        <div class="act-ring-label">
                            ${completedToday
                                ...
                                : `<span class="act-ring-xp-text">${xpText}</span>`
                            }
                        </div>
                    </div>
                </div>
        ...
        window.openCardTypePicker = function(activityId) {
            ...
            // Type labels and preview shapes (columns × rows)
            var types = [
                { t:1, label:'1×1', cols:1, rows:1 },
                { t:2, label:'Full',cols:3, rows:2 },
                { t:3, label:'2×1', cols:2, rows:1 },
                { t:4, label:'Sq',  cols:1, rows:1 },
                { t:5, label:'1×2', cols:1, rows:2 },
                { t:6, label:'2×2', cols:2, rows:2 },
            ];

            grid.innerHTML = types.map(function(tp) {
                var sel = tp.t === currentType ? ' selected' : '';
                return '<div class="ctp-card' + sel + '" data-type="' + tp.t + '" onclick="selectCardType(' + tp.t + ')">'
                    + '<div class="ctp-line"></div>'
                    + '<div class="ctp-line short"></div>'
                    + (tp.rows > 1 ? '<div class="ctp-line xshort"></div>' : '')
        ...
        // Build ring SVG for grid cards — size adapts to card type
        function _gcRing(completedToday, currentStreak, dimHex, xpText, type) {
            // r=18 for large rings, r=14 for type 3 (2×1 compact)
            var isCompact = (type === 3);
            var r = isCompact ? 14 : 18;
            var size = isCompact ? 34 : 44;
            var sw = isCompact ? 2.5 : 3;
            ...
            return '<div class="gc-ring-wrap">'
                ...
                + (completedToday
                    ...
                    : '<span class="gc-ring-xp">' + xpText + '</span>')
                + '</div></div>';
        }
        ...
        window._gcDragStart = function(e, activityId) {
            _dragSourceId = activityId;
            e.target.classList.add('gc-dragging');
            e.dataTransfer.effectAllowed = 'move';
            e.dataTransfer.setData('text/plain', activityId);
        };

        window._gcDragEnd = function(e) {
            e.target.classList.remove('gc-dragging');
            document.querySelectorAll('.gc-drag-over').forEach(function(el) { el.classList.remove('gc-drag-over'); });
            _dragSourceId = null;
        };

        window._gcDragOver = function(e) {
            e.preventDefault();
            e.dataTransfer.dropEffect = 'move';
            var card = e.currentTarget;
            if (card.dataset.aid !== _dragSourceId) card.classList.add('gc-drag-over');
        };

        window._gcDrop = function(e, targetId) {
            e.preventDefault();
            document.querySelectorAll('.gc-drag-over').forEach(function(el) { el.classList.remove('gc-drag-over'); });
            if (!_dragSourceId || _dragSourceId === targetId) return;

            // Reorder the underlying activity list in memory across all groups/ungrouped
            // Find the rendered order from the DOM then swap in userData
            var allCards = Array.from(document.querySelectorAll('.activity-grid .grid-card[data-aid]'));
            var srcIdx = allCards.findIndex(function(c) { return c.dataset.aid === _dragSourceId; });
            var tgtIdx = allCards.findIndex(function(c) { return c.dataset.aid === targetId; });
            if (srcIdx < 0 || tgtIdx < 0) return;

            // Build flat ordered list of ids, swap, then reorder userData dimensions
            var orderedIds = allCards.map(function(c) { return c.dataset.aid; });
            orderedIds.splice(srcIdx, 1);
            orderedIds.splice(tgtIdx, 0, _dragSourceId);

            // Rebuild userData activity order to match
            var allActs = [];
            (window.userData.dimensions || []).forEach(function(dim) {
                (dim.paths || []).forEach(function(path) {
                    (path.activities || []).forEach(function(act) {
                        allActs.push({ act: act, dim: dim, path: path });
                    });
                });
            });

            // Sort allActs by orderedIds position
            allActs.sort(function(a, b) {
                var ia = orderedIds.indexOf(a.act.id);
                var ib = orderedIds.indexOf(b.act.id);
                if (ia < 0) ia = 9999; if (ib < 0) ib = 9999;
                return ia - ib;
            });

            // Put them back into their paths in new order (within same path only — cross-path stays stable)
            // Simpler approach: just reorder within each path individually based on relative orderedIds order
            (window.userData.dimensions || []).forEach(function(dim) {
                (dim.paths || []).forEach(function(path) {
                    path.activities.sort(function(a, b) {
        ...
        function renderActivityGridCards(activities) {
            ...
            var actGroupMap = {};
            var groupOrder = [];
            Object.keys(window._gridGroups || {}).forEach(function(gid) {
                var g = window._gridGroups[gid];
                groupOrder.push(gid);
                (g.ids || []).forEach(function(aid) { actGroupMap[aid] = gid; });
            });

            var ungrouped = [];
            var byGroup = {};
            activities.forEach(function(a) {
                var gid = actGroupMap[a.id];
                if (gid) { if (!byGroup[gid]) byGroup[gid] = []; byGroup[gid].push(a); }
            ...
            function renderCard(activity) {
                ...
                // Ring: types 2,3,5,6 get ring
                var ringHtml = (type === 1 || type === 4) ? '' : _gcRing(completedToday, currentStreak, dimHex, xpText, type);

                // Undo button (for type 2 shown inline; others via overlay)
                var undoBtnHtml = (todayCount > 0 && !notScheduledToday)
                    ? '<button class="gc-undo" onclick="event.stopPropagation();undoActivityById(\'' + activity.id + '\')">↩ Undo</button>'
                    : '';

                // Type 2 (3×2): full showcase with all details
                var bodyContent;
                if (type === 2) {
                    var fullBadges = '';
                    fullBadges += '<span class="gc-badge gc-badge-counter" style="background:rgba(255,255,255,0.07);color:var(--color-text-secondary);">' + activity.dimensionName + ' › ' + activity.pathName + '</span>';
                    ...
                    bodyContent = '<div class="gc-name">' + escapeHtml(activity.name) + '</div>'
                        ...
                        + '<div class="gc-footer" style="margin-top:auto;">' + (isPinned ? '<span class="gc-pin">★</span>' : '') + undoBtnHtml + '</div>';
                } else {
                    bodyContent = '<div class="gc-name">' + escapeHtml(activity.name) + '</div>'
                        + '<div class="gc-xp-row">'
                        ...
                        + '</div>'
                        + (type !== 1 && type !== 4 ? '<div class="gc-path">' + activity.dimensionName + ' › ' + activity.pathName + '</div>' : '')
                        + (badges && type !== 1 && type !== 4 ? '<div class="gc-badges">' + badges + '</div>' : '')
                        + '<div class="gc-footer">' + (isPinned ? '<span class="gc-pin">★</span>' : '') + '</div>';
                }

                return '<div class="grid-card ' + stateClass + '" data-aid="' + activity.id + '" data-type="' + type + '" onclick="' + clickHandler + '"'
                    + ' draggable="true"'
                    + ' style="--dim-color:' + dimHex + ';--dim-rgba:rgba(' + dimRgb + ',0.18);"'
                    + ' ondragstart="_gcDragStart(event,\'' + activity.id + '\')"'
                    + ' ondragend="_gcDragEnd(event)"'
                    + ' ondragover="_gcDragOver(event)"'
                    + ' ondrop="_gcDrop(event,\'' + activity.id + '\')">'
                    + '<div class="gc-gradient"></div>'
                    + '<button class="gc-type-btn" onclick="event.stopPropagation();openCardTypePicker(\'' + activity.id + '\')" title="Change card size">⊞</button>'
                    + '<button class="gc-expand-btn" onclick="event.stopPropagation();openGridCardOverlay(\'' + activity.id + '\')" title="View details">⋯</button>'
            ...
            groupOrder.forEach(function(gid) {
                if (!byGroup[gid] || !byGroup[gid].length) return;
                var g = window._gridGroups[gid];
                html += '<div class="grid-group-section" style="--group-color:' + (g.color||'var(--color-accent-blue)') + '">'
                    + '<span class="grid-group-label">' + escapeHtml(g.name||'Group') + '</span>'
                    + '<div class="grid-group-inner">'
                    + byGroup[gid].map(renderCard).join('')
                    + '</div></div>';
            });
        ...
        window.openGridCardOverlay = function(activityId) {
            ...
            content.innerHTML =
                ...
                + '<button class="grid-overlay-btn" onclick="togglePinActivity(\'' + activityId + '\');closeGridCardOverlay();" style="padding:8px 14px;font-size:12px;">'
                + (isPinned ? '★ Unfavourite' : '☆ Favourite') + '</button>'
                + '</div>';
