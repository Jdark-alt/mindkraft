        ...
        function renderActivityCards(activities) {
            ...
            return activities.map(activity => {
                ...
                // Shield badge — live count including unaccounted misses since last login
                const shieldsUsed = isOccasional ? 0 : getShieldsUsedDisplay(activity);
                const shieldsLeft = Math.max(0, MAX_SHIELDS - shieldsUsed);
                // Show shield badge only when shields have been consumed on current streak
                // or the streak is at risk (0 shields left but streak still alive)
                let shieldBadge = '';
                if (currentStreak > 0 && shieldsUsed > 0) {
                    const allGone = shieldsLeft === 0;
                    shieldBadge = allGone
                        ? `<span class="activity-badge badge-shield-warn" title="All 3 shields used — next miss breaks your streak!">🛡 ⚠ 0 left</span>`
                        : `<span class="activity-badge badge-shield" title="${shieldsUsed}/${MAX_SHIELDS} shields used — ${shieldsLeft} remaining">🛡 ${shieldsUsed}</span>`;
                }
        ...
        //
        // DESIGN:
        //   Each activity has streakPauseUses (0-3). Every closed cycle window the
        //   user missed costs 1 shield. At 0 shields remaining the streak resets.
        //   Shields reset to 0 (full) whenever the streak resets, so every new
        //   streak starts with 3 fresh shields.
        //
        //   Shields are evaluated at login (processStreakShields, for ALL activities)
        //   using the same window-walking logic as processSkipPenalty.
        //   consumeStreakShields() is GONE — no longer called during completeActivity().
        //
        // DISPLAY:
        //   Shield count is shown inline on the activity card next to the streak badge.
        //   e.g.  🔥 12   🛡 1   means streak=12, shields_used=1 (2 remaining).
        //   Only shown when at least 1 shield has been used.
        //   When streak > 0 and shields all used (3/3): card shows 🛡 ⚠ 0 left warning.

        const MAX_SHIELDS = 3;

        // ── countMissedWindowsSince ──────────────────────────────────────
        // Pure function. Counts closed cycle windows after `lastAccountedWindow`
        // that had no user completion. Stops at today's open window.
        // Returns { missed, lastExamined, capped } where:
        //   missed       — number of missed closed windows found (max 3 for shield logic)
        //   lastExamined — the last window start Date we walked
        //   capped       — true if we hit the safety limit (do NOT use for streak decisions)
        //
        // SAFETY CAP: We stop at 4 misses. Shield logic only needs to know if misses
        // exceed MAX_SHIELDS (3). Counting beyond 4 serves no purpose and risks
        // misreading pruned completionHistory entries as false misses.
        function countMissedWindowsSince(activity, lastAccountedWindow) {
            const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
            const todayCycleStart = getCycleWindowStart(activity, todayMidnight);
            if (!todayCycleStart) return { missed: 0, lastExamined: lastAccountedWindow, capped: false };
            const todayCycleStartMs = todayCycleStart.getTime();

            let cursor = getNextCycleWindowStart(activity, lastAccountedWindow);
            if (!cursor || isNaN(cursor.getTime())) return { missed: 0, lastExamined: lastAccountedWindow, capped: false };

            let missed = 0;
            let lastExamined = lastAccountedWindow;
            const MISS_CAP = MAX_SHIELDS + 1; // only need to know if misses > MAX_SHIELDS

            while (cursor.getTime() < todayCycleStartMs) {
                const nextCursor = getNextCycleWindowStart(activity, cursor);
                if (!nextCursor || isNaN(nextCursor.getTime())) break;
                if (!wasCompletedInWindow(activity, cursor, nextCursor)) missed++;
                lastExamined = cursor;
                if (missed >= MISS_CAP) return { missed, lastExamined, capped: true };
                cursor = nextCursor;
            }
            return { missed, lastExamined, capped: false };
        }

        // ── getLastAccountedWindow ────────────────────────────────────────
        // Derives the starting anchor for the missed-window walk.
        //
        // SAFETY RULE: On first migration (no lastAccountedWindow stored), we anchor
        // to the window containing lastCompleted — NOT the earliest completionHistory
        // entry. Walking back through old history is dangerous because completionHistory
        // is pruned to 365 entries, so old windows appear as "missed" even when they
        // were completed. This would incorrectly burn shields or reset streaks on deploy.
        //
        // The result: on first run, countMissedWindowsSince sees only windows AFTER
        // the most recent completion — i.e. only genuinely missed recent windows.
        function getLastAccountedWindow(activity) {
            if (activity.lastAccountedWindow) {
                const d = new Date(activity.lastAccountedWindow + 'T00:00:00');
                if (!isNaN(d.getTime())) return d;
            }
            // Migration from old skip-penalty system (skip-negative only)
            if (activity.lastSkipCheckDate) {
                const checkDateWindow = getCycleWindowStart(activity,
                    new Date(activity.lastSkipCheckDate + 'T00:00:00'));
                if (checkDateWindow) {
                    const prev = getCycleWindowStart(activity, new Date(checkDateWindow.getTime() - 1));
                    if (prev) return prev;
                }
            }
            // First run: anchor to the window containing the most recent completion.
            // Never anchor to createdAt — that causes deep walks through pruned history.
            if (activity.lastCompleted) {
                return getCycleWindowStart(activity, new Date(activity.lastCompleted));
            }
            return null;
        }

        // ── calculateStreak ──────────────────────────────────────────────
        // READ-ONLY. Returns the live effective streak for display.
        // Uses the stored streak value but checks if unaccounted missed windows
        // would break it given remaining shields.
        function calculateStreak(activity) {
            ...
            if (activity.isNegative && !activity.isSkipNegative) {
                // Perform-negative: no shields, break on any miss
                ...
                return daysDiff <= graceDays ? (activity.streak || 0) : 0;
            }

            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin) return activity.streak || 0;
            const { missed } = countMissedWindowsSince(activity, lastWin);
            const usedShields = activity.streakPauseUses || 0;
            const shieldsLeft = Math.max(0, MAX_SHIELDS - usedShields);
            if (missed === 0) return activity.streak || 0;
            if (missed <= shieldsLeft) return activity.streak || 0; // shields cover it
            return 0; // streak broken
        }

        // ── getShieldsUsedDisplay ─────────────────────────────────────────
        // Returns shields already consumed PLUS any new misses not yet persisted
        // (i.e. missed since lastAccountedWindow that haven't been saved to DB yet).
        // Used purely for rendering the shield badge on activity cards in real-time.
        function getShieldsUsedDisplay(activity) {
            if (activity.frequency === 'occasional') return 0;
            if (activity.isNegative && !activity.isSkipNegative) return 0;
            const stored = activity.streakPauseUses || 0;
            // Add any new unaccounted misses (what processStreakShields would add on next login)
            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin) return stored;
            const { missed } = countMissedWindowsSince(activity, lastWin);
            if (missed === 0) return stored;
            // Clamp: can't show more than MAX_SHIELDS, and past MAX the streak breaks
            return Math.min(MAX_SHIELDS, stored + missed);
        }
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // Calculate the live streak BEFORE writing lastCompleted.
            // If the streak is currently broken (calculateStreak returns 0 but stored > 0),
            // we reset the stored value so the new streak starts from 1.
            const liveStreak = isOccasional ? 0 : calculateStreak(activity);
            if (!isOccasional && liveStreak === 0 && (activity.streak || 0) > 0) {
                // Streak was broken — reset stored fields so new streak starts clean
                activity.streak = 0;
            ...
            // Stamp lastAccountedWindow to the current cycle window for ALL streak-eligible
            // activities on every completion. This is the authoritative "settled up to here"
            // marker — it prevents calculateStreak and processStreakShields from re-walking
            // old windows (which may have been pruned from completionHistory) on future renders.
            if (!isOccasional) {
                const _winStart = getCycleWindowStart(activity, new Date());
                if (_winStart) activity.lastAccountedWindow = toLocalDateStr(_winStart);
            }
        ...
        // Called at login for EVERY streak-eligible activity (not just skip-negative).
        // Walks closed windows since lastAccountedWindow, consumes shields for each
        // missed window, and resets the streak if shields are exhausted.
        // Returns true if activity data changed (caller must save).
        ...
        // ── inferStreakFromHistory ─────────────────────────────────────────
        // Counts consecutive completed cycle windows going backwards from
        // lastCompleted. Used as a fallback when bestStreak is missing (pre-dates
        // the bestStreak field) and the bad deploy zeroed the stored streak.
        // Returns the inferred streak count (0 if nothing recoverable).
        function inferStreakFromHistory(activity) {
            if (!activity.completionHistory || activity.completionHistory.length === 0) return 0;
            if (!activity.lastCompleted) return 0;

            // Build a Set of local date strings that had a positive completion
            const completedDates = new Set();
            activity.completionHistory.forEach(e => {
                if (e.isPenalty || (e.xp || 0) <= 0) return;
                completedDates.add(toLocalDateStr(new Date(e.date)));
            });

            // Walk backwards from lastCompleted's window, counting consecutive hits
            let count = 0;
            let cursor = getCycleWindowStart(activity, new Date(activity.lastCompleted));
            if (!cursor) return 0;

            for (let i = 0; i < 400; i++) { // 400 = max streak we'd ever recover
                const nextCursor = getNextCycleWindowStart(activity, cursor);
                if (!nextCursor) break;
                // Check if any completion date falls in [cursor, nextCursor)
                const windowHit = activity.completionHistory.some(e => {
                    if (e.isPenalty || (e.xp || 0) <= 0) return false;
                    const t = new Date(e.date).getTime();
                    return t >= cursor.getTime() && t < nextCursor.getTime();
                });
                if (!windowHit) break; // gap found — stop
                count++;
                // Step backwards one window
                const prevMs = cursor.getTime() - 1;
                cursor = getCycleWindowStart(activity, new Date(prevMs));
                if (!cursor) break;
            }
            return count;
        }

            // ── Streak recovery: runs BEFORE the idempotency check ───────────────
            // The bad deploy stamped lastShieldCheckDate=today while also zeroing
            // streaks. We must recover regardless of whether we already ran today.
            // Guard: only attempt recovery once (streakRecoveredDate prevents loops).
            if (
                (activity.streak || 0) === 0 &&
                activity.lastCompleted &&
                activity.streakRecoveredDate !== today
            ) {
                const lastMidnight = new Date(activity.lastCompleted); lastMidnight.setHours(0,0,0,0);
                const todayMidnight = new Date(); todayMidnight.setHours(0,0,0,0);
                const daysDiff = Math.round((todayMidnight - lastMidnight) / (1000 * 60 * 60 * 24));
                ...
                if (daysDiff <= graceDays + 1) {
                    // Use bestStreak if available; otherwise infer from completionHistory.
                    // bestStreak may be missing on activities predating that field.
                    const recoveredStreak = (activity.bestStreak || 0) > 0
                        ? activity.bestStreak
                        : inferStreakFromHistory(activity);
                    activity.streakRecoveredDate = today; // always stamp to prevent re-running
                    if (recoveredStreak > 0) {
                        activity.streak = recoveredStreak;
                        if (!activity.bestStreak || activity.bestStreak < recoveredStreak) {
                            activity.bestStreak = recoveredStreak;
                        }
                        activity.streakPauseUses = 0;
                        const restoredWin = getCycleWindowStart(activity, new Date(activity.lastCompleted));
                        if (restoredWin) activity.lastAccountedWindow = toLocalDateStr(restoredWin);
                    }
                    // Don't return — fall through to stamp lastShieldCheckDate
                }
            }

            // Idempotent within same calendar day
            if (activity.lastShieldCheckDate === today) return false;
            activity.lastShieldCheckDate = today;

            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin || !activity.lastCompleted) {
                // No completed history — stamp today's window and exit cleanly.
                // Never penalise activities that haven't been completed yet.
                const todayWin = getCycleWindowStart(activity, new Date());
                if (todayWin) activity.lastAccountedWindow = toLocalDateStr(todayWin);
                return true;
            }

            const { missed, lastExamined, capped } = countMissedWindowsSince(activity, lastWin);

            let advanced = false;
            if (lastExamined && lastExamined.getTime() > lastWin.getTime()) {
                activity.lastAccountedWindow = toLocalDateStr(lastExamined);
                advanced = true;
            }

            if (missed === 0) return advanced;

            const usedShields = activity.streakPauseUses || 0;
            const available = Math.max(0, MAX_SHIELDS - usedShields);

            if (missed <= available) {
                activity.streakPauseUses = usedShields + missed;
            } else {
                activity.streak = 0;
                activity.streakGrantedDate = null;
                activity.streakPauseUses = 0;
        ...
        // processStreakPauses — called at login for all activities.
        // Also handles XP penalties for skip-negative activities.
        async function processStreakPauses() {
            ...
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        // Shield check for ALL streak-eligible activities
                        if (processStreakShields(act, today)) anyChanged = true;
                        // XP penalty only for skip-negative activities
        ...
        // ── processSkipPenalty ────────────────────────────────────────────
        // Applies XP penalties for skip-negative activities on missed windows.
        // Shield management is now handled entirely by processStreakShields() above.
        // Returns true if XP data changed (caller must save).
        function processSkipPenalty(activity, today) {
            if (!activity.isSkipNegative) return false;
            // Idempotent — already ran today
            ...
            const lastWin = getLastAccountedWindow(activity);
            if (!lastWin || isNaN(lastWin.getTime())) return false;

            const { missed } = countMissedWindowsSince(activity, lastWin);
            if (missed === 0) return false;

            const missedCapped = Math.min(7, missed);
            activity.skipStreak = (activity.skipStreak || 0) + missedCapped;

            // XP penalty
            const totalPenalty = (activity.baseXP || 10) * missedCapped;
