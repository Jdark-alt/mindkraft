        ...
        window.switchTab = function(tabName) {
            // ── Tutorial spotlight hook ─────────────────────────────────────
            if (window._tutorialPhase === 'tap' && window._tutorialWaitingForTab === tabName) {
                const step = window.userData ? (window.userData.tutorialStep ?? 1) : 1;
                _enterReadPhase(step);
            }
        ...
        //    1 = spotlight Friends tab
        //    2 = spotlight Challenges tab
        //    3 = spotlight Analytics tab
        //    4 = complete
        ...
        const TUTORIAL_STEPS = [
            ...
            },
            {
                tab: 'friends',
                label: 'Friends',
                tapHint: 'Tap Friends to take a look inside →',
                emoji: '👥',
                title: 'Friends & Leaderboard',
                body: 'Add friends with a friend code, compare XP on the leaderboard, and keep each other accountable. Healthy competition = better habits.'
            },
            {
                tab: 'challenges',
                label: 'Challenges',
                tapHint: 'Tap Challenges to explore →',
                emoji: '🏆',
                title: 'Challenges',
                body: 'Set personal or group challenges tied to your activities — with a deadline and a bonus XP reward when you hit the goal.'
            },
            {
                tab: 'analytics',
                label: 'Analytics',
                tapHint: 'Tap Analytics to see it →',
                emoji: '📊',
                title: 'Analytics',
        ...
        window._tutorialPhase        = null; // 'tap' | 'read' | 'confirm'
        window._tutorialWaitingForTab = null;
        window._tutorialReadTimer    = null;

        function getTutorialElements() {
            return {
                overlay: document.getElementById('tutorialOverlay'),
                lens:    document.getElementById('spotlightLens'),
                tooltip: document.getElementById('tutorialTooltip'),
                card:    document.getElementById('tutorialCard')
            };
        }

        function positionLensOnTab(tabName) {
            const btn = Array.from(document.querySelectorAll('.nav-tab')).find(
                b => b.getAttribute('onclick') === `switchTab('${tabName}')`
            );
            const lens = document.getElementById('spotlightLens');
            if (!btn || !lens) return;
            const r = btn.getBoundingClientRect();
            const pad = 6;
            lens.style.top    = (r.top    - pad) + 'px';
            lens.style.left   = (r.left   - pad) + 'px';
            lens.style.width  = (r.width  + pad * 2) + 'px';
            lens.style.height = (r.height + pad * 2) + 'px';
        }

        function getTooltipBottom() {
            const nav = document.querySelector('.nav-tabs');
            if (!nav) return '90px';
            const fixed = getComputedStyle(nav).position === 'fixed';
            return fixed ? (nav.offsetHeight + 14) + 'px' : '24px';
        }

        function _enterTapPhase(step) {
            const s = TUTORIAL_STEPS[step];
            const { overlay, lens, tooltip, card } = getTutorialElements();
            if (!overlay) return;

            window._tutorialPhase        = 'tap';
            window._tutorialWaitingForTab = s.tab;
            ...
            positionLensOnTab(s.tab);

            const bot = getTooltipBottom();
            tooltip.style.display  = 'block';
            tooltip.style.bottom   = bot;
            tooltip.style.top      = '';
            tooltip.innerHTML = `
                <div class="tut-tap-hint">
                    <span class="tut-tap-arrow">👆</span>
                    <span>${s.tapHint}</span>
                </div>`;
        }

        function _enterReadPhase(step) {
            const { overlay, lens, tooltip } = getTutorialElements();
            window._tutorialPhase        = 'read';
            window._tutorialWaitingForTab = null;
            ...
            if (tooltip) {
                tooltip.style.display = 'block';
                tooltip.style.bottom  = getTooltipBottom();
                tooltip.style.top     = '';
                tooltip.innerHTML = `<div class="tut-read-badge">Explore freely ✨</div>`;
            }

            clearTimeout(window._tutorialReadTimer);
            window._tutorialReadTimer = setTimeout(() => _enterConfirmPhase(step), 2500);
        }

        function _enterConfirmPhase(step) {
            const s = TUTORIAL_STEPS[step];
            const { overlay, lens, tooltip, card } = getTutorialElements();
            if (!overlay) return;

            window._tutorialPhase = 'confirm';
            ...
            card.style.cssText = `
                ...
                position: fixed; bottom: ${bot}; left: 16px; right: 16px;
                z-index: 8960; pointer-events: all;
                background: var(--color-bg-card);
            ...
            card.innerHTML = `
                ...
                <button class="tutorial-cta" onclick="advanceTutorial()">Next →</button>
                <button class="tutorial-skip" onclick="skipTutorial()">Skip tour</button>
            `;
        }

        window.initTutorial = function() {
            if ((window.userData.tutorialStep ?? -1) >= 0) return;
        ...
        window.showCurrentTutorialStep = function() {
            ...
            if (!s.tab) {
                // Step 0: centered card (no spotlight, user needs to create an activity)
                const { overlay, lens, tooltip, card } = getTutorialElements();
                if (!overlay) return;
                if (lens)    lens.style.display    = 'none';
                if (tooltip) tooltip.style.display = 'none';
            ...
            // Spotlight steps: switch to the tab so user sees it, then enter tap phase
            switchTab(s.tab);
            setTimeout(() => _enterTapPhase(step), 400);
        };

        window.hideTutorialOverlay = function() {
            const { overlay, lens, tooltip, card } = getTutorialElements();
            if (overlay) { overlay.style.display = 'none'; overlay.style.background = 'transparent'; overlay.style.padding = ''; }
            if (lens)    lens.style.display    = 'none';
            if (tooltip) tooltip.style.display = 'none';
            if (card)    card.style.display    = 'none';
            window._tutorialPhase        = null;
            window._tutorialWaitingForTab = null;
            clearTimeout(window._tutorialReadTimer);
            document.body.style.overflow = '';
        };
        ...
        window.skipTutorial = async function() {
            window.userData.tutorialStep = TUTORIAL_STEPS.length;
            await saveUserData().catch(() => {});
            hideTutorialOverlay();
            switchTab('activities');
            showToast('Tour skipped — you\'re good to go!', 'blue');
        };
