        ...
        window.openCardTypePicker = function(activityId) {
            ...
            if (!el || !grid) return;
            var currentType = (function(t){
                if (t === 1 || t === 2 || t === 3 || t === 6) return t;
                if (t === 7 || t === 5) return 3;
                if (t === 4) return 6;
                return 1;
            })(window._gridCardTypes[activityId] || 1);

            // Simplified to 4 sizes (down from 7 in earlier versions).
            // Earned uniqueness comes from data, not user-picked shapes —
            // these four cover the meaningful range: minimum, wide, square, hero.
            var types = [
        ...
        function renderActivityGridCards(activities) {
            ...
            // Supported card sizes: 1 (Small), 3 (Wide), 6 (Square), 2 (Showcase).
            // Older saved types (4 Column, 5 Tall, 7 Row) coerce to the nearest
            // supported size on render — "smaller wins" per design brief §3.
            function _normalizeCardType(t) {
                if (t === 1 || t === 2 || t === 3 || t === 6) return t;
                if (t === 7) return 3;   // 3×1 Row → 2×1 Wide
                if (t === 5) return 3;   // 1×2 Tall → 2×1 Wide
                if (t === 4) return 6;   // 1×3 Column → 2×2 Square
                return 1;                // unknown → 1×1 Small
            }

            function renderCard(activity) {
                var type = _normalizeCardType(window._gridCardTypes[activity.id] || 1);
        ...
        function renderPlanner() {
            ...
            if (merged.length === 0) {
                // Onboarding-empty pattern per design brief §3:
                // CTA + sub-copy explaining the first action.
                var isToday = dateStr === todayStr;
                var emptyTitle = isToday ? 'Plan your day' : 'Nothing scheduled';
                container.innerHTML = '<div class="planner-empty">'
                    ...
                    + '<button class="planner-empty-cta" onclick="openPlannerAddModal()">+ Add to schedule</button>'
                    + '</div>';
                return;
            }
        ...
        window.togglePlannerInline = function() {
            ...
            // Activity-add and count live in the toolbar — they're irrelevant
            // when the planner is open (the planner has its own +Add to schedule).
            var actAddBtn  = document.querySelector('.act-tb-add-main');
            var actCountEl = document.getElementById('activitySlotCount');
            ...
            if (!isOpen) {
                ...
                if (actAddBtn)  actAddBtn.style.display  = 'none';
                if (actCountEl) actCountEl.style.display = 'none';
                if (typeof renderPlanner === 'function') renderPlanner();
            } else {
                plannerEl.style.display = 'none';
                listEl.style.display = 'block';
                if (btnEl) btnEl.classList.remove('active');
                if (actAddBtn)  actAddBtn.style.display  = '';
                if (actCountEl) actCountEl.style.display = '';
            }
        };
