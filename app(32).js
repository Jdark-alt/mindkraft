        ...
        // Visible on-screen diagnostic for tracing tap events (no devtools needed)
        window._diag = function(msg) {
            var el = document.getElementById('diagBanner');
            if (!el) {
                el = document.createElement('div');
                el.id = 'diagBanner';
                el.style.cssText = 'position:fixed;top:60px;left:8px;right:8px;background:#222;color:#fff;font-family:monospace;font-size:11px;padding:6px 10px;border-radius:6px;z-index:99999;white-space:pre-wrap;max-height:120px;overflow:auto;border:1px solid #4a7c9e;';
                document.body.appendChild(el);
            }
            var t = new Date().toLocaleTimeString().slice(-5);
        ...
        window.openGridActionMenu = function(activityId) {
            _diag('openGridActionMenu(' + activityId.slice(0,6) + ')');
            ...
            // Bind handlers programmatically rather than inline to avoid any
            // string-parsing pitfalls.
            el.innerHTML =
                ...
                + '</div>';
            // Stop propagation at the inner wrapper so backdrop-click doesn't fire.
            ...
            document.getElementById('gcaSize').addEventListener('click', function(e) {
                e.stopPropagation();
                _diag('TAP gcaSize → opening picker for ' + activityId.slice(0,6));
                closeGridActionMenu();
                setTimeout(function() {
                    _diag('calling openCardTypePicker');
                    openCardTypePicker(activityId);
                    _diag('openCardTypePicker returned');
                }, 60);
            });
            document.getElementById('gcaDetails').addEventListener('click', function(e) {
                e.stopPropagation();
                _diag('TAP gcaDetails → opening overlay for ' + activityId.slice(0,6));
                closeGridActionMenu();
                setTimeout(function() {
                    _diag('calling openGridCardOverlay');
                    openGridCardOverlay(activityId);
                    _diag('openGridCardOverlay returned');
                }, 60);
            });
        ...
        window.openCardTypePicker = function(activityId) {
            if (window._diag) _diag('openCardTypePicker start id=' + activityId.slice(0,6));
            ...
            if (!el || !grid) { if (window._diag) _diag('PICKER ABORT: el=' + !!el + ' grid=' + !!grid); return; }
            ...
            el.style.display = 'flex';
            if (window._diag) _diag('PICKER el.style.display=flex set; computed=' + getComputedStyle(el).display);
        };
        ...
        function renderActivityGridCards(activities) {
            ...
            function renderCard(activity) {
                ...
                // Ring: types 2,3,4,5,6 get ring; only type 1 (compact square) has no ring
                ...
                if (type === 2) {
                    ...
                    bodyContent = '<div class="gc-name">' + escapeHtml(activity.name) + '</div>'
                        ...
                        + '<div class="gc-footer" style="margin-top:auto;">' + undoBtnHtml + '</div>';
                } else {
                    bodyContent = '<div class="gc-name">' + escapeHtml(activity.name) + '</div>'
                        + '<div class="gc-xp-row">'
                        ...
                        + '</div>'
                        + (badges && type !== 1 && type !== 4 && type !== 3 ? '<div class="gc-badges">' + badges + '</div>' : '')
        ...
        window.openGridCardOverlay = function(activityId) {
            if (window._diag) _diag('openGridCardOverlay start id=' + activityId.slice(0,6));
            ...
            if (window._diag) _diag('OVERLAY display=flex set; computed=' + getComputedStyle(overlay).display);
        };
