        ...
        // NOTE: consumeStreakShields() (write path) explicitly sets activity.streak = 0
        // when a streak breaks, so we can trust the stored value after a completion.
        function calculateStreak(activity) {
            ...
            if (missedCycles === 0)          return activity.streak || 0;
            if (missedCycles <= shieldsLeft) return activity.streak || 0; // shields cover it
            return 0;
        }
        ...
        // Returns 'protected' | 'broken' | 'ok' for caller awareness.
        function consumeStreakShields(activity) {
            if (!activity.lastCompleted) return 'ok';
            if (activity.frequency === 'occasional') return 'ok';
            if (activity.isNegative && !activity.isSkipNegative) return 'ok';
            ...
            const graceDays = getStreakGraceDays(activity);
            const missedCycles = daysDiff <= graceDays ? 0 : Math.ceil((daysDiff - graceDays) / graceDays);
            if (missedCycles === 0) return 'ok';

            const usedShields = activity.streakPauseUses || 0;
            const shieldsLeft = Math.max(0, 3 - usedShields);

            if (missedCycles <= shieldsLeft) {
                // Shields absorbed the missed cycles — streak survives
                activity.streakPauseUses = usedShields + missedCycles;
                activity.streakPaused = false;
                return 'protected';
            } else {
                // More misses than shields — streak is broken.
                // Write streak = 0 so stored value matches what calculateStreak() returns,
                // then reset shields so the brand-new streak starts with a full set.
                activity.streak = 0;
                activity.streakGrantedDate = null; // allow streak to re-start today
        ...
        window.completeActivity = async function(dimIndex, pathIndex, actIndex) {
            ...
            // For skip-negative activities, stamp lastAccountedWindow to the current
            // cycle window start. This tells processSkipPenalty "this window is settled —
            // start checking from the next one". Uses the same field name as the penalty
            // system so both reads and writes are consistent.
            if (activity.isSkipNegative && !isOccasional) {
                const _winStart = getCycleWindowStart(activity, new Date());
                if (_winStart) {
                    activity.lastAccountedWindow = toLocalDateStr(_winStart);
                }
            }
        ...
        window.renderStreakPauseForSelected = function(allOverride) {
            ...
            // Always look up the LIVE activity object from userData so shield counts
            // are current — spread copies in allOverride can be stale after a completion.
            let liveAct = null;
            (window.userData.dimensions || []).forEach(dim =>
                (dim.paths || []).forEach(path =>
                    (path.activities || []).forEach(act => {
                        if (act.id === selectedId) liveAct = act;
                    })));

            // For dimName/pathName we still need the allOverride or a fresh scan
            let dimName = '', pathName = '';
            ...
            const metaAct = all.find(a => a.id === selectedId);
            if (metaAct) { dimName = metaAct.dimName; pathName = metaAct.pathName; }
            if (!liveAct) { el.innerHTML = ''; return; }

            const uses     = liveAct.streakPauseUses || 0;
            const maxU     = 3;
            const isPaused = liveAct.streakPaused;
            // Compute live streak (read-only, matches what the activity card shows)
            const liveStreak = calculateStreak(liveAct);
            const streakBroke = liveStreak === 0 && (liveAct.streak === 0) && uses === 0 && !!liveAct.lastCompleted;

            const dotsHtml = Array.from({length: maxU}, (_, i) =>
                `<div class="streak-use-dot ${i < uses ? 'used' : ''}"></div>`).join('');

            const freqLabel = { daily: 'day', weekly: 'week', biweekly: '2wk', monthly: 'mo', custom: 'cycle' };
            const unit = freqLabel[liveAct.frequency] || 'cycle';

            let actionHtml;
            if (isPaused) {
                actionHtml = `<span class="streak-paused-badge">🛡 Protected</span>`;
            } else if (uses >= maxU) {
                actionHtml = `<button class="btn-pause-streak" disabled title="No shields left — all 3 used">No shields left</button>`;
            } else {
                actionHtml = `<button class="btn-pause-streak" onclick="pauseStreak('${liveAct.id}')">🛡 Use Shield</button>`;
            }

            let statusNote;
            if (liveStreak > 0) {
                statusNote = `<span style="color:var(--color-accent-olive);font-weight:600;">🔥 Active — ${liveStreak} ${unit} streak</span>`;
            } else if (uses === 0 && !liveAct.lastCompleted) {
                statusNote = `<span style="color:var(--color-text-secondary);">No streak yet</span>`;
            } else {
                statusNote = `<span style="color:var(--color-accent-red);font-weight:600;">Streak reset — start a new one!</span>`;
            }

            el.innerHTML = `
            <div class="streak-pause-item">
                <div class="streak-pause-info">
                    <div class="streak-pause-name">${escapeHtml(liveAct.name)}</div>
                    <div class="streak-pause-meta">${escapeHtml(dimName)} › ${escapeHtml(pathName)}</div>
                    <div style="margin-top:4px;">${statusNote}</div>
                </div>
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px;flex-shrink:0;">
                    <div class="streak-pause-uses" title="${uses}/${maxU} shields used this streak">${dotsHtml}</div>
                    ${actionHtml}
                </div>
            </div>
            <p class="settings-note" style="margin-top:10px;">
                ${uses}/${maxU} shields used this streak.
                ${uses > 0 && liveStreak > 0 ? ` ${maxU - uses} remaining — streak breaks on the ${maxU - uses + 1}${['st','nd','rd'][maxU-uses] || 'th'} miss.` : ''}
                ${liveStreak === 0 ? ' Shields fully reset when a streak breaks — you start fresh.' : ''}
            </p>`;
        };
        ...
        async function processStreakPauses() {
            // Use local date, not UTC — in east-of-UTC timezones, toISOString()
            // returns the next calendar day after ~18:30-23:59 local time.
        ...
        //
        // DESIGN PRINCIPLE:
        //   Tracks one field: lastAccountedWindow — the ISO-date start of the last
        //   cycle window that has been fully settled, meaning either:
        //     (a) the user completed the activity inside it, OR
        //     (b) a penalty was already applied for missing it.
        //
        //   On every login, scan windows AFTER lastAccountedWindow up to (not including)
        //   today's open window. Each unsettled missed window = 1 penalty cycle.
        //   Advance lastAccountedWindow to the furthest window examined.
        //
        //   This guarantees NO double-penalisation across app versions, patch deploys,
        //   or multi-day offline gaps.
        //
        // MIGRATION (no lastAccountedWindow yet):
        //   1. lastSkipCheckDate  — old code wrote this every login. The window
        //      containing it was the last day processed. Start AFTER that window.
        //   2. lastCompleted      — derive last-completed window, start after it.
        //   3. createdAt          — one free window, then penalties begin.
        //   4. Nothing            — stamp and exit safely.
        //
        // Called by processStreakPauses() on every login.
        ...
        function processSkipPenalty(activity, today) {
            ...
            // ── Determine lastAccountedWindow ──────────────────────────────────
            let lastAccountedWindow = null;

            if (activity.lastAccountedWindow) {
                // Already tracked by this system — most common path after first run.
                lastAccountedWindow = new Date(activity.lastAccountedWindow + 'T00:00:00');

            } else if (activity.lastSkipCheckDate) {
                // Migration: old code stamped lastSkipCheckDate = the calendar day it ran.
                // On that day, the window CONTAINING lastSkipCheckDate was still OPEN —
                // so it was neither penalised nor completed. We must NOT mark it as
                // accounted-for, or it will be silently skipped after it closes.
                // Instead, anchor to the window BEFORE the check-date window, so the
                // check-date window itself will be evaluated on the next login.
                const checkDateWindow = getCycleWindowStart(activity,
                    new Date(activity.lastSkipCheckDate + 'T00:00:00'));
                // Go one window back from the check-date window
                // (checkDateWindow is the window that was open on lastSkipCheckDate;
                //  we anchor to the one before it so it gets evaluated next login)
                const checkDateMs = checkDateWindow.getTime();
                // Find the previous window by subtracting 1ms and deriving its start
                const prevWindowEnd = new Date(checkDateMs - 1);
                lastAccountedWindow = getCycleWindowStart(activity, prevWindowEnd);

            } else if (activity.completionHistory && activity.completionHistory.length > 0) {
                // No prior check date: anchor from the EARLIEST positive completion.
                // Using lastCompleted (latest) would skip over missed windows that
                // occurred between an early completion and a later one.
                const earliest = activity.completionHistory
                    .filter(e => (e.xp || 0) > 0 && !e.isPenalty)
                    .map(e => new Date(e.date).getTime())
                    .reduce((min, t) => Math.min(min, t), Infinity);
                if (earliest !== Infinity) {
                    lastAccountedWindow = getCycleWindowStart(activity, new Date(earliest));
                }
            }
            if (!lastAccountedWindow && activity.lastCompleted) {
                // Legacy fallback: no completionHistory — use lastCompleted.
                lastAccountedWindow = getCycleWindowStart(activity,
                    new Date(activity.lastCompleted));
            }
            if (!lastAccountedWindow && activity.createdAt) {
                // Never completed: give one free window (the creation window),
                // then penalties start from the next window onward.
                lastAccountedWindow = getCycleWindowStart(activity,
                    new Date(activity.createdAt));
            }
            if (!lastAccountedWindow) {
                activity.lastSkipCheckDate = today;
                return false;
            }

            if (!lastAccountedWindow || isNaN(lastAccountedWindow.getTime())) {
                activity.lastSkipCheckDate = today;
                return false;
            }

            // Stamp today now so re-entry within the same day is blocked
            activity.lastSkipCheckDate = today;

            // The window containing today is still open — never penalise it
            const todayCycleStart = getCycleWindowStart(activity, todayMidnight);
            if (!todayCycleStart) return false;
            const todayCycleStartMs = todayCycleStart.getTime();

            // ── Walk every closed window after lastAccountedWindow ─────────────
            let cursor = getNextCycleWindowStart(activity, lastAccountedWindow);
            if (!cursor || isNaN(cursor.getTime())) return false;

            let missedCycles = 0;
            let lastExaminedWindow = lastAccountedWindow;
            ...
            while (cursor.getTime() < todayCycleStartMs) {
                ...
                if (!wasCompletedInWindow(activity, cursor, nextCursor)) {
                    missedCycles++;
                }

                lastExaminedWindow = cursor;

                if (missedCycles >= 7) break; // safety cap
                cursor = nextCursor;
            }

            // Advance lastAccountedWindow regardless of penalty outcome.
            // This is what prevents double-counting on the next login.
            // IMPORTANT: use date-value comparison, not reference comparison.
            // lastExaminedWindow is always a new Date object so !== would always be true.
            let windowAdvanced = false;
            const lastExaminedMs = lastExaminedWindow.getTime();
            const lastAccountedMs = lastAccountedWindow.getTime();
            if (lastExaminedMs > lastAccountedMs) {
                activity.lastAccountedWindow = toLocalDateStr(lastExaminedWindow);
                windowAdvanced = true;
            }

            if (missedCycles === 0) {
                // Still return true if we advanced lastAccountedWindow, so the caller
                // saves the updated field to Firestore. Without this, the next login
                // re-derives the anchor from scratch and may double-penalise.
            ...
            if (missedCycles <= availableShields) {
                // Shields absorb all misses — streak survives, mark dots used
                activity.streakPauseUses = Math.min(MAX_SHIELDS, usedShields + shieldsToConsume);
            } else {
                // More misses than available shields — streak breaks.
                // Store streak = 0 so the card shows the correct value immediately.
                // Reset shield count so the brand-new streak starts with a full 3.
                activity.streak = 0;
                activity.streakGrantedDate = null;
                activity.streakPauseUses = 0;
                activity.streakPaused = false;
