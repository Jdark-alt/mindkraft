        ...
        async function buildLevelUpCard(newLevel) {
            // 540x960 portrait — memory-safe on all mobile devices
            const W = 540, H = 960;
            ...
            // ── Theme colors from user CSS vars ──
            ...
            function hline(y, alpha) {
                ...
                g.addColorStop(0.3, rgba(uAccent,alpha||0.3));
                g.addColorStop(0.7, rgba(uProg,  alpha||0.3));
            ...
            function font(weight, size) {
                return `${weight} ${size}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif`;
            }
            ...
            // Ambient glows
            glow(90,  220, 320, uAccent, 0.18);
            glow(450, 780, 300, uRed,    0.12);
            glow(270, 480, 200, uProg,   0.10);

            // Subtle diagonal lines
            ctx.save();
            ctx.strokeStyle = rgba(uAccent, 0.06);
            ctx.lineWidth = 0.5;
            ...
            // ══════════════════════════════════════════════════════
            // SECTION 1 — Compact brand header
            // ══════════════════════════════════════════════════════
            const brandIconR = 26;
            const brandIconY = 52;
            ...
            // Icon circle
            ctx.save();
            const ig = ctx.createRadialGradient(W/2,brandIconY,0,W/2,brandIconY,brandIconR);
            ig.addColorStop(0, rgba(uProg,0.22));
            ig.addColorStop(1, rgba(uAccent,0.06));
            ctx.beginPath(); ctx.arc(W/2,brandIconY,brandIconR,0,Math.PI*2);
            ctx.fillStyle = ig; ctx.fill();
            ctx.strokeStyle = rgba(uAccent,0.28); ctx.lineWidth = 1; ctx.stroke();
            ctx.restore();
            ctx.font = '32px serif';
            ...
            bg.addColorStop(0,'#ffffff'); bg.addColorStop(1,uProg);
            ctx.fillStyle = bg;
            ctx.fillText('Mindkraft', W/2, 96);

            // Tagline
            ctx.font = font(400, 12);
            ctx.fillStyle = 'rgba(176,176,176,0.6)';
            ctx.fillText('Gamify your life.', W/2, 116);

            hline(132, 0.35);

            // ══════════════════════════════════════════════════════
            // SECTION 2 — HERO: Giant progress bar
            // ══════════════════════════════════════════════════════
            const barY   = 154;
            const barH   = 28;
            const barW   = W - PAD*2;
            ...
            // Bar track
            ctx.save();
            roundRect(PAD, barY, barW, barH, barR);
            ctx.fillStyle = 'rgba(255,255,255,0.07)';
            ctx.fill();
            ctx.restore();

            // Bar fill — full width (level complete = 100%)
            const fillGrad = ctx.createLinearGradient(PAD, barY, PAD+barW, barY);
            fillGrad.addColorStop(0, uAccent);
            fillGrad.addColorStop(0.5, uProg);
            fillGrad.addColorStop(1, '#a8d8f0');
            ctx.save();
            roundRect(PAD, barY, barW, barH, barR);
            ...
            roundRect(PAD, barY, barW, barH, barR);
            ctx.clip();
            const shimmer = ctx.createLinearGradient(PAD, barY, PAD+barW, barY+barH);
            ...
            // XP label centered on bar with left/right dash lines
            // Gather data
            const levelStart = window.userData.levelStartedAt
                ? new Date(window.userData.levelStartedAt) : null;
            ...
            const xpLabel = xpThisLevel.toLocaleString() + ' XP this level';
            ctx.font = font(400, 11);
            const xpLW = ctx.measureText(xpLabel).width;
            const xpLX = W/2;
            const xpLY = barY + barH + 18;
            const lineGap = 10;
            const lineY   = xpLY - 5;

            // Left dash line
            ctx.save();
            ctx.strokeStyle = rgba(uProg, 0.4); ctx.lineWidth = 0.75;
            ctx.setLineDash([3,3]);
            ctx.beginPath();
            ctx.moveTo(PAD, lineY);
            ctx.lineTo(xpLX - xpLW/2 - lineGap, lineY);
            ctx.stroke();
            // Arrow left
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.moveTo(PAD+6, lineY-3); ctx.lineTo(PAD, lineY); ctx.lineTo(PAD+6, lineY+3);
            ctx.stroke();
            // Right dash line
            ctx.setLineDash([3,3]);
            ctx.beginPath();
            ctx.moveTo(xpLX + xpLW/2 + lineGap, lineY);
            ctx.lineTo(W-PAD, lineY);
            ctx.stroke();
            // Arrow right
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.moveTo(W-PAD-6, lineY-3); ctx.lineTo(W-PAD, lineY); ctx.lineTo(W-PAD-6, lineY+3);
            ctx.stroke();
            ctx.restore();
            ...
            ctx.textAlign = 'center';
            ctx.fillText(xpLabel, xpLX, xpLY);

            hline(xpLY + 14, 0.25);

            // ══════════════════════════════════════════════════════
            // SECTION 3 — Level number hero
            // ══════════════════════════════════════════════════════
            const levelSecY = xpLY + 34;

            // Soft glow behind number
            glow(W/2, levelSecY + 80, 130, uProg, 0.12);

            // "LEVEL" label — center aligned
            ctx.font = font(400, 18);
            ctx.fillStyle = 'rgba(255,255,255,0.38)';
            ctx.textAlign = 'center';
            ctx.fillText('LEVEL', W/2, levelSecY + 22);

            // Level number — center aligned, large gradient
            const numFS = newLevel >= 100 ? 110 : 130;
            ctx.font = font(800, numFS);
            ctx.textAlign = 'center';
            ctx.textBaseline = 'top';
            const ng = ctx.createLinearGradient(W/2, levelSecY+30, W/2, levelSecY+30+numFS);
            ng.addColorStop(0, '#ffffff');
            ng.addColorStop(0.5, uProg);
            ng.addColorStop(1, uAccent);
            ctx.fillStyle = ng;
            ctx.fillText(String(newLevel), W/2, levelSecY + 30);
            ctx.textBaseline = 'alphabetic';

            // ── Character title pill ──
            const catXP = getCategoryXP();
            const charTitle = getCharacterTitle(newLevel, catXP);
            const titleY = levelSecY + 30 + numFS + 22;

            ctx.font = font(600, 15);
            ctx.textAlign = 'center';
            const ctW = Math.min(ctx.measureText(charTitle).width + 44, 260);
            ctx.save();
            roundRect(W/2-ctW/2, titleY-18, ctW, 30, 15);
            ctx.fillStyle = rgba(uProg, 0.12); ctx.fill();
            ctx.strokeStyle = rgba(uProg, 0.35); ctx.lineWidth = 0.75; ctx.stroke();
            ctx.restore();
            ctx.fillStyle = uProg;
            ctx.fillText(charTitle, W/2, titleY);

            hline(titleY + 18, 0.28);

            // ══════════════════════════════════════════════════════
            // SECTION 4 — Journey stats
            // ══════════════════════════════════════════════════════
            const statsY = titleY + 34;

            // Count activities completed this level
            let actCount = 0;
            if (levelStart) {
                (window.userData.dimensions||[]).forEach(dim=>{
                    (dim.paths||[]).forEach(path=>{
                ...
                // fallback: sum completionCount
                (window.userData.dimensions||[]).forEach(dim=>{
                    (dim.paths||[]).forEach(path=>{
                        (path.activities||[]).forEach(act=>{ actCount += act.completionCount||0; });
                    });
                });
            }
            ...
            const timeStr = (() => {
                ...
                const diff = Date.now() - levelStart.getTime();
                const days = diff / (1000*60*60*24);
                if (days < 1) {
                    const hrs = Math.round(days*24);
                    return hrs <= 1 ? 'in under an hour' : `in ${hrs} hours`;
                }
            ...
            // Stat rows
            const statRows = [];
            if (timeStr) statRows.push({ icon: '⏱', text: `Levelled up ${timeStr}` });
            if (actCount > 0) statRows.push({ icon: '✓', text: `${actCount} ${actCount===1?'activity':'activities'} completed` });
            ...
            statRows.forEach(row => {
                // icon
                ctx.font = '13px serif';
                ctx.fillStyle = rgba(uProg, 0.7);
                ctx.textAlign = 'left';
                ctx.fillText(row.icon, PAD, curY);
                // text
                ctx.font = font(400, 13);
                ctx.fillStyle = 'rgba(220,230,240,0.78)';
                ctx.fillText(row.text, PAD + 20, curY);
                curY += rowH;
            });
            ...
            if (topActs.length > 0) {
                ...
                // Card background
                ctx.save();
                roundRect(PAD, curY-14, W-PAD*2, 44, 8);
                const ag = ctx.createLinearGradient(PAD,curY,W-PAD,curY);
                ag.addColorStop(0, rgba(uAccent,0.14));
                ag.addColorStop(1, rgba(uProg,  0.06));
                ctx.fillStyle = ag; ctx.fill();
                ctx.strokeStyle = rgba(uAccent,0.22); ctx.lineWidth = 0.75; ctx.stroke();
                ctx.restore();
                ...
                let tName = ta.name;
                while (ctx.measureText(tName).width > W-PAD*2-110 && tName.length > 3)
                    tName = tName.slice(0,-2)+'…';
                ctx.fillText(tName, PAD+10, curY+8);

                // Streak
                if (ta.streak > 0) {
                    ctx.font = '12px serif';
                    ctx.fillStyle = uOlive;
                    ctx.fillText(`\uD83D\uDD25 ${ta.streak}d streak`, PAD+10, curY+24);
                }

                // XP badge
                ctx.save();
                roundRect(W-PAD-74, curY-10, 70, 22, 6);
                ctx.fillStyle = rgba(uProg,0.18); ctx.fill(); ctx.restore();
                ctx.font = font(700, 12);
                ctx.fillStyle = uProg; ctx.textAlign = 'center';
                ctx.fillText(`+${ta.xp.toLocaleString()} XP`, W-PAD-39, curY+6);
            ...
            const hasLevelData = Object.values(levelCatXP).some(v=>v>0);
            const top2src = hasLevelData ? levelCatXP : getCategoryXP();
            const top2 = getTop2Categories(top2src);
            ...
            if (top2.length > 0) {
                ...
                const catW = (W-PAD*2-10)/2;
                const catH = 52;
                top2.forEach((cat,i) => {
                    const cx = PAD + i*(catW+10);
                    const cy = curY;
                    ctx.save();
                    roundRect(cx, cy, catW, catH, 7);
                    ctx.fillStyle = rgba(cat.color, 0.1); ctx.fill();
                    ctx.strokeStyle = rgba(cat.color,0.28); ctx.lineWidth=0.75; ctx.stroke();
                    ctx.restore();
                    // emoji
                    ctx.font = '20px serif'; ctx.textAlign = 'left';
                    ctx.fillText(cat.emoji, cx+9, cy+30);
                    // label
                    ctx.font = font(600,13); ctx.fillStyle = cat.color;
                    ctx.fillText(cat.label, cx+34, cy+24);
                    // xp
                    ctx.font = font(400,11); ctx.fillStyle = 'rgba(255,255,255,0.38)';
                    ctx.fillText(cat.xp.toLocaleString()+' XP', cx+34, cy+40);
                });
                curY += catH + 10;
            }

            // ══════════════════════════════════════════════════════
            // FOOTER — pinned toward bottom, share copy
            // ══════════════════════════════════════════════════════
            const footerY = H - 54;
            hline(footerY - 8, 0.22);

            ctx.font = font(500, 13);
            ctx.fillStyle = 'rgba(255,255,255,0.55)';
            ctx.textAlign = 'center';
            ctx.fillText(`I reached Level ${newLevel} on Mindkraft!`, W/2, footerY + 10);

            ctx.font = font(400, 11);
            ctx.fillStyle = 'rgba(255,255,255,0.22)';
            ctx.fillText('Gamify your life at mindkraft.life', W/2, footerY + 28);
